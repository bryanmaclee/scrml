...
export const SERVER_JSON_BODY_GUARD_HELPER: string = [
  ...
  "  let total = 0;",
  "  try {",
  "    // `getReader()` is INSIDE the try on purpose: it THROWS a TypeError when",
  "    // the body stream is already locked or disturbed — an earlier consumer",
  "    // took it, e.g. a `handle()` middleware that called `request.json()` and",
  "    // then passed the same request on. Outside the try that TypeError escaped",
  "    // UNCAUGHT and bypassed this compiler-owned envelope entirely, which is",
  "    // the one guarantee §61.3 exists to make. MEASURED by executing the",
  "    // emitted helper against a locked and a disturbed Request.",
