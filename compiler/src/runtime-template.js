...
// --- chunk-local cell scope (N2/N3/N4 chunk-namespacing) -------------------
//
// Two route chunks that coexist in one live document used to share one cell
// slot per SOURCE-LEVEL name: both declaring \`<rows>\` meant the second chunk to
// evaluate clobbered the first, and then rendered ITS rows into the first
// page's fence. Module scope does not help — under --module-format=esm the
// runtime is one ES module every chunk imports, a singleton by necessity (a
// forked store / subscriber list / rehydrator registry would break reactivity
// outright).
//
// So the STORE stays a singleton and only the KEY SPACE is per-chunk. Each
// emitted chunk opens with
//
//   const { _scrml_reactive_get, _scrml_reactive_set } =
//     _scrml_cell_scope("0a1b2c3d", { appPhase: "0eeeffff$appPhase" });
//
// which SHADOWS the global accessors for that chunk's body. Every call site in
// the body is then byte-identical to the pre-namespacing output —
// \`_scrml_reactive_get("rows")\` — while the key that reaches the store is
// \`0a1b2c3d$rows\`. Effects, event handlers and render fns are all created during
// module init, so they capture this scope lexically; no dynamic scoping is
// involved and there is no window in which the wrong namespace is active.
//
// \`owners\` carries the cells this chunk does NOT own, mapped to their fully
// resolved key. A cross-file imported engine/cell is the SAME instance in every
// importing file (§51.0.A / §51.0.D) — the mechanism IS one shared slot — so an
// imported name must key under the EXPORTER's token, not the importer's.
// (Composition is not such a path: a page reading a shell-declared cell is a
// hard E-STATE-UNDECLARED, whose own text says "import the name if it is
// cross-file".)
//
// DOTTED §6.3.2 compound keys namespace their ROOT only, so the dotted walk
// below still splits on ".". The separator is "$" precisely because it is not
// "." and cannot perturb that split.
function _scrml_cell_key(ns, name, owners) {
  const raw = String(name == null ? "" : name);
  if (!raw) return raw;
  const dot = raw.indexOf(".");
  const root = dot === -1 ? raw : raw.slice(0, dot);
  const owned = owners && owners[root];
  if (owned) return dot === -1 ? owned : owned + raw.slice(dot);
  return ns + "$" + raw;
}

// Recover the author-facing cell name from a namespaced key — for diagnostics,
// devtools captions and error-boundary labels, which must read "rows" and never
// "0a1b2c3d$rows". Tokens are exactly 8 lowercase base36 chars.
function _scrml_cell_name(key) {
...
// Build one chunk's bound accessor set. Called once per chunk at module init;
// the returned closures are what the chunk body destructures over.
function _scrml_cell_scope(ns, owners) {
  const k = (name) => _scrml_cell_key(ns, name, owners);
  return {
    _scrml_reactive_get: (n) => _scrml_reactive_get(k(n)),
    _scrml_reactive_set: (n, v) => _scrml_reactive_set(k(n), v),
    _scrml_init_set: (n, fn) => _scrml_init_set(k(n), fn),
    _scrml_default_set: (n, fn) => _scrml_default_set(k(n), fn),
    _scrml_reset: (n) => _scrml_reset(k(n)),
    _scrml_derived_declare: (n, fn) => _scrml_derived_declare(k(n), fn),
    _scrml_derived_get: (n) => _scrml_derived_get(k(n)),
    // BOTH arguments are cell keys — this registers a dirty-propagation edge
    // between two of them.
    _scrml_derived_subscribe: (d, u) => _scrml_derived_subscribe(k(d), k(u)),
    _scrml_reactive_subscribe: (n, fn) => _scrml_reactive_subscribe(k(n), fn),
    _scrml_reactive_subscribe_when: (n, v, fn) => _scrml_reactive_subscribe_when(k(n), v, fn),
    _scrml_reactive_derived: (n, fn) => _scrml_reactive_derived(k(n), fn),
    _scrml_reactive_debounced: (n, fn, ms) => _scrml_reactive_debounced(k(n), fn, ms),
    _scrml_reactive_throttled: (n, fn, ms) => _scrml_reactive_throttled(k(n), fn, ms),
    _scrml_reactivity_register: (n, kind, ms) => _scrml_reactivity_register(k(n), kind, ms),
    _scrml_reactivity_cancel: (n) => _scrml_reactivity_cancel(k(n)),
    // The seed carries BARE names, so \`_scrml_ssr_seeded\` asks about the bare
    // name and the APPLY maps through k() — not the other way round.
    _scrml_ssr_seeded: (n) => _scrml_ssr_seeded(n),
    _scrml_ssr_seed_apply: (skipShell) => _scrml_ssr_seed_apply_scoped(k, skipShell),
    _scrml_replay: (n, log, i) => _scrml_replay(k(n), log, i),
    _scrml_messages_register_inline: (n, v, o) => _scrml_messages_register_inline(k(n), v, o),
    // The §41.12 Level-1 lookup keys on \`cell::validator\` too, but takes the
    // cell as its THIRD argument. Namespacing only the register side would split
    // the pair and every inline message override would silently fall through to
    // the Level-3 default.
    _scrml_message_for: (err, field, cellName) =>
      _scrml_message_for(err, field, (typeof cellName === "string" && cellName) ? k(cellName) : cellName),
    _scrml_machine_clear_timer: (n) => _scrml_machine_clear_timer(k(n)),
    _scrml_machine_arm_timer: (n, ms, t, meta) => _scrml_machine_arm_timer(k(n), ms, t, meta),
    _scrml_machine_arm_initial: (n, rules, audit) => _scrml_machine_arm_initial(k(n), rules, audit),
    // Engine helpers — the first arg is the engine's own cell.
    _scrml_engine_advance: (n, ...rest) => _scrml_engine_advance(k(n), ...rest),
    _scrml_engine_direct_set: (n, ...rest) => _scrml_engine_direct_set(k(n), ...rest),
    _scrml_engine_hydrate_init: (n, ...rest) => _scrml_engine_hydrate_init(k(n), ...rest),
    _scrml_engine_dispatch_message: (n, ...rest) => _scrml_engine_dispatch_message(k(n), ...rest),
    _scrml_engine_history_capture_on_exit: (n, ...rest) => _scrml_engine_history_capture_on_exit(k(n), ...rest),
    _scrml_engine_arm_state_timers: (n, ...rest) => _scrml_engine_arm_state_timers(k(n), ...rest),
    _scrml_engine_clear_state_timers: (n, ...rest) => _scrml_engine_clear_state_timers(k(n), ...rest),
    _scrml_engine_clear_named_timer: (n, ...rest) => _scrml_engine_clear_named_timer(k(n), ...rest),
    _scrml_engine_arm_idle_watchdog: (n, ...rest) => _scrml_engine_arm_idle_watchdog(k(n), ...rest),
    _scrml_engine_reset_idle_watchdog: (n, ...rest) => _scrml_engine_reset_idle_watchdog(k(n), ...rest),
  };
}
