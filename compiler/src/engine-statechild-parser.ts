...
import { urlSlashesAt, skipDollarBrace } from "./block-splitter.js";
...
export function scanForNestedEngineEntries(bodyRaw: string): NestedEngineEntry[] {
  ...
  while (i < bodyRaw.length) {
    ...
    while (scanned < lt) {
      // An opener's attribute region and a `${…}` block are opaque
      // STRUCTURALLY, so this byte-walk can never step into one and read a
      // `//` / `/*` / `<!--` there as a body comment.
      //
      // ⚑ THE ENGULFING CASE IS THE WHOLE POINT, and the first version of this
      // guard read `op.end <= lt` and DISCARDED the span exactly then. That is
      // backwards. A span that runs PAST `lt` means the candidate opener at
      // `lt` is not real — it is text inside an attribute value or a `${…}`
      // block (`<p title='<engine>…'>`, `${ f('<engine>…') }`). Discarding it
      // dropped the walk back to the byte scan, which no longer tracks `'`, so
      // the mention fired as a genuine opener: a phantom nested engine, a
      // phantom state-child, a spurious second E-ENGINE-INVALID-TRANSITION
      // naming an `<onTransition>` that exists only as attribute text.
      //
      // Engulf ⇒ `hitSkippable`, identical to the `skipCommentOrString` branch
      // immediately below. The two branches now agree; they must stay agreed.
      {
        const op = skipOpaqueSpan(bodyRaw, scanned);
        if (op.end !== scanned) {
          if (op.end > lt) { i = op.end; hitSkippable = true; break; }
          scanned = op.end;
          continue;
        }
      }
 ...
 *
 * ---------------------------------------------------------------------------
 * ATTRIBUTE-QUOTE-FORM — an UNRESOLVED divergence, surfaced not decided
 * ---------------------------------------------------------------------------
 *
 * An earlier revision of this comment said attribute values are "`"`- (or
 * legacy `'`-) delimited per §5.1". **That was wrong.** SPEC.md:1362 is
 * explicit and says the opposite:
 *
 *   "The attribute quoted-string form is `"`-only (double-quote; single-quote
 *    is not an attribute-string delimiter). … scrml uses one string delimiter,
 *    `"`, language-wide."
 *
 * What the implementation ACTUALLY does, measured at this watermark:
 *
 *   `<a title='hi'>` at PAGE level                    -> E-ATTR-001  (enforced)
 *   `<a title='hi'>` INSIDE an engine state-child     -> compiles CLEAN
 *   `${ ['hi'].join("") }` inside a state-child       -> compiles CLEAN
 *
 * So the `"`-only rule IS enforced at page level and is NOT enforced inside an
 * engine state-child body, and `findOpenerEnd` in this file accepts `'` as an
 * attribute delimiter unconditionally. That leniency is precisely the surface
 * the S402 round-4 phantom-opener findings live on: every one of them needs a
 * `'`-delimited attribute or a `'` string to reach the byte-walk at all.
 *
 * **This is NOT decided here, deliberately.** Making the engine locus reject
 * `'`-delimited attributes would be NEWLY-REJECTING — a one-way door and an
 * operator call, not a call for the fix that happened to trip over it. The
 * scanners below are therefore written to be CORRECT UNDER BOTH ANSWERS: they
 * treat a `'`-delimited attribute region as opaque data, which is right if the
 * form stays legal and harmless if it is later rejected upstream.
 *
 ...
 * nesting state these flat scanners do not carry, so it is left OPEN and
 * called out here rather than approximated by deleting the delimiter. (This
 * comment does not assert a ledger entry; it records the boundary. See the
 * test named "PRE-EXISTING, NOT FIXED HERE" for the measurement.)
 *
...
function skipCommentOrString(s: string, i: number, locus: ScanLocus): number {
  ...
  // — is PRE-EXISTING (base fails it identically) and is left OPEN rather than
  // folded in here; closing it needs body-mode nesting awareness in scanners
  // that do not currently have any. Stated as a boundary, not as a ledger
  // claim — this file does not touch the gap ledger.
...
function skipOpaqueSpan(s: string, i: number): { end: number; maskFrom: number } {
  // `${ … }` logic block. Its interior is LOGIC (§3.1), so a comment token
  // there is code, not a body comment, and an `<onTimeout>` / `<engine>`
  // mention there is not a real element.
  //
  // This DELEGATES to block-splitter.js's `skipDollarBrace` rather than
  // brace-counting locally, and that is the point of the round-4 fix. The
  // hand-rolled counter it replaced was not quote-aware, so a `}` inside a
  // string literal (`${ f("}") }`) ended the span early and parked the walk on
  // the now-unpaired `"`, which consumed to EOF and swallowed a following
  // `<onTimeout>` — re-introducing, THROUGH THE SHIELD, the exact
  // silent-drop-at-exit-0 class the shield exists to close. `skipDollarBrace`
  // tracks `"` and `'` with backslash escapes, so the brace depth is right.
  //
  // COUNT OF WHAT IS *NOT* CONVERTED, stated so the next editor does not assume
  // this file has one `${…}` scanner: FIVE other hand-rolled brace counters
  // remain here (findOnTransitionCloser, findEngineCloser, findOpenerEnd,
  // findStateChildCloser, parseMessageArms) and all five share the same
  // quote-blindness. They are PRE-EXISTING and out of this fix's scope —
  // converting them changes behaviour at five sites no finding implicates.
  // This reduces N from 6 to 5; it does not make N == 1.
  if (s[i] === "$" && s[i + 1] === "{") {
    return { end: skipDollarBrace(s, i), maskFrom: i };
  }
  // Markup element opener `<tag …>` / `<tag …/>`; its attribute values are DATA.
  // On the quoting form actually admitted there, see the ATTRIBUTE-QUOTE-FORM
  // note on `skipCommentOrString` above — SPEC §5.1 says `"`-only, the
  // implementation is lenient, and that divergence is SURFACED, not decided.
  if (s[i] === "<") {
    const n = s[i + 1];
...
export function parseEngineStateChildren(rulesRaw: string): EngineStateChildEntry[] {
  ...
  while (i < rulesRaw.length) {
    ...
    let hitSkippable = false;
    while (scanned < lt) {
      // ⚑ This was the ONE re-scan loop of the three that took the
      // `"markup-body"` locus narrowing but no opaque-span shield, and the
      // asymmetry was invisible because the other two sat 1700 lines away.
      // Unshielded, a state-child opener written inside another element's
      // attribute value (`<a title='<Fake rule=.Z>x</>'>`) parsed as a REAL
      // state-child. A phantom entry here is worse than a phantom
      // `<onTimeout>`: this list feeds the variant-coverage check, so it can
      // MASK a genuine E-ENGINE-STATE-CHILD-MISSING and mis-key the state map.
      //
      // Engulfing `lt` ⇒ the `<` we found is inside the span, so bail exactly
      // as the comment branch below does.
      {
        const op = skipOpaqueSpan(rulesRaw, scanned);
        if (op.end !== scanned) {
          if (op.end > lt) { i = op.end; hitSkippable = true; break; }
          scanned = op.end;
          continue;
    ...
    if (!next || next < "A" || next > "Z") {
      // ⚑ A LOWERCASE opener must be stepped OVER, not stepped INTO.
      //
      // `i = lt + 1` lands the cursor one byte past the `<`, i.e. INSIDE the
      // opener, and the next iteration then scans the attribute region as if it
      // were body text. On base that was survivable only because the `'`/`"`
      // span skip swallowed the attribute VALUE by luck. With `'` no longer a
      // delimiter, `<a title='<Fake rule=.Z>x</>'>` handed `<Fake` to the
      // state-child branch below and produced a PHANTOM state-child — which is
      // worse than a phantom timer, because this list feeds the variant-coverage
      // check and can MASK a genuine E-ENGINE-STATE-CHILD-MISSING.
      //
      // `skipOpaqueSpan` jumps the whole opener via the quote-, paren-, `${}`-
      // and angle-aware `findOpenerEnd`, which is what the WALKER scanners have
      // always done here. This makes the top-level splitter agree with them.
      // Falls through to the old byte-advance for a closer (`</…`), a stray `<`,
      // or a malformed opener with no `>`.
      const op = skipOpaqueSpan(rulesRaw, lt);
      i = op.end !== lt ? op.end : lt + 1;
      continue;
    }
