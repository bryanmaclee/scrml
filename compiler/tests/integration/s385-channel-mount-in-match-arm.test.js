...
`);
  ...
  test("inside an `<each>` that is a DIRECT entry of an `<engine>` body (hole 4)", () => {
    // Round-7. Round 6's engine loop marked EVERY direct `bodyChildren` entry
    // `seen` and then walked only its `.children`. A structural block has none
    // — traced on the built AST, `engine-decl.bodyChildren[1]` is
    // `kind: "each-block"` with keys `[…,bodyChildren,templateChildren,…]` and
    // NO `children` — so the `seen.add` made the whole subtree permanently
    // unreachable.
    //
    // MEASURED ON ROUND 6's OWN COMPILER, i.e. the gate bites in both
    // directions rather than having merely never failed: guard 0, `_scrml_ws`
    // 0, and `document.createElement("probeChan")` present in the client
    // bundle. Exit 0, silent dead channel. (Pre-existing — base was equally
    // blind.)
    const { result } = compileIn("y3", `<program>
    ${D}{
        import { "probe" as probeChan } from './chan.scrml'
        type LoadPhase:enum = { Idle, Done }
        <rows> = [1, 2]
...
`);
    ...
    expect(diag).toBeTruthy();
    // Attribution is to the NEAREST container, so the `<each>` — not the
    // engine it happens to sit in.
    expect(diag.message).toMatch(/mounted inside an `<each>` body/);
    expect(diag.message).toMatch(/including inside this `<each>` body/);
    expect(diag.message).not.toMatch(/this arm/);
  });

  test("inside a `<match>` that is a DIRECT entry of an `<engine>` body (hole 4)", () => {
    // The same hole through the other structural block: a direct-entry
    // `<match>` keeps its arms in `armBodyChildren`, equally invisible to a
    // `.children`-only walk.
    const { result } = compileIn("y4", `<program>
    ${D}{
        import { "probe" as probeChan } from './chan.scrml'
        type LoadPhase:enum = { Idle, Done }
        type Phase:enum = { Loading, Ready }
...
`);
  ...
  test("REGRESSION GUARD — the round-3 collision class, with a direct-entry `<each>` in the SAME engine", () => {
    // The round-7 widening's own blast radius. The engine loop now recurses
    // into direct entries that are NOT `kind: "markup"`; this file pairs that
    // widening with the exact shape it could plausibly break — an alias named
    // `Done` beside a `<Done rule=…>` state-child, in an engine that also holds
    // a direct `<each>`.
    //
    // It cannot break, by construction rather than by luck: `fire()` is gated
    // `kind === "markup"`, and direct MARKUP entries are still marked `seen`
    // and never classified. Pinned anyway, because "by construction" is a claim
    // and this is the measurement.
    const { result } = compileIn("y5", `<program>
    ${D}{
        import { "probe" as Done } from './chan.scrml'
        type LoadPhase:enum = { Idle, Done }
        <rows> = [1, 2]
