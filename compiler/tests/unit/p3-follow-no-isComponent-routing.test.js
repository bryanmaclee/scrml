...
const ALLOWED = {
  "block-splitter.js": 29,         // 14 stamps + doc comments + S101 §4.17 raw-content gate (!isComp guard + emit-stamp + comment) + S101 Bug-3 compound-state-decl text-emit stamp + S107 Phase 2 §18.0.1 structural raw-body gate (!isComp guard + emit-stamp) + R25-Bug-40 S136 §4.14 `:`-shorthand body gate (markup-shorthand emit-stamp + state-shorthand emit-stamp + caller-1 isComponentName(stateName) write-side stamp) + S139 Bug 51-C Shape 1/2 state-decl text-emit stamp (write-side, mirror of S101 Bug-3 compound stamp) + S177 bug-74 E-CLOSER-001 recovery-leaf `isComponent: isComp` write-side stamp + jwt-auth-bypass (2026-07-11) `/* */` block-comment-block push `isComponent: false` write-side stamp (mirror of the `//` line-comment-block push)
  "ast-builder.js": 30,            // 20 stamps + parseAttributes param + comments + S159 §4.14 HTML `:`-shorthand synthesis gate (intra-stage read of BS's write-side `isComponent` stamp — `block.isComponent !== true` excludes components from body synthesis — + its doc comment; same allowed intra-stage-read family as lines 670/7212/7226) + GITI-029 TOPLEVEL_ON_LIFECYCLE_RE lift gate `isComponent: false` write-side stamp (synthetic ${...} for a comment-flushed on-mount, mirror of the @-write / tilde lift stamps) + §23.4 USE_FOREIGN_LIFT_RE lift gate `isComponent: false` write-side stamp (synthetic ${...} for a bare `use foreign:` sidecar decl, same lift-stamp family) + §21.3.1 IMPORT_HOST lift gate `isComponent: false` write-side stamp (synthetic ${...} for a comment-joined `import:host` decl, same lift-stamp family)
