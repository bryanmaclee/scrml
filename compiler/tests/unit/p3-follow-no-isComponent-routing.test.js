...
const ALLOWED = {
  ...
  "ast-builder.js": 28,            // 20 stamps + parseAttributes param + comments + S159 §4.14 HTML `:`-shorthand synthesis gate (intra-stage read of BS's write-side `isComponent` stamp — `block.isComponent !== true` excludes components from body synthesis — + its doc comment; same allowed intra-stage-read family as lines 670/7212/7226) + GITI-029 TOPLEVEL_ON_LIFECYCLE_RE lift gate `isComponent: false` write-side stamp (synthetic ${...} for a comment-flushed on-mount, mirror of the @-write / tilde lift stamps)
