...
describe("C22 §C22.8 — qualified `Phase.Idle` regression (MemberExpr)", () => {
  test("`<phase>: Phase = Phase.Idle` still produces valid JS (qualified form)", () => {
    ...
    // call at a §40.8 default-logic body-top does not run: MEASURED at S368,
    // this fixture's emitted client JS contained NO lowered render call at all
    // (only `_scrml_render_value`, the `${@phase}` interpolation lowering) while
    // the literal string `render(m)` shipped into the emitted HTML as page text.
    // The fixture was a latent instance of the open §40.8 defect, and this test
    // never noticed because it asserts on the `_scrml_reactive_set` line, not on
    // render. SPEC §20.3a: the built-in is a `render(...)` call *in logic
    // position* — which is what the wrapper now makes it.
    //
    // ⚑ NOTHING DIAGNOSES THE BARE FORM TODAY. The S368 bare-call gate is HELD
    // (S379), so reverting this wrapper does NOT make the test fail loudly — it
    // makes the fixture silently stop exercising `render`. Keep it wrapped.
