...
I'm also in the middle of a much bigger second pass — call it v0.2.0 — that I want to be transparent about. It's a breaking change. It's been planned across multiple sessions of design work. The spec reflects it. The compiler is in the early phase of catching up.

This post tells you what's stable, what's coming, and how to read articles that reference scrml right now.
...
The compiler-source critical path — Phase A1 (foundational lex/parse) → A2 (structural elements) → A3 (validators) → A4 (schema/refinement/pinned) → A5 (resolver/typer) → A6 (codegen/runtime) — is roughly 145-235 hours of focused engineering. Parallel tracks (examples rewrite, sample curation, stdlib audit, self-host bootstrap, editor support) add another ~100-200 hours, much of which runs alongside the critical path.

Realistic calendar: 3-9 months depending on session cadence. It will ship when it's done, not on a date.
