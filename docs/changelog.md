...
Current baseline (2026-05-23 **S123 — Unit CC**). Full `bun run test` **19,933 pass / 0 fail / 175 skip / 1 todo across 756 files** (post-Unit-CC). S122 CLOSE baseline (pre-Unit-CC): 19,907 pass.

### 2026-05-23 (S123 — Unit CC: E-WRITE-NOT-IN-LOGIC-CONTEXT on bare `@x = expr` at default-logic body-top — **breaking change**)

**Unit CC** lands as the COMPANION to V-kill (S123 commits `c22b3fda` + `c2d2741a` + `489e5943`). V-kill killed silent auto-state-cell synthesis from `@x = expr` writes inside `fn` / `function` / user-written `${...}` body contexts and fires `E-STATE-UNDECLARED` on undeclared targets. V-kill explicitly carved out default-logic mode (the §40.8 `<program>`/`<page>`/`<channel>` body-top auto-lift wrapper) for Unit CC's territory. Unit CC closes that case: bare `@x = expr` at the IMMEDIATE body-top of `<program>` / `<page>` / `<channel>` now fires `E-WRITE-NOT-IN-LOGIC-CONTEXT` per the S122 user-voice Option-2 ratification (auto-lift covers DECLARATIONS only — `<x> = 0`, `function f() { }` — NOT writes; writes are logic; logic goes in `${...}`).

**Implementation**:
- `compiler/src/ast-builder.js`: new `TOPLEVEL_AT_WRITE_RE` lift wraps bare `@x = expr` text at `<program>`/`<page>`/`<channel>` direct-child positions in a synthetic `${...}` so the parser observes the write (pre-Unit-CC the text was silently dropped — bug-q-1 reproducer). New `isDefaultLogicBody` parameter to `liftBareDeclarations` discriminates the §40.8 default-logic-mode surface from `<db>`/`<state>` STATE-block bodies (where bare `@x = []` is the canonical V5-strict reactive-cell declaration). New `_nestedBlockDepth` counter on `parseLogicBody` discriminates IMMEDIATE body-top writes (Unit CC fires) from writes nested inside fn/function bodies under synthetic wrappers (V-kill carve-out preserved). V5-strict `@name = expr` parse sites (both `parseOneStatement` and main loop) tag emissions with `_isUnitCCWrite: true` at IMMEDIATE body-top of synthetic lift wrappers.
- `compiler/src/symbol-table.ts`: SYM PASS 3 `walkResolveAtNames` state-decl arm fires `E-WRITE-NOT-IN-LOGIC-CONTEXT` on `_isUnitCCWrite`-tagged nodes (parallel to V-kill's E-STATE-UNDECLARED fire). Path-based per-file exemption via `isUnitCCExempt(filePath)` (suffix-with-boundary match against `compiler/src/unit-cc-exemption-list.json`).
- `compiler/src/unit-cc-exemption-list.json`: per-file exemption registry for the pre-S123 corpus. Each adopter source file sunsets its exemption manually as migration to structural decls (`<name> = init`) or explicit logic wrappers (`${ @name = ... }`) completes. Per-file sunset (not auto-delete) because these are adopter source files, not scheduled-deletion targets like V-kill's `compiler/native-parser/*.scrml` exemption.
- SPEC §40.8 amendment: new S123 normative bullet documenting the auto-lift-covers-declarations-only rule + Unit CC discrimination + per-file exemption mechanism.
- SPEC §34 catalog: new `E-WRITE-NOT-IN-LOGIC-CONTEXT` row sibling to `E-STATE-UNDECLARED`.
- New test: `compiler/tests/unit/unit-cc-write-at-body-top.test.js` — 7 cases covering body-top fire, explicit-logic-block clean, structural-decl clean, V-kill nested fire, V-kill carve-out preservation, combined-shape clean.

**Bug Q closure note**: bug-q-1 (`<program>` body opening with `@cell = X` produced silent runtime failure) is now resolved at the compile-time loud-error layer. The bare write text is lifted via `TOPLEVEL_AT_WRITE_RE` and reaches the SYM PASS 3 fire site instead of being silently dropped.

**Out-of-scope** (sequential follow-ups): corpus migration of the exempted files (each adopter migrates as they touch the file); V-kill expansion to nested writes under synthetic wrappers (deliberately not expanded — kept narrow to bound blast radius); BUG-META-6 meta-context territory (preserved exemption).
