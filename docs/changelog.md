...
Current baseline (2026-05-20 **S111 CLOSE** — a design-direction session: the quoted-text-model investigation reached GO and landed its SPEC Wave 1 (§4.18); the native-parser **charter B** decision then expanded the native-parser project to replace the whole compiler front-end, folding quoted-text into it. The `compiler/`-tree artifacts are the SPEC §4.18 amendment (+252 lines) + the §4.18.1/§40.8 body-mode reconcile (R3, +3 lines); no `compiler/src/` changed, so test numbers carry unchanged from S109's 18-commit landing arc: Bug 2 C-narrow + Bug 1 ring partial closure + date/timestamp BUILTIN_TYPES + benchmarks refresh + match block-form Phase 5 (wildcard `<_>` explicit render + full-pipeline integration gap fix + payload-bearing enum exhaustiveness fix + sample + browser test) + 2 vacuous-test fixes + bare-variant inference SCOPING): pre-commit subset **13,362 pass / 88 skip / 1 todo / 0 fail / 694 files / 44,898 expect**; full `bun test compiler/tests/` **16,213 pass / 169 skip / 1 todo / 0 fail / 728 files / 47,333 expect**. Delta vs S108 close (16,147 / 723 / 47,209): **+66 pass / +5 files / +124 expect / 0 fail / 0 regressions**. **Headlines:** (1) **Bug 2 phantom E-SYNTAX-050 cascade — C-narrow fix** — adopter dogfood report; a bisecting reducer found the reporter's multi-line-`<a>`-hypothesis WRONG, the real trigger is *any unpaired `'` or `"` in markup-text body*. `block-splitter.js` ran global string-mode tracking in markup-text; an unpaired quote ate the rest of the file → cascade of unclosed-element errors with wrong line numbers. Fix: removed the markup-text-level quote-tracking block (sibling locus argument to Bug 4 C-narrow — strings are Logic-context, not markup-text). (2) **Match block-form was never end-to-end functional in a real compile — integration gap found + fixed** — `collectMatchBlocks` walked `fileAST.nodes` but the pipeline passes an outer wrapper with nodes under `fileAST.ast.nodes` → real compiles found 0 match-blocks → the dispatcher was never emitted. The S108 unit tests passed because they call the codegen helper with the bare AST. The S107-S108 "end-to-end functional" framing was overclaimed; now genuinely true. (3) **Match block-form Phase 5 — wildcard `<_>` explicit render + payload-bearing enum exhaustiveness fix + sample + 6-test browser arm-swap suite** — the wildcard arm now emits as the dispatcher's catch-all `else` branch; `extractEnumVariants` no longer reads `count`/`int` as phantom variants from a tokenizer-joined `Ready ( count : int )` payload-bearing enum; runtime arm-swap is now verified in happy-dom. (4) **Bug 1 ring family partial closure** — `ring-[length|color|var|keyword]` arbitrary-value emit; ring-offset + gradient still deferred (need preflight CSS infrastructure). (5) **date/timestamp BUILTIN_TYPES** — formalized as `tPrimitive` (tableFor v1.next item). **Plus**: 2 vacuous-test fixes (`compileScrml(stringPath)` silently compiles nothing — one shipped same session in `3609985`, one pre-existing in `sql-nobatch.test.js`; both fixed + a grep sweep confirmed no others) · benchmarks refresh (build −44% vs v0.3.0 STABLE via accumulated PGO Phase 3 work) · bare-variant inference SCOPING (two spurious-error gaps isolated for a ~3-4h follow-on dispatch) · 3 stale browser-conditionals assertions corrected (asserted pre-S108-Bug-5-P2 behavior; ran stale on un-recompiled `dist/`).
