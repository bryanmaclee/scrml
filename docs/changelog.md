...
Current baseline (2026-05-10 S77 close — PA-VERIFIED): **10,961 pass / 64 skip / 1 todo / 6 fail**. The 6 fails are ALL ENVIRONMENTAL on this machine (3 self-host artifacts not built; 3 test-bind A6-5 integration tests with hard-coded `/home/bryan-maclee/` cwd). Net delta vs S76 close: **+82 pass / +4 skip / +0 todo / +6 fail (env-only on this machine)** across **7 SHIP commits + 2 chore commits + 1 SPEC-docs commit on scrmlTS + 1 deep-dive-refresh commit on scrml-support**. **S77 closed: A5 computed-delay family (A5-4 + A5-5 + A5-5b SHIPPED — engine `<onTimeout>` codegen + computed-delay across both temporal surfaces + legacy `<machine>` body-parser `${...}` preservation); A5-6 Feature 2 SHIPPED (engine event-timeout watchdog `<onIdle>`); STRING-token quote-preservation across all 4 test-block parsers; codegen-tightening (multi-statement `~{}` bodies); memory-leak detection deep-dive REFRESHED in scrml-support (Stage 7.5 → 7.6; W-LEAK-010 NEW info-level lint); SPEC W-LEAK-010 catalog row + §51.12.4 chained-rearm-skips-computed amendment.**
