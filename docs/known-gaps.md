...
**FULL ARC SCOPED (deferred, status=open):** add safety-aware Migration 4 + settle SSE + run across SPEC/kickstarter/PRIMER/examples with per-file compile-verify, preserving `server fn`. Dispatch-shaped. <!-- @gap id=g-server-keyword-drift sev=LOW status=open -->
