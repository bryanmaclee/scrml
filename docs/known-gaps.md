...
<!-- @gap id=g-derived-engine-expression-form sev=LOW status=open -->
...
### G-CHANNEL-TOPIC-FORWARD-REF — `<channel topic=@var>` fires E-SCOPE-001 when `@var` is declared AFTER the channel in source order (forward-ref doesn't hoist) — `NEW S186; LOW; TS-pass scope-check ordering`
<!-- @gap id=g-channel-topic-forward-ref sev=LOW status=open -->
...
<!-- @gap id=g-markup-const-consumes-cell-decl sev=LOW status=open -->
...
<!-- @gap id=g-derived-rhs-interp-wrapped sev=LOW status=open -->
...
### G-ATTR-GTE-TAGCLOSE — bare `>=` in an unparenthesized attribute value breaks (leading `>` consumed as tag-close); `>`/`<`/`<=`/`==`/`!=` all work — `NEW S188 dog-food; LOW; attr-value scanner two-char-op`
<!-- @gap id=g-attr-gte-tagclose sev=LOW status=open -->

**Surfaced S188 dog-food (channels §38 → derived/expr sweep).** `<h1 if=@n >= 3>X</h1>` — a bare (unparenthesized) `>=` comparison in an attribute value — breaks (the leading `>` of the two-char `>=` is greedily consumed as the opening-tag close `>`, leaving `= 3>` malformed). **Isolated (verify-before-claim), per-operator:** bare `>` (opgt) → CLEAN; `<` (oplt) → CLEAN; `<=` (ople) → CLEAN; `==` (opeq) → CLEAN; `!=` (opne) → CLEAN; **`>=` (opge) → FAIL** — `>=` is the SOLE comparison operator that breaks bare in an attribute. So the attr-value scanner resolves a lone `>` as a comparison (not tag-close) but does NOT special-case the two-char `>=` (the `>` is taken first). **Severity LOW** — narrow (only `>=`); clean workaround = parenthesize (`if=(@n >= 3)` → confirmed clean, as are all the other bare ops). Surprising because `>=` is the natural spelling and the one operator that fails. **Fix direction:** the attribute-value scanner's `>`-vs-tag-close heuristic must look ahead for a following `=` (a `>=` two-char operator) before treating `>` as the tag close. Repros: `/tmp/dfn/opge.scrml` (fail) vs `opgt/oplt/ople/opeq/opne.scrml` + `we.scrml` (clean).


### G-ATTR-UNQUOTED-COMPOUND-SILENT-DROP — `<p if=@x && @y>` (unquoted compound attr condition) SILENTLY drops `&& @y` → emits `if (@x)` only (wrong condition, no hard error) — `NEW S188; MED; attr-value scanner silent-wrong-output`
<!-- @gap id=g-attr-unquoted-compound-silent-drop sev=MED status=open -->

**Surfaced S188 by the g-not-neg attr-hole follow-up + PA-isolated.** SPEC §5.5.2 admits exactly 4 unquoted attribute-condition forms (`@var` / `obj.prop` / `(expr)` / `fn()`); a COMPOUND condition (`&&` / `||`) MUST be parenthesized (`if=(@x && @y)`). But the compiler does NOT cleanly reject the unquoted-compound form — `<p if=@x && @y>` **compiles** and SILENTLY DROPS the `&& @y`: the emitted if-condition is `_scrml_reactive_get("x")` ONLY (the `&& @y` is gone), with the sole tell an easy-to-miss `E-DG-002` ("`@y` … never consumed"). **Silent-WRONG-OUTPUT** — the `if` checks the wrong condition (just `@x`), so the rendered logic is incorrect with no hard error. **Isolated (verify-before-claim):** `<p if=@x && @y>` (uc1) → compiles, condition = `@x` only + E-DG-002 on `@y`; `<p if=(@x && @y)>` (uc2) → clean + correct; `<p if=@x>` single (uc3) → clean. The unquoted-attr-value scanner truncates at the operator. **Severity MED** — silent-wrong-output is worse than the loud-fail sibling [[g-attr-gte-tagclose]] (`>=` → E-CODEGEN); clean workaround = parenthesize. **Cluster-A** (the tokenizer unquoted-attr-value reader — same region as g-attr-gte-tagclose + the g-not-neg attr-bare fix `tokenizer.ts` `isPrefixNotOperandAhead`). Same "SPEC-invalid form not cleanly rejected" class as [[g-not-negation-unenforced]] / [[g-given-rebind-not-rejected]]. **Fix:** the unquoted-attr-value scanner must either capture the full compound expression OR cleanly diagnose ("compound attribute condition must be parenthesized: `if=(@x && @y)`", per §5.5.2) — do NOT silently drop. Repros: `/tmp/dfn/uc1.scrml` (drops `&& @y`) vs `/tmp/dfn/uc2.scrml` (paren, correct) vs `/tmp/dfn/uc3.scrml` (single, clean).
