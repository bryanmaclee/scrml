...
### Bug 70 — `@.`-iter-arg in a Tier-0 for-lift CALL-REF event handler (`onclick=ping(@.id)`) emits raw `@.` → `E-CODEGEN-INVALID-JS` — `NEW S157; MED; DEFERRED` (PRE-EXISTING, NOT engine-related; surfaced by the Bug 65 fix agent)
