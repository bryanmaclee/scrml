...
**Bug 51-A — CE drops `_scope` from new FileAST (silent codegen miss). RESOLVED S139 `<pending>`.**
...
**Bug 51-B — Shape 2 empty-string init produces empty-arg `_scrml_reactive_set` emit. RESOLVED S139 `<pending>`.**
