...
|---|---|---|---|---|
| R28-7 | HIGH* | OPEN (decision-pending) | schemaFor + tableFor reject `T \| not` (nullable) optional struct fields as "unmappable union" — `E-SCHEMAFOR-NO-SQL-MAPPING` / `E-TABLEFOR-NO-DISPLAY-MAPPING` | `T \| not` is NOT in the SPEC unmappable-type lists (§41.15.8/§41.16.6 = fn/Promise/snippet/foreign/proxy); §14.8.3 normatively ties nullable-column ↔ `T \| not` (bidirectional — compiler introspects nullable→`T\|not` but refuses to lower back). **5/5 devs.** PA-confirmed minimal repro. The L22 "define once" promise breaks on the most common DB shape. *Severity + v1.0-vs-v1.next scope = PA-decision.* |
| R28-6 | HIGH | OPEN | variant-progression `transition()` enforcement DORMANT on the `.get()` SQL-row return path — omitting `transition()` before post-transition field access compiles exit-0 | SPEC §14.12.6.2/§14.12.10 normatively require `E-TYPE-LIFECYCLE-VARIANT-NOT-TRANSITIONED`. Mechanism IS wired (svelte's enum-payload shape fires) but the lifecycle annotation is lost through the loosely-typed `.get()` SQL-row return. 4 dormant / 1 fires. PA-confirmed (`/tmp/r28-pa/r6_dormant.scrml`). The round's design-target surface; real type-safety hole on the common adopter shape. Don't soft-classify. |
| R28-2 | HIGH | OPEN — **un-defer Bug 54** | tableFor `<column>` row-access broken BOTH ways: `:let={(row)=>…}` (§41.16.3-MANDATED) → `W-ATTR-001`/forwarded-as-HTML (gate-fires in full files); `@row` (§41.16.10-DEFERRED) → emits `_scrml_reactive_get("row")` (reads a nonexistent cell, **not** the row binding) | Spec-impl INVERSION: the mandated form is unimplemented, the deferred form has wrong codegen. = the known DEFERRED **Bug 54** (`:let` parse-layer); the `@row` wrong-lowering is NOVEL. Both documented paths broken → tableFor column slots non-functional for row data. PA-confirmed minimal (`/tmp/r28-pa/r2.scrml`+`r2b.scrml`). 4/5. |
| R28-1 | HIGH | OPEN | `@.` each-sigil leaks raw into emitted JS for block-form `<match on=@.field>` nested in `<each … as alias>` → gate-caught `E-CODEGEN-INVALID-JS` (`_scrml_match_match_NNN_dispatch(@.field)`) | The `@.`-to-loop-var lowering is dropped for the match dispatch expr when an `as`-alias is active on the enclosing `<each>`. Both constructs canonical individually; only the combination fails. **Context-dependent** — does NOT reproduce in isolation (multi-match dispatch-counter interaction); reproducers are `dev-2-go.scrml`/`dev-3-elixir.scrml`/`dev-5-pascal.scrml`. Gate true-positive. 3/5. Workaround: `on=alias.field`. |
| R28-3 | HIGH | OPEN | `:`-shorthand engine state-child (`<Variant> : expr`) preceded by a `//` markup comment breaks block-splitting → `W-PROGRAM-001` + `E-CTX-001`/`E-CTX-003` (closer-stack unwinds); bodied `</>` form compiles clean at the same position | BS tag-stack accounting (R24-BUG-4 generic-closer lineage). The `//` comment is a standalone co-trigger; trips the **kickstarter §4.1 Mario flagship shape** (`<Small rule=.Big> : '🧍'`). Diagnostic message also inaccurate ("inside ${} logic body" — none present). PA-confirmed (`/tmp/r28-pa/r3c.scrml` fires; `r3.scrml` comment-free clean). 2/5 (go/svelte). |
| R28-4 | MED | OPEN | `E-PA-002` advertises a `?{} CREATE TABLE` resolution but the PA introspection scanner ignores `CREATE TABLE` in `?{}` blocks (top-level AND inside fn bodies) | Misleading diagnostic — the error's own suggested fix doesn't satisfy the check. (Companion claim — `<schema>`/`schemaFor` DDL should satisfy `<db>` introspection — is NOT a bug; SPEC is silent on a connection, the surfaces are intentionally decoupled.) elixir; overseer-confirmed. Don't soft-classify (advertises a non-working escape hatch). |
| R28-5 | MED | OPEN — **= C4** | `E-TYPE-001` dormant on object-literal struct construction (`const a: T = {…}; a.field`) | lifecycle tracked only on Shape 1 reactive cells, not local object-literal const/let bindings. **= the known C4** (object-literal lifecycle dormancy) — R28 adds a clean adopter reproducer (`/tmp/r28-pa/`, pascal's `(not to string)` field probe). Fold into C4. |
| R28-C1 | HIGH | CANON-FIX | SPEC §14.12.6.2 + PRIMER §6.5 flagship variant-progression worked example uses `server fn … -> (.A to .B) { ?{} }` → `E-FN-001` (fn is pure; `server` doesn't lift the SQL ban) | The flagship example doesn't compile verbatim; should be `server function`. 4/5 devs hit it. Compiler CORRECT; canon wrong. Fix SPEC §14.12.6.2 (line ~8118) + PRIMER §6.5. (Also §14.12.6.x examples use unimplemented `print()`.) |
...
| R28-C2 | MED | CANON-FIX | kickstarter drift bundle: §11.3 stale file-level `<channel>` (SPEC §38.1 v0.3 = inside-`<program>`; would fire `E-CHANNEL-OUTSIDE-PROGRAM`); §11.13 SSE recipe omits `sleep` import; `< db>`/`< schema>` leading-space (`W-WHITESPACE-001`); `print()` in SPEC §14.12.6.x examples | lints/SPEC CORRECT; kickstarter stale (2026-05-04 predates v0.3 reversals). Canon-maintenance backlog (overlaps the §R27 canon-drift note). |
...
| C9 | LOW | OPEN | E-DG-002 false-positive: state read only inside a derived `.filter()` arrow flagged "never consumed" | runtime subscribes correctly; DG consumption-tracker under-counts arrow-body reads. |
