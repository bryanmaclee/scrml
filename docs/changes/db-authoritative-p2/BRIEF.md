...
On the RediLedger shape (`invoices` with an `immutable` column + a SECDEF `fn`), after `scrml db-migrate` applies as owner, a 4-assertion live-PG negative test:
