#!/bin/bash
# Diff two corpus sweeps produced by sweep.sh. Run from WORKTREE_ROOT.
# Usage: bash docs/changes/s385-each-in-scope-check/diff.sh <before-label> <after-label>
#        (default: base build-r2)
...
echo "NEWLY-FAILING (PASS->FAIL): $(grep -c '^PASS->FAIL' "$D/flips.txt")"
grep '^PASS->FAIL' "$D/flips.txt" | sed 's/^PASS->FAIL\t/  /'
echo "NEWLY-PASSING (FAIL->PASS): $(grep -c '^FAIL->PASS' "$D/flips.txt")"
grep '^FAIL->PASS' "$D/flips.txt" | sed 's/^FAIL->PASS\t/  /'

echo
echo "===== DIAGNOSTIC CODE-SET CHANGES (incl. files that stayed FAIL) ====="
join -t$'\t' -j1 <(sort -t$'\t' -k1,1 "$BASE") <(sort -t$'\t' -k1,1 "$BUILD") \
  | awk -F'\t' '$3 != $5 { print $1"\n    before: "$3"\n    after : "$5 }' > "$D/codeset-changes.txt"
echo "files with a changed code set: $(grep -cv '^ ' "$D/codeset-changes.txt")"
