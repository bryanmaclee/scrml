...
| K8 | The native-parser `.scrml` files are uniformly `function`-form (M1/M2 carry-over of the OQ4/D4-P2 pre-§48.6.4 workaround). SPEC §48.6.4 (`fn` file-scope hoisting + mutual recursion; landed S98, parser-recognition shipped S105) now unblocks the canonical Pillar-5b `fn`-form. | M3.4 | M-ladder cleanup — refactor every parse-function across `parse-stmt` / `parse-expr` / `parse-markup` / … `function` → `fn`. Whole-native-parser scope; co-gated on K2 (the `.scrml` won't compile cleanly until the circular import is fixed). Non-blocking for M4/MK4 — the `.js` shadows are unaffected. |
...
| K9 | Markup-layer circular import — the MK-era twin of K2: `block-context.scrml` ↔ `parse-ctx.scrml` circular import (E-IMPORT-002) + aliased imports (`push as pushBracket`, `depth as bracketDepth`, `isTagNameChar as tagNameCharCanonical` — E-SCOPE-001, 9-11 per file) across `block-context` / `parse-ctx` / `parse-markup` / `tag-frame`. Surfaced by the M1.x cluster's K2-gating sweep (9 of 27 native-parser `.scrml` still fail post-K2; this is group 1). The `.js` shadows execute fine (full suite passes). | M1.x cluster (S113) | Follow-up dispatch — mirror the K2 recipe: extract the shared `block-context`↔`parse-ctx` surface into a leaf + de-alias the markup-file imports. Must be fixed before M6 (self-host), like K2. |
| K10 | `ast-expr.scrml` (~line 575) uses `!= not` — fires E-EQ-002; should be `is not` (the canonical absence check). `parse-expr.scrml` + `parse-stmt.scrml` import `ast-expr` and transitively inherit the `.scrml`-compile failure (K2-gating sweep group 2). The `.js` shadows are unaffected. | M1.x cluster (S113) | One-line scrml-source fix (`!= not` → `is not`). Sequence AFTER M4 — M4.1-M4.3 are editing `ast-expr.scrml`; avoid a file collision. |
