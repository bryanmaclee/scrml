...
// puppeteer's bundled default resolves to a version that may not be installed.
// Resolve the locally-cached Chromium instead: $SCRML_CHROME wins, else the
// first build under the current user's puppeteer cache.
...
const browser = await puppeteer.launch({ headless: true, args: ["--no-sandbox"], executablePath: resolveChromePath() });
