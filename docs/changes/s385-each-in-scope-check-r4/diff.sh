#!/bin/bash
# Diff two corpus sweeps produced by the r4 sweep.sh. Run from WORKTREE_ROOT.
# Usage: bash docs/changes/s385-each-in-scope-check-r4/diff.sh <before-label> <after-label>
#        (default: base build-r4)
D="$(pwd)/docs/changes/s385-each-in-scope-check-r4"
BL="${1:-base}"
AL="${2:-build-r4}"
BASE="$D/$BL.tsv"
BUILD="$D/$AL.tsv"
