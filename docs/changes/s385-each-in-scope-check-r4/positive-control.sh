...
#
# Usage:  bash docs/changes/s385-each-in-scope-check-r4/positive-control.sh [in|key] [sweep-label]
#         (defaults: in build-r4)   Run from WORKTREE_ROOT, after sweep.sh.
set -u
WT="$(pwd)"
SLOT="${1:-in}"
LABEL="${2:-build-r4}"
D="$WT/docs/changes/s385-each-in-scope-check-r4"
SP="$(mktemp -d)"
trap 'rm -rf "$SP"' EXIT
