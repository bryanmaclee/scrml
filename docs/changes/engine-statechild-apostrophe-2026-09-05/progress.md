...
# S402 ROUND 4 — the shield became the hole

Round 3 added `skipOpaqueSpan` to close review findings 1+2. It introduced five
regressions of its own, **including a fresh instance of the exact
silent-drop-at-exit-0 class it was written to close.** All five reproduce; all
five are fixed.

Method: file-copy swap inside ONE checkout, three variants —
**TRUEBASE** (`git show origin/main:…`, verified byte-identical to the
`base-parser.ts` used in earlier rounds), **SHIP-r3** (the regressed branch),
**SHIP-r4** (landed).
...
| probe | base | r3 | r4 |
|---|---|---|---|
| `scanForNestedEngineEntries("<p title='<engine>…'>y</p>")` | 0 | **1** | 0 |
| `scanForNestedEngineEntries("${ f('<engine>…') }")` | 0 | **1** | 0 |
| `parseEngineStateChildren("<a title='<Fake …>'>…<Real …>")` | `["Real"]` | **`["Fake","Real"]`** | `["Real"]` |
| `scanForOnTimeoutEntries('${ f("}") } <onTimeout …/>')` | 1 | **0** | 1 |
| `scanForOnTransitionEntries("<p title='… <onTransition …>'>")` | 0 | **1** | 0 |

Controls green on all three variants: real nested engine found, real
`<onTimeout>` found, real `<onTransition>` found, the apostrophe fix intact,
and the finding-3 display-text-literal case intact.

## Findings 1+2 — the guard discarded the span in the ONLY case that matters

Both `bodyRaw` re-scan loops guarded with `op.end !== scanned && op.end <= lt`,
throwing the span away exactly when it ran PAST the candidate — i.e. when the
candidate is text inside an attribute value or a `${…}` block, which is the
entire reason the shield exists. **The comment above it asserted
"`skipOpaqueSpan` never engulfs the candidate", and that assertion is why the
guard read as correct.** On rejection the loop fell back to the byte walk, which
no longer tracks `'`, so the mention fired as a genuine opener.

Engulf now means `hitSkippable`, identical to the `skipCommentOrString` branch
immediately below it — the reviewer's suggested shape, adopted after measuring
it rather than on assertion.

**End-to-end limb — reproduced, but only on the second construction.** My first
fixture (`<p title='ask <onTransition on=.A to=.B>x</> here'>`) was identical on
all three variants. The one that reproduces uses an INVALID target variant:

    <A rule=.B><p title='<onTransition from=.A to=.NOPE>x</>'>y</p></>

r3 emits `E-ENGINE-RULE-INVALID-VARIANT` for `.NOPE` — a variant that exists
nowhere but inside a quoted string. Base and r4: clean. The reviewer predicted
`E-ENGINE-INVALID-TRANSITION`; the actual code differs, the SHAPE is theirs.
⚑ Worth recording that the first fixture read GREEN — a near-miss construction
of a real defect is indistinguishable from absence of the defect.

## Finding 3 — TWO limbs, and fixing the named one did not close the probe

Limb A (named by the review): `parseEngineStateChildren`'s re-scan loop was the
one of three that took the `"markup-body"` narrowing and no shield.

Limb B (**found by re-measuring after fixing limb A — the probe still failed**):
on a non-uppercase opener the top-level splitter did `i = lt + 1`, landing the
cursor INSIDE the opener and scanning its attribute region as body text. That is
what actually produced the phantom state-child. It now steps OVER the opener via
`skipOpaqueSpan`/`findOpenerEnd` — what the walker scanners have always done, so
the splitter now agrees with them.

Phantom state-children are the worst of the five: the list feeds the
variant-coverage check, so a phantom can MASK a genuine
`E-ENGINE-STATE-CHILD-MISSING` and mis-key the state map.

## Finding 4 — the shield re-created the class it closes, via a SECOND hand-rolled scanner

`skipOpaqueSpan`'s `${…}` brace counter was not quote-aware. `${ f("}") }` ended
the span early and parked the walk on the now-unpaired `"`, which ran to EOF and
swallowed a following `<onTimeout>` — silent, exit 0, no diagnostic.

Fixed by **deleting** the hand-rolled counter and delegating to
block-splitter.js's `skipDollarBrace`, which already tracks `"` and `'` with
backslash escapes. Exported it (a one-word change) rather than writing a third
copy.

⚑ **COUNTED, NOT CLAIMED.** Five other hand-rolled `${…}` brace counters remain
in this file — `findOnTransitionCloser`, `findEngineCloser`, `findOpenerEnd`,
`findStateChildCloser`, `parseMessageArms` — all with the same quote-blindness.
Pre-existing; no finding implicates them; converting them changes behaviour at
five untested sites. **N goes 6 → 5. It is not 1, and the source comment says
so.** This is the structural answer to "a second hand-rolled scanner is how this
class keeps recurring": reduce N and state the residual, rather than add one.

## Finding 5 — my citation was wrong; the divergence is SURFACED, not decided

I wrote that attribute values are ``"``- or legacy ``'``-delimited "per §5.1".
`SPEC.md:1362` says the opposite, verbatim:

> "The attribute quoted-string form is `"`-only (double-quote; single-quote is
> not an attribute-string delimiter). … scrml uses one string delimiter, `"`,
> language-wide."

**What the implementation actually does, measured:**

| shape | result |
|---|---|
| `<a title='hi'>` at PAGE level | `E-ATTR-001` — enforced |
| `<a title='hi'>` INSIDE an engine state-child | **compiles CLEAN** |
| `${ ['hi'].join("") }` in a state-child | compiles CLEAN |
| `<a title="hi">` (canonical) either level | OK |

So the `"`-only rule IS enforced at page level and is NOT enforced inside an
engine state-child body, and `findOpenerEnd` here accepts `'` unconditionally.
**That leniency is the surface every round-4 phantom lives on** — each one needs
a `'`-delimited attribute or a `'` string to reach the byte walk at all.

⚑ The review's framing was that the class "exists ONLY for `'`-delimited
attribute values". Nearly right, and the correction matters: it also exists for
`'` and backtick strings inside `${…}` logic blocks, which ARE accepted
end-to-end (measured above). So tightening attributes alone would not close it.

**NOT DECIDED HERE.** Rejecting `'`-delimited attributes at this locus is
newly-rejecting — a one-way door and an operator call. The scanners are written
to be correct under BOTH answers: a `'`-delimited attribute region is treated as
opaque data, which is right if the form stays legal and harmless if it is later
rejected upstream.

## Findings 6 and 7 — bookkeeping, but the second is the important kind

6: `computeCommentRegions`' JSDoc had been orphaned by inserting
`skipOpaqueSpan`'s doc between it and its function. Moved back; both docs now
lead their own symbol.

7: source comments no longer say the two residual gaps are "filed". They state
the boundary and point at the test that measures it. **A source comment must not
assert the state of a ledger it does not touch** — and this branch touches no
ledger. The gap-filing itself is the landing side's, per the coordinator.

## Round-4 re-verification (all verified by execution)

- **Five probes** back to base behaviour; **five controls** intact.
- **Round-3 probes re-run** — unit, residual and e2e matrices all still match base.
- **Three-way table** — odd / even / control compile; contractions compile.
- **Equivalence pin** — bare apostrophised run == its apostrophe-free twin.
- **True negative** — `E-ENGINE-STATE-CHILD-MISSING` still fires on a genuinely
  absent `<B>`, and a phantom `<B>` in attribute text does NOT satisfy coverage.
- **Artifact inertness** — corpus differential base vs r4, same `--compiler-root`,
  1920 sources / 7427 artifacts: **NO DIFFERENCES**.
- **Full `bun run test` RE-RUN** (not carried forward — the reviewer flagged the
  earlier byte-identical claim as unverified by them): 54 failing, failure SET
  **identical** to the pre-existing baseline, 0 NEW / 0 GONE, `comm` both
  directions empty.
- **Unit tier** 18272 / 0. **Conformance + integration** 5062 / 0.
- **`types:check`** — output **byte-identical to base**; same 12 NEW, none
  naming either touched file.
- **R26 part 1** — 118 adopter sources, 220 artifacts, 0 syntax failures, 0
  symptom. **R26 part 2** — single-apostrophe injection into four real adopter
  files compiles clean and emits the prose.
- **FACTS** refreshed.

## Test file

**56 tests** (was 44). The 12 new ones sit under "the SHIELD must not become the
hole"; **6 of the 12 fail against the r3 parser** (verified by swap), and the
other 6 are controls proving the shield does not suppress REAL nested engines,
onTimeouts, onTransitions, or state-children following a lowercase opener.

## Standing risk, stated plainly

The shield is now the highest-risk surface in this file. It is guarded by 12
tests and its `${…}` walk is delegated rather than hand-rolled, but it is still
a flat scanner approximating a nesting-aware grammar. If a fifth round finds
another hole in it, the answer is probably not another patch — it is that the
opener/logic opacity belongs in ONE shared scanner used by all six body walkers
in this file, rather than bolted onto the three that currently lack it.
