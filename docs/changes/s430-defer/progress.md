...
## Round 6 (re-review of 5c4bb974) — landing round, narrow fail-closed set
- main: origin/main advanced to 7922091e (#1050, conformance xfail) — merged below.
- B: E-DEFER-DUPLICATE-FUNCTION (lint-defer) — two `function` decls of one name in a block that contains a `defer` (the lowered block is a host try block: strict-mode SyntaxError). Names both. §7.3.3 unchanged (dup functions outside a defer block stay legal; conformance duplicate-function-no-defer-ok pins it). §19.16.6 now lists the EXACT fail-closed exceptions to "a defer does not change what compiles"; §7.3.3 cross-references them.
- F-with-defer: a `defer` as the whole UNBRACED body of an if / else / for / while / do arm -> E-DEFER-UNSUPPORTED-SITE. Decided from the token stream at parse time in BOTH parsers (prev token `else`/`do`, or a `)` whose matching `(` follows if/for/while) -> `unbracedArm` flag. §19.16.2 amended. The live unbraced-else drop itself is NOT fixed here (coordinator filing it as a HIGH gap).
- A: native bridge marks statements flattened out of a bare block with `bareBlockScope`; lint-redeclare keys declarations by it (legal shadowing no longer a false E-SCOPE-REDECLARE) and later-shadow ignores flattened inner-block decls.
- D: the lambda check now requires the `defer` to be inside an Arrow/Function node of the parsed escape-hatch text (native parser) — the doubly-nested bare block no longer gets a spurious E-DEFER-OUTSIDE-FUNCTION@1.
- E: native parseMatchArm reports E-DEFER-UNSUPPORTED-SITE for a single-statement `defer` arm body and parses the deferred statement as the body (no parse-error cascade).
- G: defer diagnostics landing on an expression node (whose span is snippet-relative) are reported at the nearest enclosing statement's span.
- `defer [`: a `[` separated from `defer` by whitespace is a statement lead (both parsers); adjacent `defer[0]` stays an index. §19.16.1 amended; conformance array-literal-lead.
- C (by design): the unanalysable later-shadow message now says how to fix it (move the later declaration above the defer, rename, or move the arrow body into a named function).
- later-shadow-neg case reordered (`<trace>` before `const x`) — both pipelines now parse it identically.
