...
> 4. **Code spellings (§65.10).** CONFIRMED: `E-DEFAULTS-MISUSE` / `W-STYLE-DEFAULTS-DEAD` (match the ruled `<defaults>`) + `E-STYLE-IMPORTANT-INTERNAL` + new `W-STYLE-IMPORTANT-INTEROP` (per resolution 3).
> 5. **Placement scope (§65.9).** `<theme>`/`<defaults>` are PROGRAM-scope (children of `<program>`, sibling of `<channel>` per Insight 30) for v1. Page-level override DEFERRED (a page references program tokens; per-page override is v1.next).
> The Tier-3 OQs (OQ-3 token-unification · OQ-5 keyframes-namespacing · OQ-6 content-treatment · OQ-7 @apply-long-term · OQ-8 global-`#{}`) remain genuinely open (§65.12) — bryan-deferred, distinct from these 5 finalizations.
