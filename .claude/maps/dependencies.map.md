...
# External deps RE-VERIFIED unchanged (`git diff 15e5e070..HEAD -- package.json` is EMPTY — no add,
...
# answering "is this name async in client mode" (`async-combinators.ts`, #442). Decision sites 3 -> 1.**
# See the new "§13.2 ASYNC-NAME PROVIDER" row in the pipeline table — read it BEFORE adding any
# fourth consumer, and before hand-writing an async-name disjunct anywhere.
# Also this window: the U1 client server-fn call-site await (#429) and its five-site `clientAsyncBody`
# threading chain; `emit-server.ts`'s `session` Proxy prologue binding (#435, GH #357) and the
# `@currentUser`/`<channel auth=>` dangling-ref pair (#440). Everything else carries its prior walk.
