...
# NOTE (S320 INCREMENTAL pass): over `e80b692e` -> `b929b9c9` (31 commits, six sessions). External deps
# RE-VERIFIED unchanged (`git diff e80b692e..HEAD -- package.json` is EMPTY — zero new runtime/dev
# dependency). Added: `scheduling.ts`'s NEW `parenthesizeAwaitServerCallsInExpr` (match-arm auto-await,
# #394); the `distRelativeServerSpecifier` -> `distRelativeLocalSpecifier` generalization (#390, §64
# tool/library imports now share the D-4 dist-space re-basing); `context.ts`'s NEW
# `crossFileClientReads` field (#358 series, the cross-file client-reachability seed for directly-
# imported `export const`); the `export let`/`var` + serve-tool import-reachability fix (#388).
# Everything else carries its prior walk.
...
| **§13.2/§19.9.3 match-arm value-position auto-await (NEW #394)** | **codegen/scheduling.ts** — `parenthesizeAwaitServerCallsInExpr(exprCode, serverFnNames)` (:587-685, NEW). Operates on an already-EMITTED (pre-name-mangle) EXPRESSION STRING, not a statement — a different input shape from `injectServerCallAwaitsViaAst` above, which is why it is a separate function rather than an extension of it. | `emit-logic.ts`'s value-form `match`-decl arm lowering (`emitMatchExprDecl`) — a path the #87 statement-level `injectPromiseAwait` never reached. **Wraps the CALL NODE in grouping parens** (`fn().ok` → `(await fn()).ok`), never a bare prefix: a bare `await fn().ok` parses as `await (fn().ok)`, reading `.ok` off the PENDING Promise. Await-legality is real-parser-modelled (a sync callback body / any formal-parameter list is await-illegal), mirroring the statement-level injector's discipline. Returns input UNCHANGED on a non-JS fragment or when nothing classifies. |
...
Neither touches `scheduling.ts`'s `isControlFlowBoundary` treatment of `given`/`if`/`match` bodies as
opaque to the CPS scheduler — that is the separate, larger, HELD arc (PR #405, unmerged; see
primary.map.md).
