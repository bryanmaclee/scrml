...
# External deps RE-VERIFIED unchanged (`git diff b929b9c9..HEAD -- package.json` is EMPTY). **CORRECTION:
# PR #405 (the CPS auto-await choke-point consolidation) LANDED at `649d6fce`, reviewed clean at
# `bbd77bec` — it is NO LONGER "HELD, unmerged".** `scheduling.ts` was substantially rewritten as part
# of that landing: the per-statement string-regex `injectPromiseAwait` is RETIRED; ONE shared AST walker
# (`collectAwaitSites`/`applyAwaitSites`) now backs all three auto-await entry points, and coverage
# widened from if/for/while-only to ALSO descend into `given`/match-block/`try` bodies. Also new this
# window: `reactive-deps.ts`'s `collectStructuralDeclNames` (§6.8, #417) and its threading through
# `emit-reactive-wiring.ts`/`codegen/index.ts`; `emit-each.ts`'s `W-IF-IN-EACH` detection helpers
# (§17.1, #416). Everything else carries its prior walk.
