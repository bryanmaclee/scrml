...
No dependency-manifest change at this HEAD — root `package.json` and `compiler/package.json` are untouched by every landing in this window. **The whole ESM-chunks arc is first-party and adds ZERO external dependency**: `codegen/runtime-esm.ts` and `codegen/emit-client-esm.ts` reuse the already-present `acorn` (imported untyped via `@ts-ignore`, the same convention `validate-emit.ts` and `expression-parser.ts` use). The #131 each-fence model, the S280 gate scripts (`scripts/snippet-gate.js`, `scripts/facts.ts`, `scripts/claim-gate.js` — all plain `bun`-run, stdlib-only) and the dep-script depth fix likewise add none. One module was DELETED: `lint-w-each-table-foster.js`.
...
| Name/symbol resolve | name-resolver.ts, symbol-table.ts | codegen. **symbol-table.ts PASS 15.5 owns the `<outlet>` placement pass**: `walkValidateOutlets` (:10096) -> `collectOutlets` (:10194) group outlets per enclosing `<program>` and emit E-OUTLET-OUTSIDE-SHELL / E-OUTLET-DUPLICATE / **E-OUTLET-AND-MAIN** (:10372 `fireOutletAndMain`, #124). Wave-1c added the per-shell `shellMains` map + `wrappingMains` WeakSet + `inRouteScope` flag that make the one-landmark invariant decidable at SYM. |
...
| **ESM chunks — the second client module format (`--module-format=esm`, U1-U3)** | codegen/runtime-esm.ts (`toEsmRuntime` :304, `deriveTopLevelExportNames` :256, `LIFT_TARGET_GLOBAL` :71) + codegen/emit-client-esm.ts (`toEsmClientChunk` :270) | Consumed ONLY from codegen/index.ts, all four call sites `moduleFormat === "esm"`-gated: :2496 (transform the assembled+sliced runtime, AFTER slicing so the derived export set names only surviving symbols, BEFORE the content hash so esm gets its own cache key), :2602 (per-route chunk transform with a `../`-depth-derived runtime URL), :852 + emit-html.ts:4052 (`type="module"` on the emitted/injected `<script>` tags), :1657/:1685 (the esm-vs-classic cross-file tag split — classic still emits the `_scrml_modules` dep-script tags, esm does not). api.js's `rewriteChunkImportRefs` (~:2947) content-hashes the in-chunk `import` specifiers on the BUILD path only. `classic` (the default) is a no-op through every one of these — the classic byte output is unchanged by the arc. `--module-format=esm` + `--embed-runtime` = esm has no effect (the embedded runtime stays a classic script), surfaced by commands/module-format-notice.js. |
| **`<each>` mount FENCE + the range-aware reconciler (#131)** | codegen/emit-each.ts (`emitEachMountHtml` :371 -> `<!--scrml-each:N--><!--/scrml-each:N-->`; reconcile emit calls `_scrml_each_clear`/`_scrml_each_append` and locates via `_scrml_find_each_anchor`) | runtime-template.js (`_scrml_reconcile_list` :1652 branches on `container.nodeType === 8`; `_scrml_each_end` :1963, `_scrml_find_each_anchor` :1989, `_scrml_each_clear` :2006, `_scrml_each_append` :2019, `_scrml_remount_each` :2131) + codegen/emit-ssr-render.ts (SSR fills BETWEEN the fences) + codegen/emit-match.ts / emit-variant-guard.ts (arm remount). The mount is a COMMENT, so it is invisible to `querySelector` — every locator is a SHOW_COMMENT TreeWalker, mirroring the older `_scrml_find_if_marker` (:1381). The emit is uniform across the top-level (comment fence) and nested (runtime `<div>`, emit-each.ts:1041) paths because the runtime ops are polymorphic over anchor-vs-element. |
| Cross-file dep `<script src>` depth | codegen/index.ts `computeDependencyClientScripts` + codegen/utils.ts `stripPagesPrefix` | BOTH the dep path and the entry dist dir now go through `toDistRel()` so the `pages/` strip is modelled symmetrically; the asymmetry previously inflated every cross-file ref on a nested route by one `../`, escaping the dist root. CLASSIC path (the default), not esm-only — emit-client-esm.ts already applied the strip, so the fix CONVERGED the two. Three pre-existing gaps remain open on this surface (see non-compliance.report.md). |
| **Client Router — landmark + shell composition (NEW #124, S276, §20.8.1.1/§40.8.2)** | codegen/emit-html.ts (`treeHasAuthorMain` :1005, the `tag === "outlet"` emit branch ~:1650-1730) + codegen/index.ts (`findOutletMarkedOpenTag`/`findBareMainOpenTag` slot detection, `retagOpenTag` :699, `findMatchingCloseIdx` :724, `htmlHasMainElement` :689, the per-page `routeOwnsLandmark` :2171 / `slotShouldPromote` :2196 decisions) | TWO STAGES, one invariant. emit-html decides the landmark PER FILE at emit time (`documentHasAuthorMain() ? "div" : "main"`, :1720) and stamps the synthetic `data-scrml-outlet` marker + `tabindex="-1"`; codegen/index.ts decides the composition SLOT ACROSS FILES at composition time (marker-first, bare-`<main>` fallback for the pre-§20.8 static MPA path) and re-tags the slot per composed document. The two communicate ONLY through the emitted marker attribute — there is no shared AST state, because **`<outlet>` is not a dedicated AST node** (a `kind: "markup"` node with `tag: "outlet"`). The runtime is the third party to the same contract (`runtime-template.js` `querySelector("[data-scrml-outlet]")`). See domain.map.md for the full four-case table. |
...
| codegen/index.ts (= code-generator.js) | the codegen dispatcher AND, since #124, the owner of §40.8.2 multi-file shell composition: slot location by attribute NAME (a real open-tag/attribute tokenizer, so `data-scrml-outlet-debug` and `data-testid="data-scrml-outlet"` correctly do NOT match), `findMatchingCloseIdx` depth-counted close-tag scan (skips comments + `<script>`/`<style>` raw text; a naive `indexOf` spliced route bodies above the real close and silently dropped content once the slot could be a nestable `<div>`), `>=` slot-bounds test so an EMPTY slot still composes, and per-composed-document landmark demotion/re-promotion. |
| codegen/emit-html.ts | markup emission; owns `analyzeWriterConflict` (#81, E-ATTR-WRITER-CONFLICT) and the §20.8.1.1 landmark decision (`treeHasAuthorMain` — a deliberately BROAD walk over every array/object-valued property, WeakSet cycle-guarded, `span` skipped; breadth is intentional because a false negative emits two `<main>` elements while a false positive is invisible). |
...
## Tags
#scrml #map #dependencies #module-graph #stdlib #css-conflict-check #pipeline #bun #acorn #sql-lex #tenant-egress #tenant-floor #ssr-auth-scoped #theme-reset #content-hash #css-wave1 #colorless-async #async-combinators #writer-ownership #session-establishment #outlet #one-landmark #shell-composition #e-outlet-and-main #esm-chunks #module-format #runtime-esm #emit-client-esm #each-fence #foster-safe #dep-script-depth
