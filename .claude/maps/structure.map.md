...
compiler/tests/  — **1100 .test.js files total at `cf1471dd`** (+6 S224-S226; was **1071 .test.js files total at `852d6aa7`** (verified `find compiler/tests -name '*.test.js' | wc -l` = 1055; +2 S219: engine-arrow-body-legacy-reject-b2.test.js unit 150L + lint-w-input-state-markup-nonreactive.test.js unit 266L; +14 S217/S218 [4 browser + 7 unit + 1 integration + 1 conformance + 1 unit-extended]; +3 S214: render-shadowing-ss16-c3.test.js integration + eq-payload-variant-lint-ss16-c4.test.js unit + g-on-mount-bare-call-render-slot.test.js unit; also EXTENDED: bug-1-tailwind-unrecognized-class.test.js + emit-html-meta.test.js + bare-variant-nested-context-inference.test.js NEW + parser-conformance-within-node-allowlist.json); was **1036 at `fec0a054`** (verified `find compiler/tests -name '*.test.js' | wc -l` = 1024; +4 S210) (verified `find` count) across all categories (S173: +2; S174: log()/any-reject; S175: +5 typed-SQL-row; S176: +6 unknown-type-name/scalar-vocabulary; S177: +8; S179: +2; S180: +5; **S181: +2 [display-text-overquote.test.js, server-keyword-error-msg-canon.test.js]**)
...
### S232 — §14.8.9 protect-floor: server→client protected-column egress redaction (`2d5e96c8` floor + `4d6b5756` channel/SSE; g-sql-row-protect-leak / g-protect-channel-sse-egress RESOLVED)
The "tag at query-lowering, read at the egress sink" mechanism. A `protect=` column's `(table, column)` ORIGIN is the key (alias-safe — `passwordHash AS h` redacts identically). A `Symbol.for("scrml.protect.origin")`-keyed descriptor rides the row through every compiler-emitted construction step (enumerable so `{...row}`/`.map` carry it; Symbol-keyed so `JSON.stringify` ignores it). At the egress sink `_scrml_protect_redact(value)` drops protected-origin columns not `reveal`-stamped. Sound BY CONSTRUCTION (reads a tag at egress; no value-flow proof). NEW codes E-PROTECT-004 (Error) + I-PROTECT-STRIP-001 (Info) now FIRE (were S230 catalog-named-not-firing).
- **NEW `codegen/protect-egress.ts` (268L)** — the module above (`ProtectContext`/`buildProtectContext`/`resolveProtectedOutputColumns`/`SERVER_PROTECT_HELPER`/`wrapWithProtectTag`/`detectProtectedRawEgress`). Soundness bound (do NOT over-claim): complete for explicit-column flows of statically-resolvable SQL, BY ORIGIN. NOT covered — derived/implicit flows (`{ hasPw: row.pw != "" }`), member-extraction into a fresh literal (`{ secret: row.pw }`), covert channels. Unresolvable dynamic SQL strips WHOLESALE (fail-closed).
- `codegen/emit-server.ts` (+182L) — `_protectActive` master gate (`_protectCtx.protectedByTable.size > 0`; non-protect app emits byte-identically). `buildProtectContext(_protectAnalysis)` :935; E-PROTECT-004 fail-closed scan per server-fn SOURCE :938-965 (`detectProtectedRawEgress`); `setProtectContextForRewriter(_protectActive ? _protectCtx : null)` :991. `_scrml_protect_redact(...)` wired into EVERY client-egress sink: server-fn return (:2205/:2405/:2420/:2530 CSRF + non-CSRF + Response paths), SSR `/__serverLoad` (:2860/:2907 Tier-1 SELECT* + Pattern-C), channel `broadcast()` (:1135 `_broadcastData`), `server function*` SSE `data:` frames (:1774/:1778 `_sseEventData`/`_sseBareData`). `SERVER_PROTECT_HELPER` injected-on-use :3183 (mirrors `_scrml_wire_encode` inline-on-use; server-only). I-PROTECT-STRIP-001 deduped-per-query Info :3324-3340.
- `codegen/rewrite.ts` (110270→113550 bytes) — `setProtectContextForRewriter` :119 (module protect ctx) + `protectTagSqlResult` / `_scrml_protect_tag` wrap at SQL lowering (:106/:141/:151, reuses `resolveProtectedOutputColumns` + `wrapWithProtectTag`).
- `codegen/emit-logic.ts` (+19L) — imports `protectTagSqlResult` from rewrite.js; wraps the lowered `?{ SELECT }` result (`(await ...)[0] ?? null` / `await ...` / `.unsafe(...)`) with the protect-tag at :3009-3027.
- `codegen/emit-expr.ts` (+21L) — `<value>.reveal("col")` lowering :1915-1933 → `_scrml_protect_reveal(obj, col)` (the SOLE admit path past the egress sink; server-only; stamps `revealed` on the descriptor copy).
- `codegen/index.ts` (+21L) — `CgProtectAnalysis` interface :119 + `protectAnalysis?` on CgInput :127; threaded into `generateServerJs(..., protectAnalysis)` :873 + `analyzeAll({... protectAnalysis})` :564; `protectedFields` surfaced :892/:971.
- SPEC: §14.8.9 flipped Nominal→Implemented (§8110-8192); E-PROTECT-004 (§34 :17568) + I-PROTECT-STRIP-001 (:17569) extended to channel/SSE. `value.reveal("col")` is the explicit declassify verb.

### S232 — foreign forms fail-closed-honest: E-WASM-NOMINAL + E-FOREIGN-SIDECAR-NOMINAL (`cd887f41`; g-nominal-foreign-forms-not-failclosed)
The v1.0 fail-closed-Nominal invariant: a Nominal/spec-ahead form must fail closed with an honest "not implemented" diagnostic, NEVER a silent miscompile or a misleading code.
- `type-system.ts` (+87L) — NEW `tryFireWasmCallCharNominal` (§23.3 WASM call-char recognizer, ~:19041-19072): BOTH `extern <call-char> name(...)` foreign-fn decls (§23.3.3) AND `<call-char>{ … }` call-char sigils (default registry r/c/C/z/o/a, §23.3.1) are recognized and fail closed with **E-WASM-NOMINAL** (Error). Pre-fix both fell through as bare idents and fired a MISLEADING E-SCOPE-001 ("Undeclared identifier `r` … did you mean `@r`?"). Fired at let/const/state-decl-init + bare-expr ident-check sites (:8925/:9259/:9587 skip the RHS validation once E-WASM-NOMINAL fires). The `<program callchar=X>` ATTRIBUTE stays legal — only the USE forms fail closed.
- `ast-builder.js` (part of +95L) — §23.4 `use foreign:name { fn-list }` sidecar: NEW lift RE + `parseLogicBody` `use` handler `foreign:` branch fires **E-FOREIGN-SIDECAR-NOMINAL** (Error, :10207-10227). Pre-fix it SILENTLY MISCOMPILED (bare `use foreign:` line + trailing `server function` leaked as literal HTML; nested sidecar `<program>` → a `new Worker(...)` referencing a never-emitted bundle; ZERO diagnostic, exit 0). The recognizer lifts the bare text into the logic stream (no HTML leak), fires the code, stamps `_foreignSidecarNominal`, skips the nested sidecar `<program port=…>` from worker extraction. Plain `use scrml:ui`/`vendor:`, CSS `#{}`, `import:host` untouched.
- `gauntlet-phase1-checks.js` (+10L) — E-USE-001 (`use` inside a `${ }` logic block) EXEMPTS the lifted `use foreign:` sidecar (`!node._foreignSidecarNominal` guard :323) so the honest E-FOREIGN-SIDECAR-NOMINAL is not buried under a misleading "move it out of the logic block".
- SPEC §34: E-WASM-NOMINAL (:17507) + E-FOREIGN-SIDECAR-NOMINAL (:17511), both Error. When the real layers land, the spec'd E-WASM-001/002/003 + E-FOREIGN-010/011/012 supersede these placeholders.

### S232 — NEW §23.5 Capability Declaration (Nominal/spec-ahead; `e321da1e`, capability-vocab V1 SPEC)
SPEC-only this revision (Nominal — NO impl, NO fire sites yet). `capabilities=` is a `<program>` attribute (§4.12.2) declaring the intended host-capability surface for foreign code (`_{}` / WASM sigil / `use foreign:` sidecar) — a bracketed list of tokens `{ network, fs-read, fs-write, spawn, env, db }` (§23.5.3). Closest-ancestor-`<program>`-wins (§23.5.4); omitted = `capabilities=[]` (declare-nothing). Advisory in v1.0 (**W-FOREIGN-UNDECLARED-CAPABILITY**, NAMED-in-§23.5 but NOT firing); an unrecognized capability token is **E-FOREIGN-CAPABILITY-UNKNOWN** (§23.5.7, NAMED-not-firing). SPEC §16291-16371 + attr-table row :728. Build deferred (Rule 4: codes land WITH enforcement). The intent is to fix the declaration HABIT (§23.5.5) so enforcement is a later additive non-breaking flip (§23.5.6).

### S232 — E-FN-ARROW-BODY: reject `fn(args) => expr` (`1512d4bc`; g-fn-shortform-arrow, ruling A)
`fn(args) => expr` (e.g. `arr.map(fn(n) => n * 2)`) mixes the two canonical anonymous-callable forms — block-body `fn(args) { return expr }` (§48.2.1) and the plain inline lambda `args => expr` — and is NOT a sanctioned form. Pre-fix it fell through the codegen `fn`→`function` string-rewrite and emitted invalid JS `function(args) => …`, mis-framed as E-CODEGEN-INVALID-JS.
- `expression-parser.ts` (+96L) — NEW `detectFnKeywordArrowBody(processed, pos)` :440 runs on the acorn-rejection path; `fnArrowDiagnostic?: {code,message,offset}` field threaded through the tolerant-parse return (:79/:530/:2656-2710). Surfaces a clean **E-FN-ARROW-BODY** (Error, §48.2.1) steering to the two valid forms.
- `ast-builder.js` (part of +95L) — the escape-hatch ParseError node carrying `fnArrowDiagnostic` fires E-FN-ARROW-BODY :320-323.
- SPEC §34: E-FN-ARROW-BODY (:17669), Error.

### S232 — inline value-form control-flow renders reactively in markup interp (`3d319f1d`; g-inline-value-match)
A value-form `${ match … }` / `${ if … else … }` as the SOLE content of a markup `${...}` interpolation (§18.0 / §17.6) now renders reactively (was raw / non-rendering). The `if` value-form requires an `else` (must yield a value on every path).
- `codegen/emit-control-flow.ts` (+63L) — NEW `emitIfValueExpr(node, opts)` :2124 + `_emitIfValueExprInner` :2137 (else-if → nested ternary; total for the value-form shapes). Companion to the existing `emitMatchExpr` IIFE.
- `codegen/emit-html.ts` (+98L) — NEW value-form-control-flow classifier (~:174-209, `isValueFormControlFlow`: a `${}` whose sole child is a value-form `match`/`if`-with-`else`) + the markup-interp render seam :2479-2497 (emits the IIFE / ternary into the slot).
- `codegen/emit-event-wiring.ts` (+71L) — imports `emitIfValueExpr` from emit-control-flow.ts; the `${}` text-interpolation reactive path routes a value-control-flow node through `emitMatchExpr` / `emitIfValueExpr` (:928-957).
- `codegen/binding-registry.ts` (+19L) — NEW `value-control-flow` binding kind (:127-135) so the reactive wiring re-renders the selected value on dep change.
- `codegen/collect.ts` (+9L) — propagates the value-form control-flow node (:232) so the binding registrar sees it.
- `codegen/emit-reactive-wiring.ts` (+12L) — SKIPS the control-flow body of a value-form `${ match / if }` interp (:483) so emit-html owns its render (no double-wire).

### S232 — §19.9.1 server-error-wire SPEC currency fix (`04e7a1bb`; g-server-error-wire-spec-impl-divergence, RESOLVED)
DOC-only. §19.9.1 corrected from the stale `{ __variant, __data }` (a shape the implementation NEVER emitted or read) to the canonical `{ __scrml_error: true, type, variant, data }` envelope that `emit-logic.ts` actually emits and every reader (`<errorBoundary>` gate, CPS continuation, client arm-dispatch) keys on via `.__scrml_error`. ZERO code change. (The §19.12 test-mode `{ __ok:false, __variant }` is a SEPARATE test-harness representation, not this wire.)

## Key S229 Source Changes (forward-fold onto dec70dce; S228 window NOT reconciled — see header)

### S229 — g-reactive-map-set-control-flow: reactive map/set ops inside control-flow (`ffc6fbec`, MED)
- compiler/src/codegen/emit-control-flow.ts — NEW `mapVarNames?`/`setVarNames?`/`orderedMapVarNames?` on `IfOpts` + on the `emitForStmt` opts param (the REACTIVE twin of ss52's `localMapVarNames`/`localSetVarNames`/`localOrderedMapVarNames`). Threaded into: `_emitIfStmtInner` `_ifExprCtx` (if condition) + the if/else body opts; `_emitForStmtInner` C-style `_cCtx` + the C-style body `emitLogicBody` opts + the reactive `_forExprCtx` + the plain `_plainForCtx`; `emitHoistedForStmt(node, _hoist, dbVar, opts)` now RECEIVES `opts` (was dropped → hoisted-for bodies missed the reactive sets). The ss52 comments asserting 'the reactive `@`-path is a separate pre-existing control-flow gap, left untouched' are REMOVED — the gap is closed.
- compiler/src/codegen/emit-logic.ts — `emitLogicNode` `case 'while-stmt'`/`'do-while-stmt'` + the nested if-expr-alt-chain spread now forward `setVarNames`/`orderedMapVarNames` alongside the pre-existing `mapVarNames` into `emitWhileStmt`/`emitDoWhileStmt`/the nested ctx (previously only `mapVarNames` + the `local*` trio crossed).
- EFFECT: a reactive `@m.insert(k,v)` / `@m.size` / `@s.has(k)` / `@m[k]` / set `.union`/`.intersect`/`.difference` inside ANY control-flow construct lowers to `_scrml_map_*(_scrml_reactive_get("m"), …)` — the same form top-level reactive already produced. Was RAW `_scrml_reactive_get("m").insert(...)` → the HAMT runtime object has no `.insert` method / `.size` property → silent runtime TypeError (compiled clean + passed `node --check`).

### S229 — g-markup-session-read-undeclared: `@session` reserved-ambient projection (`0df45d2e`, LOW; S228 ruling)
- compiler/src/type-system.ts — NEW module-const `RESERVED_AMBIENT_PROJECTION_NAMES: ReadonlySet<string> = new Set(["session"])` (~:6401, adjacent to `LOGIC_SCOPE_GLOBAL_ALLOWLIST`). `checkLogicExprIdents` `@`-branch (~:6504): `if (RESERVED_AMBIENT_PROJECTION_NAMES.has(atBase)) return;` exempts `@session` from the read-side E-STATE-UNDECLARED fire (placed AFTER the `_`-prefix `@_internal` guard, BEFORE the `typeRegistry`/`knownFnNames` `@`-branch guards). Scoped to the exact reserved name — a genuine `@typoCell` still fires.
- compiler/src/codegen/emit-expr.ts — NEW per-file module flag `_sessionProjectionActive` + exported `setSessionProjectionActive(on)` (~:96, mirrors `_renderShadowedInFile`/`setRenderShadowedInFile`). `emitIdent` `@`-base branch: `if (bare === "session" && _sessionProjectionActive) return "${_m}session";` (client mode only — server `@session` keeps the `_scrml_body["session"]` form above it). Lowers `@session` to the bare projection var `session` (emit-client.ts `var session = window._scrml_session_projection ?? …`) instead of `_scrml_reactive_get("session")` (which reads `_scrml_state["session"]` → undefined → `.current` crash). Markup + logic emit identically (both route through `emitIdent`).
- compiler/src/codegen/index.ts — imports `setSessionProjectionActive` (emit-expr.ts) + `collectReactiveVarNames` (reactive-deps.ts). `setSessionProjectionActive(false)` defaulted at BOTH runCG per-file sites (worker bundles carry no auth session projection); re-set after auth-MW resolution: `setSessionProjectionActive(authMW !== null && !collectReactiveVarNames(fileAST).has("session"))` — active iff auth middleware is configured for the file AND no user reactive cell named `session` shadows the projection.
- compiler/src/route-inference.ts — the §20.5 SERVER-only `session` pattern in `SERVER_ONLY_PATTERNS` changed `/\bsession\b/` → `/(?<!@)\bsession\b/`: the bare `session` object still auto-escalates its enclosing fn to the server; the CLIENT `@session` window-scoped projection no longer does (which would mis-read `_scrml_body["session"]`). Mirrors the `(?<!public )\benv\s*\(/` lookbehind in the same table.
...
- compiler/src/codegen/emit-control-flow.ts (2013L) — **Bug 64 fix (Tier-0 control-flow path)**: `pushLiftReconcileCtx` called inside the `for`-loop `createFn` builder with `{ wrapperVar, keyVar: keyVar, iterVar: varName }` (key captured as `item?.id != null ? item.id : _scrml_idx`, mirroring the `_scrml_reconcile_list` keyFn); `popLiftReconcileCtx` called after the createFn body. Engine ctx threaded into all `emitConsolidatedLift` / `emitLiftExpr` / `emitIfStmtWithContainer` / `emitForStmtWithContainer` calls inside the body. **S229 (`ffc6fbec`): NEW reactive `mapVarNames`/`setVarNames`/`orderedMapVarNames` on `IfOpts` + `emitForStmt` opts, threaded through the if cond/body, for C-style/reactive/plain ctxs, `emitHoistedForStmt`, and (via emit-logic.ts) `emitWhileStmt`/`emitDoWhileStmt` — reactive value-native map/set ops inside control-flow now lower to `_scrml_map_*` (was raw HAMT method → runtime TypeError); the reactive twin of ss52. See 'Key S229 Source Changes'.**
...
## Key S216 Source Changes (`7c01b22a`)

### Bug-51 — enum-defs-into-server-bundle + E-CG-016 collision guard

**compiler/src/codegen/emit-server.ts** (multiple changes):
- `topLevelBindingExists(body, name)` (module-scope helper, lines 118-150) — line-anchored scan
  for top-level bindings (`const`/`let`/`var`/`function`/`class` decls + named `import` specifiers)
  used to detect enum-name collisions BEFORE injection.
- `localServerImportNameUsed(body, name)` (lines 92-99) — word-boundary identifier-reference check;
  gates whether a server-imported local name is actually referenced in the emitted body (tree-shaking).
- `injectAfterHeader(emitted, helper)` (lines 79-83) — shared helper that inserts a helper block after
  the file header's first `

` boundary (hoists helpers above route handlers); used by structural-eq /
  wire / log / enum injection paths (replaces previously inline `headerEndIdx` logic).
- NEW `generateValueOnlyServerJs(fileAST)` export (lines 696-730) — generates a const-only server emit
  for a `g-const-only-module-no-server-emit` file (no route handlers; exports only value bindings
  referenced by sibling server bundles). Runs `emitModuleValueExportLines` + the same helper-inlining
  scans (structural-eq / wire / log) as the route-handler path so drift is impossible.
- Bug-51 enum injection (lines 2500-2589): post-prune enum scan using `emitEnumVariantObjects(fileAST)`,
  filtered by `localServerImportNameUsed` (usage gate), collision-checked by `topLevelBindingExists`
  (E-CG-016 fire on clash), injected via `injectAfterHeader` AFTER the import-prune pass to keep the
  prune's word-boundary scan honest. Route: `emit-client.js buildVariantFieldsRegistry` +
  `emitEnumVariantObjects` (existing — now also called from server path).

### ss1 — server-emit / route-inference fixes

**compiler/src/ast-builder.js** — `tryConsumeSqlInit` / `consumeSqlChainedCalls` pattern now attaches
structured `sqlNode: { kind:"sql", query, chainedCalls }` consistently on ALL `<var server> = ?{}` decl
forms (state-decl logic, markup, let-decl at lines ~6790/6902/6953/10074/10103/10198). Prior gap: some
decl paths left `sqlNode` unattached (`init: ""` + no `sqlNode`), so the route-inference walk missed
the `?{}` SQL trigger for those forms. `preprocessWorkerAndStateRefs` gains `childRanges` param (ss15
request-bridge lift path auxiliary — no route-inference impact).

**compiler/src/route-inference.ts** — `sqlNode` SQL-trigger recognition added at additional walk sites:
- `walkBodyForTriggers` let-decl / state-decl path (lines ~882/1028/1117) — a `let <var> = ?{}` or
  `<var> = ?{}` statement whose `sqlNode.kind === "sql"` now fires the SQL-server trigger.
- Return-stmt walk (lines ~1186/1195/1200) — `return ?{}` shapes carrying `sqlNode` recognized.
- These close the gap where a `<var server> = ?{}` state-decl OUTSIDE a function body was not triggering
  the server escalation.

**compiler/src/codegen/emit-server.ts** — §52.6.5 Pattern C loop (lines 2086-2123):
- `_patternCLoadDecls`: `collectServerVarDecls(fileAST).filter(d => serverVarDeclLoadKind(d) === "sql-load")`
  (param-free inline `?{}` decls only).
- For each decl: emits `async function _scrml_serverLoad_<varName>_handler(_scrml_req)` that runs
  `emitLogicNode(sqlNode, {boundary:"server"})` (same path as `?{}` inside a server fn body — `.get()`/
  `.all()` terminators and bound params handled identically) + wraps in `Response(JSON.stringify(...))`.
- Exports `export const _scrml_route___serverLoad_<varName> = { path: "/__serverLoad/<varName>", method: "POST", handler }`.
- Mirrors the Tier-1 `_scrml_route___serverLoad_<var>` SELECT* route (lines 2055-2083) but runs the
  cell's ACTUAL `?{}` query (not a fixed SELECT *).

### Family-A — bind-directive convergence

**compiler/src/codegen/emit-bindings.ts**:
- `BindDirectiveBodyOpts` interface (exported, ~line 429) — options bag: `getElement`, `pushDisposer`,
  `contextId`, `fileAST`, `errors`, `compileCtx`.
- `emitBindDirectiveBody(bAttr, mkNode, opts: BindDirectiveBodyOpts): string[]` exported (~line 482) —
  root-agnostic bind-directive body lowering (element-acquisition via `opts.getElement()`, disposer via
  `opts.pushDisposer()`). Both the file-scope `emitBindings` path and the arm-body path in
  emit-variant-guard.ts now call this shared helper.

**compiler/src/codegen/emit-variant-guard.ts**:
- `emitArmWireFunction` (~line 601-632): dynamic import of `emitBindDirectiveBody`, `buildEnumVarMap`,
  `buildReactiveTypeMap` from `emit-bindings.ts` (lines ~612-614). For each arm-tagged `bind:*` directive
  collected in `wireableDirectives`, calls `emitBindDirectiveBody(binding.bindAttr, binding.bindNode, opts)`
  with `_root`-rooted element acquisition + disposer push onto `_disposers`. Replaces a previously
  duplicated inline bind-directive emission path.

### §52-Pattern-C — W-AUTH-004 + client-side IIFE

**compiler/src/codegen/collect.ts**:
- `ServerVarLoadKind = "sql-load" | "param-bearing" | "none"` type exported (line 656).
- `serverVarDeclLoadKind(decl): ServerVarLoadKind` exported (line 658) — inspects `decl.sqlNode`;
  returns `"param-bearing"` when `sqlNode.query` contains `${` (client-local cell interpolation);
  `"sql-load"` for a param-free inline `?{}`; `"none"` otherwise.

**compiler/src/codegen/emit-sync.ts**:
- `emitDeclRhsSqlLoad(varName: string): string[]` exported (line 152) — the client-side async IIFE
  that POSTs to `/__serverLoad/<varName>` on mount and reactive-sets the result:
  `(async () => { const r = await fetch("/__serverLoad/<var>", {method:"POST",...}); _scrml_reactive_set("<var>", await r.json()); })()`.

**compiler/src/codegen/emit-reactive-wiring.ts** (lines 674-687):
- `serverVarDeclLoadKind(decl)` called per Tier-2 `<var server>` decl.
- `"sql-load"` → calls `emitDeclRhsSqlLoad(varName)` (Pattern C mount fetch).
- `"param-bearing"` → skips load; W-AUTH-004 was already emitted by the type system.
- `"none"` → falls through to existing `emitInitialLoad` / `emitUnifiedMountHydrate` path.

**compiler/src/type-system.ts** (lines 9320-9354):
- `_isPatternCParamFree` / `_isPatternCParamBearing` detection on `decl.sqlNode.query`.
- `_isPatternCParamBearing=true` → fires `W-AUTH-004`; W-AUTH-001 NOT fired (steer is more precise).
- `_isPatternCParamFree=true` → W-AUTH-001 suppressed (Pattern C param-free path loads on mount).

## Key S214 Source Changes (`1ff06eae`)

### ss16 C3 — render() builtin shadowing fix + W-RENDER-SHADOWED lint

**codegen/log-loc.ts** — `fileDeclaresLog` refactored into generalised `fileDeclaresFn(fileAST, name)`;
`fileDeclaresLog` delegates; NEW `fileDeclaresRender` export delegates to `fileDeclaresFn(ast, "render")`.
**codegen/emit-expr.ts** — NEW `_renderShadowedInFile` module-level flag + `setRenderShadowedInFile(on)` export;
`emitCall` `render()` hijack gated: if `_renderShadowedInFile || ctx.declaredNames?.has("render")` → fall through
to generic call path so `fnNameMap` post-pass rewrites `render` → `_scrml_render_N`.
**codegen/index.ts** — imports `fileDeclaresRender` + `setRenderShadowedInFile`; wires both per-file sites.
**type-system.ts** — NEW `checkRenderShadowing(fileAST, errors)` fires W-RENDER-SHADOWED at every file-level
`function render`/`fn render` decl; wired at type pass.

### ss16 C4+C5 — ==payload-variant lint + ctor-arg contextual typing

**type-system.ts** — NEW `checkEqPayloadVariantOperands(exprNode, typeRegistry, span, errors)` fires
W-EQ-PAYLOAD-VARIANT when `==`/`!=` operand is a `.Variant(args)` call against a declared payload variant.
NEW `inferBareVariantsAtVariantCtorArgs(exprNode, ctxType, typeRegistry, span, errors)` — contextual
typing for bare `.V(arg)` call-args using the payload-field shape; suppresses spurious E-TYPE-063.
Both wired at: `let`/`const` RHS, reactive cell init, bare-expr, if-condition, return expr-walk sites.

### ss15 item-1 — tailwind author-class exclusion

**compiler/src/tailwind-classes.js** — NEW `collectAuthorDefinedClasses(source)` (+85L): brace-balanced
scan of `#{}` CSS blocks and `<style>...</style>` blocks extracts every `.identifier` class selector the
author defines; first char `[A-Za-z_-]` guard prevents numeric CSS fractions being mistaken for selectors.
`findUnrecognizedClasses` calls this pre-pass and excludes the returned Set from `W-TAILWIND-UNRECOGNIZED-CLASS`.

### ss15 item-2 — bare-expr in default-logic position is an EFFECT not a render slot

**compiler/src/codegen/emit-html.ts** — NEW `DEFAULT_LOGIC_MODE_TAGS = Set(["program","page","channel"])`;
`generateHtml` signature gains optional 6th param `nestedMarkupContext?: boolean`; NEW `markupParentStack: string[]`
seeded `["__nested-markup__"]` when `nestedMarkupContext=true`. A bare-expr whose `enclosingMarkupTag` is in
`DEFAULT_LOGIC_MODE_TAGS` (or top-of-stack is `__nested-markup__`) is classified as a mount EFFECT → no
`<span data-scrml-logic>` render slot. A bare-expr inside a real markup element still renders (that element
pushed its tag first). Closes g-on-mount-bare-call-render-slot.
**compiler/src/ast-builder.js** — `preprocessWorkerAndStateRefs(raw, childRanges)` gains a `childRanges` param
for the ss15 request-bridge lift-path (avoids re-parsing child ranges that are already request-state refs).

### ss15 items 3+4 — `<request> <#id>` lift-path bridge (if-stmt / for-stmt)

**compiler/src/codegen/emit-lift.js** — NEW `pushLiftRequestIds(set)` + `popLiftRequestIds()` +
`currentLiftRequestIds()` exports; `liftExprCtx(extra)` reads top-of-stack + adds `requestIds` to every
`EmitExprContext`; NEW `liftExprReadsRequestState(rewritten)` — if the rewritten lift expression references
`_scrml_request_<id>`, wraps the `textContent` write in `_scrml_effect(...)` for reactive re-render.
**compiler/src/codegen/emit-control-flow.ts** — `IfOpts` and for-stmt opts gain `requestIds?: Set<string>|null`;
`emitIfStmt` and `emitForStmt` wrap their inner implementations in `pushLiftRequestIds`/`popLiftRequestIds`
try/finally; `_ifExprCtx` gains `requestIds`; nested else-if bodies thread `requestIds` forward.
**compiler/src/codegen/emit-variant-guard.ts** — `emitArmRenderFunction` and `emitInitialArmHtmlForMount`
pass `nestedMarkupContext: true` (6th arg) to lazy-required `generateHtml` calls.

### A2 W5 — example 32 (§60 BYOB external API)

**examples/32-external-api.scrml** (NEW, ~137L) — canonical §60 BYOB typed-external-API example.
Two endpoints: `getUser(UserQuery) -> GET "/users/${id}" : UserResult` (variant GET) + `sendFeedback(FeedbackInput) -> POST "/feedback" : FeedbackResult` (body POST). Inline commentary covers §60.1 identity-reframe, §60.3 must-not-lie, §60.5 boundary-parse, §60.6 pure-client (no .server.js), §60.7 LIMIT-PRIMITIVES.
§60 status flipped **Nominal → Implemented** (A2 Wave 5 complete).
**examples/README.md** — updated to add example 32 entry.

## Key S217 Source Changes (`062165a5`)

### escalation-2 — `__ri_route_*` author-route binding fix (route-inference.ts)

**compiler/src/route-inference.ts** [lines ~3663-3676]:
- `generateRouteName(functionName)` [line ~835] — ALWAYS synthesizes a `__ri_route_<safe>_<counter>` identifier.
  An author-declared `route="/some/path"` on a `server function*` SSE / `handle()` is the HTTP request path
  (the stable foreign-facing contract URL per §12.3 carve-out), NOT a JS binding identifier. Pre-fix:
  `getOrCreateRouteEntry` used the raw author path string as the generated route name → emitted
  `export const /fsp/deltas = { ... }` — an ESM `SyntaxError` on any route containing `/`. Fix: always
  use `generateRouteName()` for the JS binding; `hasExplicitRoute` carries the author path into
  `explicitRoute` separately for emit-server to mount at that URL. Application-mode-only: library/SSE
  functions with no `route=` are unaffected (codepath unchanged).

### GITI-029 — comment-flushed `on mount` lift gate (ast-builder.js)

**compiler/src/ast-builder.js** [lines 577-578, 1606-1623]:
- `TOPLEVEL_ON_LIFECYCLE_RE = /^\s*on\s+(?:mount|dismount)\s*\{/` — NEW module-level constant [line 577].
  A `//` line comment in the same text block as `on mount {}` causes the BS to flush the comment as a
  separate chunk; the lifecycle directive then arrives as the LEADING token of a NEW text block (`on` is
  a contextual identifier, not a decl keyword — no existing lift gate matched). The directive shipped RAW
  into the DOM, silently defeating the hook.
- `preprocessBSBlocksForAst` [line ~1606]: when a top-level text block (gated `isDefaultLogicBody`) matches
  `TOPLEVEL_ON_LIFECYCLE_RE`, wraps it in a synthetic `${ ... }` logic block carrying `_bareDeclLift: true`
  + `_onLifecycleLift: true` markers (same marker class as `_atWriteLift` / `_tildeBearingLift` / `_bareDeclLift`).
  Conservative regex: requires `on` + `mount`/`dismount` + `{` so prose like `on Monday { ... }` never matches.
  The `_onLifecycleLift` marker is LIVE-only; within-node-classifier STRIP_KEYS += `"_onLifecycleLift"`.

### g-onmount — `_onMountEffect` unconditional fire-and-forget marker (ast-builder.js + emit-html.ts)

**compiler/src/ast-builder.js** [4 stamp sites]:
- `parseOneStatement` on-mount/dismount desugar [lines ~8739, ~8753]: bare-expr node stamped
  `_onMountEffect: true`.
- Markup-child on-mount/dismount desugar [lines ~12146, ~12160]: same stamp.

**compiler/src/codegen/emit-html.ts** [line ~2479]:
- `_onMountEffect` skip gate added to the `logic`-node binding loop [line 2479]:
  `if (child.kind === "bare-expr" && child._onMountEffect) continue;`
  A lifecycle bare-expr stamped `_onMountEffect` skips `addLogicBinding` + render-slot allocation in
  ALL enclosing contexts — including inside a `<db>`/`<program db=>` body where `inDefaultLogicMode` is
  false. Pre-fix: in a `<db>` body, the bare-expr from `on mount { val() }` registered a logic binding
  → emit-event-wiring emitted `_scrml_render_value(el, val())` (`[object Promise]` at page top) + an
  `_scrml_effect` re-run wrapper. The `_onMountEffect` skip gates BEFORE the `inDefaultLogicMode` skip
  (which only covers the strictly-default-logic case). LIVE-only marker; within-node-classifier STRIP_KEYS
  += `"_onMountEffect"`.

### GITI-030 — `<code>`/`<pre>` raw-content body verbatim in `<each>` per-item emit (emit-each.ts)

**compiler/src/codegen/emit-each.ts** [lines 84-95, ~390-480]:
- `RAW_CONTENT_ELEMENT_NAMES = new Set(["pre", "code"])` [line 95] — mirrors `RAW_CONTENT_ELEMENTS` in
  block-splitter.js + `RAW_CONTENT_NAMES` in lint-w-interp-in-raw-content.js.
- `renderTemplateChildToJs` [lines ~390-479]: when descending into a raw-content element's children,
  sets `insideRawContent = true` and skips `rewriteContextualSigil`. Pre-fix: `${@.id}` inside
  `<code>${@.id}</code>` was half-rewritten to `${_scrml_each_item.id}` (corrupting the §4.17 verbatim
  literal). Per §4.17 a `<pre>`/`<code>` body is a single raw text run — no interpolation, no sigil-rewrite.

### GITI-031 — `subscribeSubPath` for `<match on=@cell.subfield>` dispatch (emit-match.ts + emit-variant-guard.ts)

**compiler/src/codegen/emit-match.ts** [lines 215-330]:
- `OnExprResolution` interface [line ~215] gains `subscribeSubPath: string` field.
- `resolveOnExpr` [line ~323-331]: member-access branch `@ident.path` captures the dotted suffix into
  `subscribeSubPath` (e.g. `".state"` for `on=@cell.state`); bare `@ident` returns `subscribeSubPath: ""`;
  all Shape-B forms (expression, string-literal, bare-variant) return `subscribeSubPath: ""`.
- `emitMatchBodyRenderForFile` [line ~1152]: threads `subscribeSubPath` from `onResolved` into the
  `emitVariantGuardedRender` options object.

**compiler/src/codegen/emit-variant-guard.ts** [lines 225-232, 987-1165]:
- `EmitVariantGuardOpts` interface [line ~225] gains `subscribeSubPath?: string` field.
- `emitVariantGuardedRender` [line ~987]: reads `const subPath = opts.subscribeSubPath ?? ""`.
- Subscribe callback [line ~1165]: emits `_scrml_reactive_subscribe(name, function(_cv) { dispatchFn((_cv)${subPath}); })` when `subPath` is non-empty; bare `@cell` (empty `subPath`) stays `dispatchFn(_cv)`.
- DOMContentLoaded init-fire [line ~1193]: reads `_scrml_reactive_get(name)${subPath}`.

### within-node-classifier.ts — STRIP_KEYS rebaseline (native-parser-canary/within-node-classifier.ts)

**compiler/src/native-parser-canary/within-node-classifier.ts** [lines ~171-187]:
- `STRIP_KEYS` set gains two new entries:
  - `"_onMountEffect"` [line 171] — LIVE-only bare-expr lifecycle-effect marker (g-onmount / S217).
  - `"_onLifecycleLift"` [line 182] — LIVE-only synthetic-`${}` lift-origin marker (GITI-029 / S217).
  Both are pipeline-internal codegen-support fields with no native-parser analogue; the classifier skips
  them to avoid false divergence counts.


## Key S218 Source Changes — dpa-003 `_{}` inline foreign-code primitive + GITI-032 + 6nz Bug AI (since watermark 489951aa → 162564f3)

Nine compiler source files changed. `162564f3`.

### GITI-032 — markup-as-value in `<match>`/`<engine>` arm body + native→live AST bridge fix

- compiler/src/ast-builder.js — `isHtmlFragment` over-fire fix (markup-as-value-in-expression-2026-06-17): `containsMarkupTokens(expr)` gated by start-anchor discriminator (ast-builder.js ~:207). A sub-expression inside a `<match>` or `<engine>` arm body that contained a `<` token was incorrectly routed through the markup-as-value parse path; the fix requires the expression to START with a markup opener (not merely contain one). Also: `preprocessWorkerAndStateRefs` `childRanges` param widened for the native→live AST bridge to correctly carry per-child span context through the ForeignBlock producer path. ALSO this file is the PRIMARY PRODUCER of `ForeignBlock` nodes — `buildBlock case "foreign"` (~:15864) parses the `_=*{ … }=*` interior, extracts the optional `in:{ name }` crossing header, and returns the `{kind:"foreign", level, lang:null, raw, body, crossings, span}` node shape. `foreignNode` attachment on `let-decl` (~:6947), `const-decl` (~:7054), `return-stmt` (~:7705). `_BLOCKREF_PP_TYPES` += `"foreign"` (~:15639).
- compiler/src/codegen/emit-each.ts — GITI-032 adjacent: markup-value in per-item emit. `RAW_CONTENT_ELEMENT_NAMES` guard ensures `<pre>`/`<code>` body verbatim text is not rewritten as contextual sigil — `insideRawContent` flag propagated through recursive child walk (GITI-030; also landed S217 adjacent).
- compiler/src/codegen/emit-variant-guard.ts — arm display updated to route through node-aware `_scrml_render_value(el, rewrittenExpr)` (GITI-032 adjacent, ~:548) for the initial-arm and effect-wired paths; primitive values remain byte-identical; markup-value arms now display correctly in `<match>`/`<engine>` arm bodies. Also carries `EmitVariantGuardOpts.subscribeSubPath` field for GITI-031 (S217).

### 6nz Bug AI — each/empty reconcile teardown fix

- compiler/src/runtime-template.js — `_scrml_reconcile_list` (`_scrml_reconcile_list` ~:1541): added `container.replaceChildren()` at the entry of the `oldNodes.size === 0` non-empty path (~:1621). On the empty→non-empty edge, the empty-branch DOM placeholder was not torn down before the new keyed items were appended, leaving stale empty-state DOM. Fix mirrors the existing `oldNodes.size > 0` path (which owns its teardown). Zero new runtime helpers; ZERO new §34 codes.

### dpa-003 — `_{}` inline foreign-code primitive (the major new feature)

- compiler/src/block-splitter.js — BS-gate: `_=*{` opener recognition at ~:2018 (`matchForeignOpener`) + `pushForeignContext` (~:2043) emitting a `type:"foreign"` frame with `foreignLevel` on the stack. Opaque interior scan at `frame.type === "foreign"` (~:2356): brace-depth-aware + level-aware closer (`}` + exactly `foreignLevel` `=` chars). `buildBlock case "foreign"` slices the right opener/closer width using `foreignLevel` (+`block.foreignLevel` threaded). Orphan-brace body: a `_=*{` inside an orphan-brace body is recognized and re-split on the `${...}` path (~:2784). `f.type === "foreign"` added to the frame-guard filter at ~:816.
- compiler/src/tokenizer.ts — `BLOCKREF_TYPES += "foreign"` (~:1166): the tokenizer emits a `BLOCK_REF` placeholder token for a `foreign` child block so the logic token stream carries the embedded foreign slice (mirrors the existing `sql`/`css`/`error-effect`/`meta` children treatment).
- compiler/src/codegen/emit-logic.ts — NEW `case "foreign"` (~:2749): wraps the opaque verbatim slice in an `async IIFE`: `await (async (params) => { inner })(args)`. A single-expression slice (no top-level `;` or `return`) gets `return (slice);` so the value flows back; a multi-statement slice is spliced verbatim (author owns value-flow via their own `return`). The `scanForeignSliceShape` helper (~:2784) is a brace/paren/bracket-depth-aware, string/comment-skipping scan over the opaque slice — syntactic only, no parse, §23.2.3 opacity preserved. `in:{}` crossings become IIFE params/args. `let-decl.foreignNode` and `const-decl.foreignNode` paths (~:1737/:1856) route through `case "foreign"` and emit a client-side stub comment on the client boundary (server-only slice). Mirrors `case "sql"` end-to-end.
- compiler/src/route-inference.ts — foreign-block server-color escalation: three sites add `kind === "foreign"` / `foreignNode?.kind === "foreign"` escalation checks (~:986/:990/:1107/:1110/:1231/:1232/:2014/:2015/:2115/:2116/:2117) so any fn containing a ForeignBlock node (directly or via `foreignNode` stamp) is classified as a server fn. Mirrors the `sqlNode` escalation pattern exactly. `resourceType: "foreign-inline"` for the escalation reason. Client-stripped on slice (RI classifies server; codegen emits stub on client boundary).
- compiler/src/type-system.ts — foreign OUT-typing + validation: `checkForeignBlocks(fileAST, programLang, errors)` (~:18614; wired at type pass ~:19624) — two-pass walk: Pass 1 collects admitted `foreignNode`-bearing decls/returns; Pass 2 walks every `kind:"foreign"` node: admitted nodes get the lang gate (`E-FOREIGN-003` / `E-FOREIGN-005`) + a `lang` stamp resolved from `programLang`; bare nodes get `E-FOREIGN-004`. The `programLang` is resolved from the ancestor `<program lang=>` attribute (passed in as an arg from the type-pass file-loop). OUT typing for `let/const x = _{}` decls (~:8656): defaults to `asIs` (§14.7 — a foreign value is UNOWNED; no inference from the opaque side); an explicit `typeAnnotation` on the decl overrides (via `resolveTypeExpr`). NEW codes: E-FOREIGN-003 / E-FOREIGN-004 / E-FOREIGN-005 (all Fatal).

**+3 NEW §34 codes (E-FOREIGN-003 / E-FOREIGN-004 / E-FOREIGN-005); ZERO new FileAST node types in ast.ts (ForeignBlock is a LIVE-pipeline-only node shape produced by ast-builder.js, NOT declared in types/ast.ts); BLOCKREF_TYPES += "foreign" in tokenizer.ts.**

## Key S219 Source Changes — 6nz B2 arrow-body reject + W-INPUT-STATE-MARKUP-NONREACTIVE lint + §61 spec (since watermark 162564f3 → 26ffea4e)

Six compiler source files changed. One NEW source file. Two NEW test files. `26ffea4e`.

### 6nz B2 — E-ENGINE-RULE-LEGACY-SYNTAX whole-body fire site on `<engine for=T>` without `name=`

- **compiler/src/types/ast.ts** (2081L) — NEW field `hadNameAttr?: boolean` on `EngineDeclNode` (interface line ~979): TRUE iff an explicit `name=` attribute was present on the engine opener (the §51.3.2 NAMED-machine form `<engine name=X for=T>`, which legitimately admits a whole-body arrow grammar). FALSE for the §51.0.C state-engine form (`<engine for=T initial=...>`), whose body SHALL use state-child `rule=` openers. Read by SYM PASS 11/B15 to restrict the new fire site to state-engine-only.
- **compiler/src/ast-builder.js** (16984L) — 6nz B2 stamp: `buildBlock case "engine"` extracts `nameMatch` from the opener attribute list and records `hadNameAttr: nameMatch !== null` on the engine node (~:15419-15427). Scope guard: fires E-ENGINE-RULE-LEGACY-SYNTAX only when `!isLegacyMachineKeyword && !isNamedMachine && !isDerivedEngine` (the state-engine form with a whole-body `.From => .To` body but no `name=` attribute — previously compiled silently to a `__scrml_transitions` table WITHOUT the §51.0.C cell auto-declaration/init, leaving `@var` undefined at mount).
- **compiler/src/symbol-table.ts** (12291L) — SYM PASS 11 / B15 fire site: `validateEngineStateChildrenAndRules` gains the three-clause exemption check (`isLegacyMachineKeyword` / `isNamedMachine` / `isDerivedEngine`) before `fireB15Diagnostic(errors, "E-ENGINE-RULE-LEGACY-SYNTAX", ...)`. Pre-fix: a `<engine for=T>` with a whole-body arrow body was classified as a legacy MachineType, emitted `__scrml_transitions_<var>` but SKIPPED the §51.0.C cell auto-declaration/init — the governed cell was `undefined` at mount with ZERO diagnostic. Fix fires a targeted Error steers to state-child `rule=`. ZERO new §34 codes (E-ENGINE-RULE-LEGACY-SYNTAX was already in the catalog; the whole-body fire site is NEW — "Catalog addition S68 — A1b B15; whole-body fire-site added 6nz B2 2026-06-24." in §34).
- **compiler/native-parser/collect-hoisted.js** — `synthEngineDecl` parity stamp: adds `hadNameAttr: legacyName !== null` to the synthesized native `EngineDeclNode` shape (~:461), matching ast-builder.js so SYM PASS 11 sees a consistent `hadNameAttr` on both parse paths.
- **compiler/tests/unit/engine-arrow-body-legacy-reject-b2.test.js** (NEW 150L) — 6nz B2 unit test. Covers: `<engine for=T>` whole-body arrow fires E-ENGINE-RULE-LEGACY-SYNTAX; named form `<engine name=X for=T>` whole-body arrow is EXEMPT; `<machine>` keyword whole-body arrow is EXEMPT.
- **compiler/tests/unit/engine-statechild-b15.test.js** (700L, EXTENDED) — extended with B2 cases.

### 6nz B1 — W-INTERP-IN-RAW-CONTENT promoted Info → Warning

- **compiler/src/lint-w-interp-in-raw-content.js** — severity promoted from `info` to `warning` (6nz B1, 2026-06-24). A dropped interpolation inside `<pre>`/`<code>` is a silent rendering break, not merely informational. Partitions into `result.warnings` (non-fatal; CLI exit unchanged) per the `W-` prefix; the SPEC §34 row entry updated: "Promoted info→warning 6nz B1".
- **compiler/tests/unit/lint-w-interp-in-raw-content.test.js** (EXTENDED) — severity assertions updated to `warning`.

### W-INPUT-STATE-MARKUP-NONREACTIVE lint (6nz AF, ratified S210 + S219)

- **compiler/src/lint-w-input-state-markup-nonreactive.js** (NEW 292L) — post-BS info-level lint: surfaces input-state `<#id>.field` reads (`<keyboard>` / `<mouse>` / `<gamepad>`, §36) placed inside markup interpolations (`${ … }` in a markup body). Per §36.1 + §36.6 input-state is a LIVE-READ source, NOT a reactive cell — the interpolation renders ONCE at mount with NO re-render on input change. That behavior is by-design and CORRECT (input drives imperative game logic at `animationFrame`, not reactive UI); the defect closed is the SILENCE. **Spec:** SPEC §36.6, fire steered to `W-INPUT-STATE-MARKUP-NONREACTIVE`. **Resolution in diagnostic:** use the `@cell` bridge (read `<#id>` inside an `animationFrame` loop, assign to an `@cell`, then interpolate the cell). **Conservative by construction:** fires ONLY when the ref id matches a DECLARED input-state element (never the §6.7.7 `<request>` render bridge) and NOT inside an `animationFrame` loop (the bridge / game loop). Runs as a post-BS pass over the block-split AST (`bsResults`), alongside the sibling `runWInterpInRawContent`. A markup `${ … }` interpolation is a BS-captured `{ type: "logic" }` node; the lint scans the interior expression text.
- **compiler/src/api.js** (2827L) — Stage 2.5b: imports `runWInputStateMarkupNonreactive` from `lint-w-input-state-markup-nonreactive.js` and invokes it over `bsResults` alongside the sibling raw-content lint; diagnostics pushed through `collectErrors("BS-LINT", ...)` so the `W-` prefix + `severity:"info"` partition them into `result.warnings` (non-fatal).
- **compiler/tests/unit/lint-w-input-state-markup-nonreactive.test.js** (NEW 266L) — W-INPUT-STATE-MARKUP-NONREACTIVE unit test. Covers: keyboard/mouse/gamepad `<#id>.field` inside markup interp fires; `<#id>` not a declared input-state does NOT fire; inside an `animationFrame` callback EXEMPT; `<request>` ref does NOT fire.

### §61 `<endpoint>` typed-inbound-endpoint SPEC section (Nominal/spec-ahead)

- **compiler/SPEC.md** — NEW §61 section (33189–33345L, 157L). Nominal/spec-ahead; W1 of the `<endpoint>` build arc (NO compiler implementation as of S219 — W2 parser / W3 typer / W4 codegen / W5 tests follow). The typed INBOUND edge — the serve-side mirror of §60 `<api>` (typed outbound ⇄ typed inbound): a foreign client calls a scrml-served route, the compiler owns the decode + exhaustive dispatch + the JSON envelope, the author fills per-variant arms (REUSE §18.0.1 arm + §51.0.B.1 payload binding). Grammar: `<endpoint path= method= accepts=:enum>`. Decode: `parseVariant` §41.13 over the body. Exhaustiveness: `E-ENDPOINT-NOT-EXHAUSTIVE` (the inbound-honesty guarantee — a new `accepts=` variant without an arm = compile error). §61.2–§61.11 grammar/decode/exhaustiveness/JSON-envelope/client-codegen-SKIP/method-path-auth/relationship-map/planned-codes/limits/xrefs. Named planned codes (reserved; §34 rows land WITH impl per Rule 4): E-ENDPOINT-NOT-EXHAUSTIVE / E-ENDPOINT-ACCEPTS-NOT-ENUM / E-ENDPOINT-PATH-MISSING / E-ENDPOINT-METHOD-INVALID / E-ENDPOINT-ACCEPTS-MISSING. Registry rows: §4.15 + §24.4. Section count 60 → 61. Authority: dpa-002 reopen DD + S219 ratification. Build arc: `docs/changes/endpoint-primitive-2026-06-25/`.
- **compiler/SPEC-INDEX.md** — §61 row added to Sections-table (row 107: §61 Typed Inbound Endpoint — `<endpoint>`, lines 33189–33345, 157L); total sections updated 60 → 61 in header ("Total lines: 33,344 | Total sections: 61 + appendices"). Quick-Lookup updated (+3 entries for `<endpoint>`). Line-ranges regenerated via `bun scripts/regen-spec-index.ts`. No other section ranges shifted (§61 appended at SPEC end).

**+1 NEW §34 code (W-INPUT-STATE-MARKUP-NONREACTIVE); ZERO new FileAST node types in ast.ts (ForeignBlock was already live-only; `hadNameAttr` is a NEW field on existing EngineDeclNode, not a new node type); E-ENGINE-RULE-LEGACY-SYNTAX whole-body fire site is NEW but the code was pre-existing in the §34 catalog. §61 is Nominal/spec-ahead — ZERO implementation changes.**


## Key S220 Source Changes — ss17 each-peritem emitter + ss18 `<endpoint>` Implemented + ss19 auth-wall + ss20 render-codegen + g-paren-ternary (since watermark 26ffea4e → 852d6aa7)

Sixteen NEW test files (+7 browser, +5 integration, +4 unit). 1071 .test.js at `852d6aa7`. §61 `<endpoint>` promoted Nominal → **Implemented** (W2 parser + W3 typer + W4 codegen + W5 tests LANDED). Five E-ENDPOINT-* codes wired. `852d6aa7`.

### ss17 — each-codegen per-item emitter completeness (3 MED gaps closed)

- **compiler/src/codegen/emit-each.ts** — NEW functions (ss17 per-item emitter completeness):
  - `emitEachPerItemMarkupValue(exprNode, iterVarName, fragmentVar, lines, indent)` — lowers a markup-as-value expression (from a `return <markup>` or ternary arm) inside an `<each>` per-item body; reconciles instead of skipping. Routes per-item markup-as-value through the existing `createFn` reconcile path.
  - `buildEachExprHandlerBody(preLowered, iterVarName): string` — constructs the function body string for an expression-form event handler (bare arrow / `fn(params){body}`) inside a per-item context, adding the own `function(event){}` wrapper and Bug-73 live-keying prelude.
  - `splitMarkupTextInterp(value)`, `splitEachMarkupTextChildren(children, iterVarName)`, `rewriteEachScopeInExprNode(n, iterVarName)` — internal helpers for markup-text interpolation rewriting in the each-scope context.
  - `_scrml_resolve_item` call injected in nested-each outer-context recovery: when a handler inside an inner `<each>` reads the outer iter var, `_scrml_resolve_item(outerMount, outerKey)` is emitted to re-resolve the live outer item (prevents stale outer-item reads after reconcile).

### ss18 — §61 `<endpoint>` typed-inbound-endpoint: W2 parser + W3 typer + W4 codegen + W5 tests (LANDED)

- **compiler/src/ast-builder.js** — W2 parser: `case endpoint` in `buildBlock` (after `case api`). Recognizes the `<endpoint path= method= accepts=Enum>` structural element from the BS `type=markup name=endpoint` block. Parses `path=`/`method=`/`accepts=` opener attrs. Fires parse-time errors: `E-ENDPOINT-PATH-MISSING` (no `path=`), `E-ENDPOINT-METHOD-INVALID` (bad/absent HTTP method), `E-ENDPOINT-ACCEPTS-MISSING` (no `accepts=`). Arm bodies parsed in the same `buildArmBodies` path as `<match>`/`<engine>`. Produces `kind: endpoint-decl` AST node (LIVE-pipeline-only runtime shape; NOT in types/ast.ts — mirrors the `api-decl` precedent). NEW helper `_findEndpointOpenerEnd` for parsing the opener boundary.
- **compiler/src/block-splitter.js** — registers `endpoint` in the structural block recognizer alongside `api`, `match`, `each` — the BS produces `type=markup name=endpoint` blocks with arm-body content (NOT a line-format body like `<api>`).
- **compiler/src/type-system.ts** — W3 typer: NEW `checkEndpointDeclarations(allNodes, typeRegistry, apiExemptTypeNames, errors, fileSpan)`. Internal helpers: `collectEndpointDecls` (deep-walk for `kind:endpoint-decl` nodes, mirrors A2 W5 `collectApiDecls`), `checkEndpointAcceptsRef` (resolves `accepts=` against type registry). Fires: `E-ENDPOINT-ACCEPTS-NOT-ENUM` (non-enum `accepts=`; undeclared reuses `E-TYPE-UNKNOWN-NAME`), `E-ENDPOINT-NOT-EXHAUSTIVE` (unhandled `accepts=` variant). Reuses §18.0.1/§51 exhaustiveness machinery (`checkEnumExhaustiveness`). Wired at the type pass after `checkApiDeclarations`.
- **compiler/src/codegen/emit-server.ts** — W4 codegen: NEW `emitEndpointServerHelperLines(fnNodeId, opts)` — emits a LOCAL non-exported pure `function <name>(...) {}` for an `<endpoint>` arm-body helper (§61.2 canonical arm); consumes `RouteMap.endpointServerHelperIds`. NEW `collectEndpointDecls` (codegen-side collector) + `parseArmBindings` (arm payload binding extractor). Per-`<endpoint>` emission: async handler `async function _scrml_endpoint_<id>(_scrml_req) { ... }` decodes via `parseVariant` §41.13, dispatches through arms via the inline `emitEndpointArmEnvelope`, emits `_scrml_endpoint_decode_error` for malformed body, exports route record `{ path, method, handler }`. Client-codegen SKIP (§61.6): server-only — `emit-functions.ts` gates on `endpointClientSkipIds`.
- **compiler/src/route-inference.ts** — NEW fields on `RouteMap`: `endpointServerHelperIds: Set\<string\>` (non-exported pure fns referenced by endpoint arm bodies — need server emission, bypass the value-export path), `endpointClientSkipIds: Set\<string\>` (subset of `endpointServerHelperIds` NOT also client-reachable from markup/exported fns). Closure built from `endpointArmReferencedNames` (identifier scan over all arm `bodyRaw` strings). Also: §19 #8 peer-await path consistency + #9 db-src runtime path fix (ss19 routing fixes in the same landing).
- **compiler/src/codegen/emit-functions.ts** — consumes `routeMap.endpointClientSkipIds`: a function whose `fnNodeId` is in the skip set is NOT emitted to the client bundle (§61.6 client-codegen-SKIP).
- **compiler/src/codegen/index.ts** — threads `endpointServerHelperIds` + `endpointClientSkipIds` from route-map into the `CgInput` shape consumed by `runCG`.

### ss19 — Ryan Cheese-Craft Group A: 7 auth-wall fixes

- **compiler/src/auth-graph.ts** — diagnostic message improvements: steer messages now point to `pages/login.scrml` (the corrected default path — was `pages/auth/login.scrml`; matches the generate-auth fix below). No new codes; message-only reword.
- **compiler/src/protect-analyzer.ts** — NEW `harvestRawCreateTables(text, out)`: harvests raw `CREATE TABLE … (…)` DDL statements from a plain text string into the ColumnDef map, reusing `CREATE_TABLE_RE`. Wired into `parseSchemaBlock` to handle a `<schema>` block carrying RAW SQL DDL directly (g-schema-block-raw-ddl — previously only the declarative `tableName { … }` form was recognized; raw DDL reached neither `parseSchemaBlock` nor the `?{}` walker, causing spurious `E-PA-002`). ZERO new §34 codes.
- **compiler/src/commands/generate.js** — `generateAuth` default output path corrected: `pages/login.scrml` (was `pages/auth/login.scrml`). The auth gate redirects to the path declared in the scrml app's protect config; the scaffold must land where the gate actually points. Help text + inline comment updated.
- **compiler/src/codegen/emit-client.ts** — ss19 #5 stdlib-client strip: the client-bundle function-emission loop (emitting named client-side functions) now checks `routeMap.endpointClientSkipIds?.has(fnNodeId)` and skips emission for endpoint-arm helper fns. Mirrors the `endpointClientSkipIds` contract from route-inference + emit-functions.

### ss20 — 5 render-codegen fixes (2 HIGH, 3 MED)

- **compiler/src/codegen/emit-html.ts** — `ifGuard` tracking: NEW local `interface IfDisplayGuard` (condition expression or display-toggle variant) + `ifGuardStack: IfDisplayGuard[]` + push/pop at `if=`/`show=`-bearing elements. Stamped as `ifGuard` on `addLogicBinding` calls for both interpolation and attr-binding sites. `computeDisplayToggleCondition` helper builds the JS guard expression from the stack-top `IfDisplayGuard`. Compound-bind source-leaf fix: when the dotted path's root is a Variant-C structural nested-object cell, the bind target resolves to the compound leaf (g-compound-bind-value-source-cell); backing-leaf sets populated via `compoundLeafKeys`.
- **compiler/src/codegen/emit-bindings.ts** — NEW `ifGuard` field on the `LogicBinding` / binding shape (compound-bind + interpolation). The reactive `_scrml_effect` emission gate: when `binding.ifGuard` is present, `computeDisplayToggleCondition` is called and the effect body is wrapped `if (!(guard)) return;` before the DOM update — so the toggle and the effects it guards stay in lockstep (g-if-guard-inner-effect-not-gated HIGH).
- **compiler/src/codegen/emit-event-wiring.ts** — if-guard threading into the `_update_chain` and per-binding `_scrml_effect` wrappers; each-submit `preventDefault` fix (g-each-mount-form-submit-preventdefault): a submit handler inside an `<each>` body now emits `event.preventDefault()` before the server-fn call (mirrors the non-each form handler path).
- **compiler/src/codegen/binding-registry.ts** — backing-leaf tracking for compound-bind: `compoundLeafKeys` field added to the registry binding shape so `emit-bindings` can resolve the source-leaf correctly for compound `bind:` directives.
- **compiler/src/codegen/emit-lift.js** — markup-text interpolation reactive lowering (g-emit-lift-markup-text-interp ss20 items 4+5): a markup TEXT child whose value carries an `${...}` interpolation is now lowered via `_scrml_effect(function() { textNode.textContent = String((expr) ?? \); })` rather than a one-shot string append. `splitMarkupTextInterp` helper splits TEXT vs EXPR segments so only the interpolated portion subscribes reactively.
- **compiler/src/codegen/emit-variant-guard.ts** — arm-body display pass: the `_scrml_render_value` arm path (S217 GITI-032) updated to thread the `ifGuard` from the enclosing scope so that a `match`-arm display effect also obeys the if-guard gate.

### g-paren-ternary — re-parenthesize loose operands of binary/ternary expressions (HIGH, flogence)

- **compiler/src/codegen/emit-expr.ts** — NEW `binaryOperandNeedsParens(node, parentKind): boolean` (sibling of `receiverNeedsParens`, S210): a BINARY or TERNARY operand that is itself a conditional (`?:`), binary, or prefix-unary expression is re-parenthesized when required by JS operator-precedence rules — closing the class where `@a + @b * @c` emitted correct precedence but `cond ? @a : @b - @c` dropped parens around the ternary arms. NEW `isAwaitedPeerCall(node, ctx)` helper — guards the special peer-call-receiver case (an awaited peer call that is also a receiver needs grouping parens, e.g. `(await peer(...)).field`). ZERO new §34 codes (re-parenthesization is a PURE codegen correctness fix).

**+5 NEW §34 codes (E-ENDPOINT-PATH-MISSING, E-ENDPOINT-METHOD-INVALID, E-ENDPOINT-ACCEPTS-MISSING — W2 parse-time; E-ENDPOINT-NOT-EXHAUSTIVE, E-ENDPOINT-ACCEPTS-NOT-ENUM — W3 typer). ss17 / ss19 / ss20 / g-paren-ternary carry ZERO new diagnostic codes. ONE new runtime-only AST shape: `endpoint-decl` (live-pipeline-only, NOT in types/ast.ts — mirrors api-decl). find-count 1071 at `852d6aa7` (verified `find compiler/tests -name '*.test.js' | wc -l` = 1071).**


### ss31 — codegen expr-validity (3 fixes B1/B2/B3, S222)

- **compiler/src/ast-builder.js** (B1) — exponent-diagnostic plumbing: a bare prefix-unary left of `**` (`-@a ** 2`) is surfaced LOUD at parse-time as a targeted `E-CODEGEN-INVALID-JS` ("write (-@a) ** 2") instead of silently emitting invalid JS — the acorn-escape-hatch path the S221 AST guard missed. NO auto-paren (no design fork taken).
- **compiler/src/expression-parser.ts** (B1) — the exponent-left-unary diagnostic detection that feeds `ast-builder.js`; parse-time surfacing (the choke-point that catches the bare unary before it reaches codegen).
- **compiler/src/codegen/emit-expr.ts** (B2+B3) — B2: client structured `<x> = Enum.toEnum(@raw)` lowers to `(Enum_toEnum[..] ?? null)` on the AST path (the Pass-9 string-rewrite twin, robust to nested-paren args; client-gated; mirrors the ss22 server fix, §14.4.3). B3: `-(-a)` serializes as `- -a` (was `--a`, a pre-decrement) via a minimal space-split in both serializers. Follow-on minted: `g-unary-of-exponent-arg-no-paren` (the `**`-ARGUMENT paren sibling `-(@a ** 2)`).

### ss37 — value-native Set `set[K]` (§59.12 B2 map-alias, S222)

- **compiler/src/type-system.ts** (D1) — `MapType.set` flag + `tSet(key)` (→ `{kind:"map", value:boolean, ordered:false, set:true}`) + `set[…]` resolution in `resolveTypeExpr` + `forEachTypeNameLeaf` set branch + set-aware diagnostic render (`set[K]` not `[K:bool]`) + set-aware `E-MAP-BRACKET-WRITE` message. A set IS a map → inherits §59.4 key-comparability, the §59.7 bracket-write gate, §59.9 order-independent `==`, the §57 codec — UNCHANGED.
- **compiler/src/codegen/reactive-deps.ts** (D3) — NEW `collectSetVarNames(fileAST): Set<string>` (strict subset of map cells) + `isSetTypeAnnotation(anno)` + `fileHasSetAlgebraUsage(...)` (precise stdlib-data chunk gate); set cells ALSO join `collectMapVarNames` so `.has`/`.remove`/`.size`/bracket-read ride the `_scrml_map_*` surface for free.
- **compiler/src/codegen/emit-expr.ts** (D4) — NEW `setVarNames?: Set<string>` on `EmitExprContext` + set-vocab method interception (FIRES BEFORE the map-method interception so set spellings win): `.add(k)`→`_scrml_map_insert(s,k,true)`; `.elements()` / `<each in=@s>`→`_scrml_map_keys(s)`; `.union`/`.intersect`/`.difference(t)`→DELEGATE to the shipped `scrml:data` value-canonical algebra (ONE algebra) rebuilt via `_scrml_map_from_entries`.
- **compiler/src/codegen/{emit-each, emit-client, emit-event-wiring, emit-functions, emit-logic, emit-reactive-wiring}.ts + scheduling.ts** — `setVarNames` threaded through the emit-* surface alongside `mapVarNames` (a set IS a map; every set cell is also in `mapVarNames`).
- **No parser change (D2):** native `collectTypeAnnotation` captures `set[K]` generically (empirically verified; no ast-builder change, no `.scrml` mirror). **Adjacent root-cause fix:** `deepEqualExprNode` gained `case "map-lit"` (was `default: return false`, which rejected ANY `[:]`/`[k:v]` corpus init in the examples idempotency invariant — latent until a map-lit init existed).
- Worked example `examples/34-value-native-set.scrml` (NEW; freight-lane certification). +4 NEW test files (unit `expr-node-round-trip` + unit `value-native-set-type-system` + unit `value-native-set-codegen-collector` + integration `value-native-set-e2e`). ZERO new §34 codes (set reuses E-MAP-BRACKET-WRITE). SPEC §59.12 Nominal→Implemented + §59 banner + SPEC-INDEX §59 currency; §14.11 tuple forward-ref cleared (NO-TUPLE ruled S222).

**structure.map watermark advanced `852d6aa7`→`12600217` (S222). find-count **1090** at `12600217` (verified `find compiler/tests -name '*.test.js' | wc -l` = 1090; was 1071 at `852d6aa7`).**

## Key S223 Source Changes — ss38 HAMT map + ss32/ss41 auto-await hardening + ss33 runtime-minimality + ss34 endpoint diagnostic + ss39 parser fixes + §26.8 @apply (since watermark 12600217 → 6988c426)

+4 NEW test files (unit × 2, integration × 2). find-count **1094** at `6988c426` (verified `find compiler/tests -name '*.test.js' | wc -l` = 1094). SPEC §26.8 `@apply` ratified (Nominal — W2 implementation forthcoming). +1 NEW §34 code (E-ENDPOINT-MULTI-STATEMENT-ARM). ss33/ss38/ss39/ss41 carry ZERO new §34 codes.

### ss38 — HAMT persistent structural-sharing swap for §59 value-native map runtime (`5fb41cb9`)

- **compiler/src/runtime-template.js** (4870L) — MAP runtime implementation swapped: COW (copy-on-write flat-copy of the full entries object on every mutation) → **persistent HAMT** (hash-array-mapped trie with structural sharing). Observable §59 API unchanged. Internal map shape: `{ __scrml_map:true, _root: <HAMT-node>, _count: <int>, _seq: <int>, ordered: bool }`. The `entries` compatibility view is now a LAZY MEMOIZED property (`_scrml_map_define_entries`). NEW internal helpers: `_scrml_hamt_find`, `_scrml_hamt_put`, `_scrml_hamt_remove`, `_scrml_hamt_put_collision`, `_scrml_hamt_merge_leaves`, `_scrml_hamt_collect`, `_scrml_map_new`, `_scrml_map_define_entries`. Differential correctness proof: `value-native-map-hamt-differential-ss38.test.js` (NEW unit, all §59 ops matched against prior COW baseline).

### ss32 + ss41 — auto-await reactive-server IIFE error hardening (`a0559651` + `e02758e9`)

- **compiler/src/codegen/emit-client.ts** (2894L) — **ss32:** auto-await wrapper `(async () => _scrml_reactive_set(name, await fn(...)))()` now appends `.catch(_scrml_async_err => _scrml_error_boundary_log(name, _scrml_async_err))` on the IIFE CALL (valid in statement + expression position; the `_scrml_error_boundary_log` was a dead letter pre-fix). **ss41:** the `!{}` error-arm dispatch for a reactive-server auto-await IIFE is now emitted INSIDE the async IIFE (after the `await`) rather than OUTSIDE the IIFE call (closes the dead-promise-check class). `emit-client.ts:2104/2142/2158-2172`.

### ss33 — runtime-minimality + configurable server idleTimeout (`6988c426`)

- **compiler/src/codegen/emit-client.ts** (2894L) — `detectRuntimeChunks` / read-prune pass now scans the runtime-spliced server body (`ss33 item 2: g-client-read-prune-chunk-dependent`). Helpers referenced only in the spliced block were invisible to the prior scan → incorrectly tree-shaken. `emit-client.ts:2391-2401`.
- **compiler/src/commands/build.js** — `--idle-timeout <n>` flag (parsed at :105; default 120 s). `generateServerEntry(serverModules, mcpOpts, idleTimeout)` signature extended; emits `idleTimeout: N` at :365. `parseArgs` returns `idleTimeout` field at :128.
- **compiler/src/commands/dev.js** — `--idle-timeout <n>` flag (parsed analogously; raises Bun.serve `idleTimeout` on the dev server for MCP/SSE long-running sessions).

### ss34 — E-ENDPOINT-MULTI-STATEMENT-ARM diagnostic (§61.10) (`f76f9fd0`)

- **compiler/src/codegen/validate-emit.ts** (168L, **NEW FILE**) — exports `isSingleJsExpression(src: string): boolean` (line 141). Parses `src` via the §2.2.1 Acorn gate; returns true iff the whole string is exactly one JS expression. Also exports `E-ENDPOINT-MULTI-STATEMENT-ARM` descriptor (line 130). Imported by `emit-server.ts`.
- **compiler/src/codegen/emit-server.ts** (3411L) — `import { isSingleJsExpression } from "./validate-emit.ts"` (:20). In `emitEndpointArmEnvelope` (:2410): `if (!isSingleJsExpression(expr))` → fires `E-ENDPOINT-MULTI-STATEMENT-ARM` and returns a comment-stub `// §61.10 multi-statement bare body not lowered` instead of letting the malformed `await (…)` reach the generic E-CODEGEN-INVALID-JS gate.

### ss39 — nested-template interpolation scanner + match-arm comment trivia (`04db09de`)

- **compiler/src/tokenizer.ts** (2224L) — NEW `scanInterpBody(): string` (line 1260) + `scanTemplateBody(): string` (line 1309). Mutually recursive brace-balanced scanner for `${...}` interpolation bodies. Called at line 1332 (`scanTemplateBody` calls `scanInterpBody` on `${`; `scanInterpBody` calls `scanTemplateBody` on nested `` ` ``). Prevents premature interpolation close on nested template literals (g-nested-template-raw-mangle CLOSED).
- **compiler/src/match-statechild-parser.ts** (741L) — `//` comment trivia gate at line 69 + :535. A `//` end-of-line comment in a match-arm body is now stepped over as opaque — angle-bracket sequences inside comment text no longer trigger false arm-boundary or tag-close events (g-markup-comment-angle-bracket-parsed-as-tag CLOSED).

### §26.8 `@apply` Utility Composition — SPEC ratified Nominal (S223 W1; `727ad220`)

- **compiler/SPEC.md** — NEW §26.8 `@apply — Utility Composition in Author CSS (Nominal — S223)` (lines 16513-16572). Specifies the directive syntax, expansion rules (composing-family `var()` model carries through), v1 scope (flat single-rule utilities; variant-prefixed / multi-rule / pseudo-element forms reject), and three §34 codes named-ahead-of-implementation: `E-APPLY-UNKNOWN-UTILITY` / `E-APPLY-VARIANT-UNSUPPORTED` / `E-APPLY-NON-INLINABLE-UTILITY`. Per Rule 4 these codes land WITH the W2 implementation — NOT wired as of S223. Cross-ref: §34 now includes `E-ENDPOINT-MULTI-STATEMENT-ARM` (wired ss34).
- **compiler/SPEC.md** — §34 row `E-ENDPOINT-MULTI-STATEMENT-ARM` added (line 17683 + 33422/33429). The §61 `<endpoint>` W4 codegen gap is now loud + named — `f76f9fd0`.

**structure.map watermark advanced `12600217`→`6988c426` (S223). find-count **1094** at `6988c426` (verified `find compiler/tests -name '*.test.js' | wc -l` = 1094; was 1090 at `12600217`).**

## Key S224-S226 Source Changes — §26.8 @apply W2 + ss43 multi-scrutinee + ss49/ss50/ss52 fixes (since watermark 6988c426 → cf1471dd)

### S224 — §26.8 `@apply` Utility Composition W2 (`9ad78593`)
- **compiler/src/codegen/emit-css.ts** (+165L, now 343L) — `renderApplyGroupedDeclarations(declarations, errors?)`: resolves each `@apply` token via the Tailwind registry, classifies variant-prefixed / non-inlinable / unknown tokens (E-APPLY-VARIANT-UNSUPPORTED / E-APPLY-NON-INLINABLE-UTILITY / E-APPLY-UNKNOWN-UTILITY), and inlines the flat CSS declarations with last-wins `--tw-*` dedup. NEW helpers: `splitCssDeclarations`, `dedupeLastWins`, `renderDeclValue`. `generateCss` updated to call `renderApplyGroupedDeclarations` when a rule body contains any `@apply` node (fast-path skips rules with no `@apply`).
- **compiler/src/tailwind-classes.js** (+80L) — `@apply` utility-lookup integration: `lookupTailwindUtility(token)` returning flat declarations or `null` for non-inlinable / unknown; called from `renderApplyGroupedDeclarations`.

### S224 — ss42 Named-Machine Undeclared-Read (`7d009fb`)
- **compiler/src/type-system.ts** — `hadNameAttr` gate in the E-STATE-UNDECLARED walker: scopes the whole-body arrow-rule reject to the no-`name=` state-engine form, allowing named-machine `@cell` reads in non-handler positions to surface the correct E-STATE-UNDECLARED diagnostic.

### S225 — ss43 §18.19 Multi-Scrutinee Match W2 (`41438514`)
- **compiler/src/ast-builder.js** (+347L, now 17586L) — `detectMultiScrutineeHead(headerText)` (detects `( e1, …, eN )` all-wrapped head), `splitTopLevelCommaList`, `matchPositionIsPatternShaped`, `joinPatternTokens`, `scanProductPatternArm` (N-position product arm parser). Stamps `scrutinees[]` + `scrutineeExprs[]` on `MatchExprNode` for block-form; `subjects[]` on expression-form `MatchExpr`.
- **compiler/src/type-system.ts** (+371L, now 23666L) — `checkMultiScrutineeMatch(node, errors)`: arity validation (E-MATCH-SCRUTINEE-ARITY, NEW), nested-pattern exclusion per position (E-SYNTAX-012, first live fire site), product exhaustiveness via the existing `checkEnumExhaustiveness` (reuses E-TYPE-006/E-TYPE-020 with `(X × Y)` label). `partial match` skips exhaustiveness.
- **compiler/src/codegen/emit-expr.ts** (+215L, now 2578L) — `emitMultiScrutineeMatch(node, scrutinees, ctx, mode)`: desugars to nested single-scrutinee dispatch (outermost scrutinee is the outer `switch`, remaining scrutinees are inner matches). `emitMultiArmBody(resultStr, prelude, ctx)`: arm-body emitter for a product arm. `mapCellBareName(objNode, ctx)` + `setCellBareName(objNode, ctx)`: helper extractors for ss52 non-reactive local map/set. `emitUnary` fix: when `node.argument.kind === "binary" && op === "**"`, wraps the unary in parens unconditionally (every prefix unary before `**` is a SyntaxError in JS).
- **compiler/src/types/ast.ts** (+16L) — `MatchExprNode.scrutinees?: string[]` + `scrutineeExprs?: ExprNode[]`; `MatchExpr.subjects?: ExprNode[]` (all §18.19 additions).

### S225 — ss49 fixes (`3957f38e`, `bf7da16d`)
- **compiler/src/ast-builder.js** — concise-arrow SQL detection: when the last collected token in `collectExpr` is the arrow operator `=>`, a sibling BLOCK_REF (SQL block) is recognized as being in the concise body of that arrow — the enclosing function is escalated to server (`bf7da16d`). `@-led endpoint arm trailing value-expr` fix: an `@`-led endpoint arm that had a trailing value-expression was silently dropping the expression; the `collectExpr` read now continues past the `@` prefix (`3957f38e`).

### S225 — ss52 Non-Reactive Local Map/Set Method Lowering (`5ebdbce3`)
- **compiler/src/codegen/reactive-deps.ts** (+315L, now 1736L) — `buildFnReturnMapKinds(fileAST)`: builds a per-function-name registry of `"map" | "set"` return-kind from type annotations and init-expr shapes. `collectLocalMapSetNames(fnBody, declaredNames, mapKinds)`: scope-aware collector of every NON-REACTIVE local map/set name (keyed on a separate `localMapVarNames`/`localSetVarNames` set from the reactive `@cell` maps). `collectAllLocalMapSetNames(fileAST)`: file-wide union per function.
- **compiler/src/codegen/emit-control-flow.ts** (+147L, now 2224L) — `emitWhileStmt` + `emitDoWhileStmt` updated to thread `localMapVarNames`/`localSetVarNames`/`localOrderedMapVarNames` into `emitLogicBody` opts (previously these were missing from while/do-while bodies, causing local map/set method calls inside a while loop to mis-lower via the reactive `@cell` path).
- **compiler/src/codegen/emit-expr.ts** — `mapCellBareName` + `setCellBareName` helpers: detect a non-reactive local map/set receiver by checking the `localMapVarNames`/`localSetVarNames` sets on `EmitExprContext`; local receivers lower without the reactive write wrapper.

### S226 — ss50 Concise-Arrow SQL Detection + Unary-`**` Paren (`1eb8ada5`)
- **compiler/src/codegen/detect-sql-in-arrow.ts** (NEW, 177L) — `detectSqlInArrow(fileAST, filePath, errors)`: post-AST walk for E-SQL-009 on concise-body or curried arrow shapes where the emit-server.ts site misses. Case A (direct decl/return): detects a dangling `( x ) =>` escape-hatch whose last token is the arrow operator. Case B (curried/in-callback): scans `.init` / `.expr` text for `?{` + `}` pair. Wired at `api.js` Stage 2 (TAB post-pass), runs for EVERY file. Disjoint from emit-server site via the `conciseArrowBodyHasSql` `=>`-not-followed-by-`{` gate.
- **compiler/src/expression-parser.ts** (+91L) — `conciseArrowBodyHasSql` predicate: `true` when a source fragment contains `=>` NOT followed by `{` AND a `?{` SQL opener (the concise-arrow SQL signature).
- **compiler/src/codegen/emit-expr.ts** — `emitUnary` fix: when `node.argument.kind === "binary"` and `op === "**"`, wraps the whole unary expression with parens (`-(2 ** 3)` not `-2 ** 3`). Unconditional on all prefix unary operators (all are SyntaxErrors in front of `**` per the JS spec).

**structure.map watermark advanced `6988c426`→`cf1471dd` (S224-S226). find-count **1100** at `cf1471dd` (verified `find compiler/tests -name '*.test.js' | wc -l` = 1100; was 1094 at `6988c426`).**

## Tags
#scrmlts #map #structure #s226 #s225 #s224 #s223 #s222 #s221 #s220 #s218 #value-native-set #section-59-12 #set-bracket-k #tset #map-type-set-flag #collect-set-var-names #is-set-type-annotation #file-has-set-algebra-usage #set-var-names #set-vocab-interception #scrml-map-insert #scrml-map-keys #set-algebra-delegate #examples-34-value-native-set #deep-equal-expr-map-lit #expr-node-round-trip #ss31 #expr-validity #g-unary-left-of-exponent-no-paren #g-enum-toenum-client-structured-decl #g-double-unary-minus-emit-decrement #g-unary-of-exponent-arg-no-paren #e-codegen-invalid-js #no-tuple #ss38 #hamt #persistent-hamt #structural-sharing #_scrml_hamt_find #_scrml_hamt_put #_scrml_hamt_remove #_scrml_hamt_collect #_scrml_map_new #_scrml_map_define_entries #_root #_count #_seq #fbip-increment-1 #s94-blowup-fix #cow-to-hamt #runtime-template-hamt #value-native-map-hamt #ss32 #ss41 #auto-await-catch #_scrml_error_boundary_log #reactive-server-iife #dead-promise-check #error-arm-inside-iife #g-auto-await-reactive-server-no-error-arm #g-auto-await-error-arm-dead-promise-check #ss33 #read-prune-spliced-body #detect-runtime-chunks #idle-timeout #generate-server-entry #bun-serve-idle-timeout #g-client-read-prune-chunk-dependent #runtime-minimality #ss34 #validate-emit #is-single-js-expression #e-endpoint-multi-statement-arm #emit-endpoint-arm-envelope #section-61-10 #multi-statement-bare-body #named-diagnostic #g-endpoint-multi-statement-arm-diagnostic #ss39 #scan-interp-body #scan-template-body #nested-template-literal #brace-balanced-scanner #g-nested-template-raw-mangle #match-arm-comment-trivia #comment-angle-bracket-opacity #g-markup-comment-angle-bracket-parsed-as-tag #section-26-8 #apply-utility-composition #e-apply-unknown-utility #e-apply-variant-unsupported #e-apply-non-inlinable-utility #nominal-spec-ahead #bug-1-tailwind-apply #@apply #dpa-003 #inline-foreign-code #foreign-block #foreignblock #foreignnode #_-opaque-brace #iife-injection #async-iife #scan-foreign-slice-shape #opaque-scan #foreign-server-escalation #resource-type-foreign-inline #lang-gate #e-foreign-003 #e-foreign-004 #e-foreign-005 #check-foreign-blocks #program-lang #value-returning-foreign #crossing-header #in-colon-header #blockref-types-foreign #match-foreign-opener #push-foreign-context #foreign-level #level-aware-closer #bs-foreign-gate #out-typing-asis #foreign-asis-default #giti-032 #markup-as-value-arm-body #is-html-fragment-overfire #contains-markup-tokens #scrml-render-value-arm #6nz-bug-ai #each-empty-reconcile #replace-children-empty-edge #_scrml_reconcile_list-fix #old-nodes-size-zero #emit-logic-case-foreign #compiler #s216 #bug-51 #e-cg-016 #enum-server-bundle #generate-value-only-server-js #top-level-binding-exists #local-server-import-name-used #inject-after-header #ss1 #pattern-c-load #server-cell-load #server-var-decl-load-kind #emit-decl-rhs-sql-load #sql-load #param-bearing #section-52-6-5 #w-auth-004 #family-a #emit-bind-directive-body #bind-directive-body-opts #arm-body-bind-directive #scrml-route-server-load #cli #bun #engine-graph #source-map #each #each-in-dynamic-context #match #engine-statechild #cross-file-modules #enum-subset #message-dispatch #s154 #s155 #s156 #s157 #s158 #s159 #s160 #bug60 #bug62 #bug63 #bug64 #bug65 #bug70 #bug71 #bug72 #bug73 #r28-1c #r28-8 #per-item-reactivity #live-keyed #colon-shorthand-html #colon-shorthand-canonical #shape4-no-rhs #bare-variant-inference #native-parser #native-parser-swap #each-promotion #match-promotion #flip-failure-families #f1-engine-substrate-closed #engine-substrate-fix #b1-reset-expr #b2-message-arm-closed #native-exprnode-walker #f2-match #promote-each #typed-atcell #server-fn-star #bare-function-failable #cross-file-export-bodystart #flip-605 #flip-508 #deepset-write-loss #reactive-nested-assign #reactive-array-mutation #s161 #s162 #s163 #s164 #s165 #s166 #s167 #s168 #s169 #value-native-maps #map-type #cycles-prereq #set-algebra #scrml-data #bug-b-structural-compound-deepset #stamp-compound-deepset-targets #on-mount-dismount #const-at-derived-decl #f5-closed #deepset-node-synth #exprtext-backfill-walker #blockstub-verbatim-body #mario-match-arm-fix #s170 #s173 #e-export-001 #w-type-fn-field #s174 #log-builtin #log-loc #location-transparent-logging #w-log-shadowed #e-type-any-forbidden #no-any-hard-line #production-strip #production-flag #s175 #typed-sql-row #sql-projection #width-subtyping #e-sql-row-contract-mismatch #w-sql-row-untyped #e-struct-function-field #function-boundary #passed-vs-stored #f-schema-001 #fn-return-inference #flagship-typed-data #s176 #e-type-unknown-name #unrecognized-type-name #w-pure-deprecated #pure-deprecation #migration-3 #scrml-math #scrml-random #scrml-time-now #capability-scoped #non-deterministic #e-fn-004 #transitive-shim-copy #s177 #g-formfor #markup-expansion-in-arms #arm-body-children #r27-c6 #r28-7b #bug-4 #bug-48 #bug-74 #e-closer-001 #schemafor-predicated-base #s169-map-inline-insert #inline-sibling-shim-imports #client-stdlib-inliner #math-de-leak #s179 #e-route-003 #e-route-004 #wire-serializability #i-fn-promotable-inferred-server #e-fn-001 #s180 #server-keyword-eliminate #w-deprecated-server-modifier #migration-4 #trigger-7 #trigger-8 #channel-broadcast #handle-middleware #inferred-boundary #sse-route #emit-client-inferred #mcp-descriptors-inferred #s181 #w-display-text-overquote #display-text-overquote #inverse-footgun #e-unquoted-display-text #check-display-text-overquote #server-keyword-reword #e-cg-006 #w-lint-019 #s182 #e-engine-effect-not-interpolated #engine-effect #effect-interpolated #engine-var-dedup #e-engine-003 #s183 #e-formfor-not-imported #e-tablefor-not-imported #scan-for-unimported-type-data-element #formfor-unimported #tablefor-unimported #w-tailwind-001 #w-tailwind-unrecognized-class #find-interpolation-ranges #token-touches-interpolation #dynamic-class-prefix #g-tailwind-dynamic-class-prefix #fn-pure-canonicity #i-fn-promotable-reword #s184 #lifecycle-field-comment-leak #collect-braced-body-comment-skip #e-struct-function-field-fp #e-type-001-double-fire #statement-text-normalize #w-lint-007-ghost-fp #shape-1-variant-lifecycle-initializer #infer-enum-from-variant-lifecycle-annotation #e-variant-ambiguous #snippet-fill-exemption #braced-body-opens-paren-arrow-lambda #w-lint-004 #w-lint-021 #payload-binding-gaps #extract-match-arm-payload-bindings-by-variant #match-block-arm-payload-scope #error-arm-multi-field #table-for-synth-skip #w-each-promotable #s186 #channel-codegen #g-channel-reconnect-bare-int #g-channel-handler-wiring #onserver #onclient #channel-ws-handler #is-channel-ws-handler #collect-channel-attr-handler-names #channel-attr-to-call #reconnect-bare-int #e-scope-001-exempt #s185 #errarm-refail #fail-from-arm #re-fail-from-arm #parse-fail-expr-string #emit-fail-expr #match-arm-fail-expr #ns1-recurse-failexpr #e-validator-inline-colon #validator-inline-msg-paren #try-recover-colon-inline-message #colon-form-reject #s19-5-2 #s188 #g-not-negation-enforce #e-type-045 #prefix-not-negation #not-prefix-detector #harvest-not-prefix-negation #lowering-choke-point #cluster-A #e-attr-unquoted-operator #attr-condition-operator-ahead #is-condition-attr-name #is-prefix-not-operand-ahead #attr-op-reject #unquoted-condition-atomic-only #g-division-in-ternary-arm #ternary-depth-guard #typed-reactive-boundary #s189 #given-rebind-reject #e-syntax-045 #given-narrows-in-place #channel-cell-write-client-side #ruling-a #e-channel-server-cell-read #detect-server-context-channel-cell-read #trigger-7a-drop #channel-cell-client-held #for-each-ident-in-expr-node #schemafor-create-table-pa #extract-schemafor-create-table-statements #pa-pluralize-struct-name #split-struct-fields-top-level #pre-bind-reactive-state-cells #reactive-cell-hoist #section-6-9 #annotation-ordering #s190 #cluster-c #e-decl-rhs-interp-wrapped #markup-root-closed #markup-const-sibling-swallow #g-derived-rhs-interp-wrapped #g-markup-const-consumes-cell-decl #derived-engine-expression-form #derived-expr-node #derived-expr-kind #inline-match-derived #expr-derived #legacy-source-var #b16-no-rules #b16-no-initial #b16-no-write #b16-circular #derived-upstream-enum #e-engine-004-steer #c14-derived-substrate #e-derived-engine-initial-undefined-rt #dependency-graph-derived-edges #section-51-0-j #section-51-9 #section-52-1 #s191 #tailwind-composing-families #approach-c #inline-var-fallback #no-preflight-block #register-ring #register-gradient #register-transform #register-filters #register-backdrop #box-shadow-compose #transform-compose #filter-compose #backdrop-compose #ring-shadow-setter #section-26-7 #section-26-7-1 #section-26-7-2 #section-26-7-3 #if-fn-condition #call-ref-conditional #g-attr-if-fn-call-misroute #g-attr-if-fn-display-not-mount #g-attr-if-fn-chain-head-call-misroute #update-chain-call-ref #reactive-conditional-not-event #section-5-1 #s194 #server-authority #tier-1-read-authority #tier-2-server-var #auto-persist-retracted #q1-c #q2-wf #emit-server-authority-load #server-load-route #select-star-load #collect-server-authority-types #try-parse-server-authority-decl #authority-server-gate #server-authority-instance #server-authority-table #w-auth-001-suppressed #w-auth-002-ssr-residual #ssr-pre-render-residual #section-52-6 #section-52-6-2 #section-52-6-6 #section-52-6-7 #spec-issue-026 #server-write-fn-convention #channel-fan-out-composition #broadcast-explicit-composition #emit-sync-stub-deleted #optimistic-update-deleted #read-authority-only #s195 #gap-a #void-element-self-terminating #match-arm-void #each-arm-void #find-structural-body-end #find-arm-closer #void-elements-set #arm-closer-scanner #flush-closer-fix #e-ctx-001-unclosed-match #e-match-parse-001 #s196 #render-expression #render-of #render-element #render-held-accessor #all-variant-render-exprs #emit-boundary-markup-expr #e-render-no-of #e-render-no-clause #e-render-not-enum #e-match-arm-markup-in-value #held-error-display #g-held-error-display-closed #errors-as-states #section-19-15 #section-4-15 #section-24-4 #section-18-0 #structural-element #data-scrml-render-anchor #limit-the-primitive #s198 #s199 #engine-hydration #initial-cell #server-source #scrml-engine-hydrate-init #s200 #g-each-component-body-invalid-js #g-each-peritem-if-predicate #helper-export-hoist #transitive-helper-import #nested-expression-prop #lower-each-expr #s201 #markup-as-value #markup-value-expr #pillar-1 #section-1-4 #section-7-4 #emit-markup-value-expr #markup-to-dom-node-iife #scrml-render-value #node-aware-display #ternary-markup-arm #return-markup #parse-expr-with-markup-values #saw-ternary-at-root #scan-shape12-decl-end #value-terminator-discriminator #markup-typed-derived #derived-chunk-treeshake #g-markup-value-ternary-fnreturn-codegen #g-each-body-bare-variant-arg #bare-variant-call-arg #serialize-call-args-lowered #g-nested-component-member-arg-misparse #normalize-tokenized-raw #member-access-collapse #s202 #g-each-over-arm-payload-binding-unbound #arm-payload-binding #stamp-arm-payload-eaches #g-each-inline-component-prop-member-unsubstituted #g-inlined-component-root-class-interp-raw #substitute-interp-segments #class-merge-post-substitution #layer-2-string-literal-attr #e2e-render-map #known-failure-map #render-detectors #d0-d7 #render-harness #oracle-free #tier-tag-corpus #flograph #project-graph #typed-edge #provenance-sweep #trucking-board-flagship #board-each-conversion #derived-filtered-cells #board-high-0 #s204 #e-control-flow-in-markup #bare-control-flow-in-markup #bare-control-flow-markup-re #lift-bare-declarations #section-17-4 #section-7 #reject-recover #sibling-e-unquoted-display-text #s111 #g-raw-interp-channel-meta-corners #s205 #g-emit-string-tree-paren-drop #emit-string-from-tree #bin-prec #is-prec #precedence-aware-parens #idempotent-serializer #emit-string-tree-precedence #trucking-slice-3 #each-sweep #g-each-peritem-attr-ternary-quoted-arms #within-node-rebaseline #g-match-alternation-value-vs-derived #variant-pattern-alternation #arm-scanner-alternation #parse-arm-pattern #exhaustiveness-alias-gap #match-alternation-value-vs-derived #section-4-10 #section-18-2 #or-chain-codegen #s206 #g-colon-shorthand-markup-misparse #colon-shorthand-markup-body #angle-depth #compound-lift-exempt #engine-exempt #machine-exempt #scan-attributes-angle-depth #section-4-13 #section-4-14 #g-engine-autodecl-bare-variant-write #return-stmt-comparison-prepass #infer-bare-variants-at-comparison-sites #section-14-10 #section-7-6-1 #comparison-in-return #trucking-slice-2 #decl-coupled-validators #section-55 #validity-surface #shape-2-variant-c #g-compound-field-render-by-tag-unexpanded #s206-block-analysis-emit #block-analysis-footprint #block-analysis #footprint-for-block #block-footprint #dotted-path-footprint #block-analysis-sidecar #block-lease #break-1 #shallow-footprint #honest-empty #build-block-analysis #build-block-analysis-for-file #serialize-block-analysis #build-block-analysis-json #block-analysis-block #block-analysis-artifact #source-order-blocks #end-line-derivation #rel-path-anchor #block-kind #collect-function-decls #logic-body-walk #dock-lease-anchor #s207 #block-analysis-emit-d3 #emit-block-analysis-flag #block-analyses-result #compile-js-write-loop #identity-match-by-relpath #g-each-peritem-attr-ternary-quoted-arms #interpolation-depth-aware #interp-depth #interp-string-ch #tokenize-attributes-fix #depth-0-terminator #block-analysis-emit-d5 #endline-collapse-fix #source-from-file #underscore-source-text #off-by-one-endline #span-end-minus-1 #true-extents #g-compound-field-render-by-tag-unexpanded-resolved #compound-member-rbt #non-lexical-rbt #lookup-compound-members-by-leaf-name #compound-member-to-parent #e-cell-ambiguous-member-render #sibling-form-rbt #shape-2-compound-field #render-by-tag-compound-member #s207-g-compound #s209 #ss2 #engine-codegen-statechild #engine-statechild-grammar #engine-state-child-reserved-attrs #state-child-structural-tags #w-engine-server-deferred #server-flag-bare #engine-server-deferred #boot-effect-write-validation #e-engine-invalid-transition #opener-effect-c1 #fire-site-11 #engine-shorthand-body-render #section-51-0-i #iscolon-shorthand #display-text-literal-inner #native-parse-file #block-analysis-d6 #import-inlined-channel-fns #span-file-origin #phantom-block #ownerfile-guard #collect-function-decls-local #b17-activated #a2-api-decl #api-decl-parser #e-api-base-missing #e-api-method-invalid #e-api-response-type-undeclared #e-api-endpoint-malformed #section-60-9 #ss2-derived-engine-crash #derived-engine-autoderive-crash #ss3-paren-group #paren-grouping-preservation #paren-receiver #g-paren-binary-group-dropped #each-sigil-structuring #at-dot-sigil #at-dot-expr-node #ss8-tailwind #ring-offset-arbitrary #tw-arbitrary-string #is-string-arbitrary-value #getTwArbitraryValue #section-26-4-arbitrary #ring-offset-len #s211 #ss11 #w-interp-in-raw-content #raw-content-lint #lint-w-interp-in-raw-content #pre-element-raw-body #code-element-raw-body #section-4-17 #detect-token #walk-raw-content #bs-lint-stage #a2-w3 #api-decl-typer #check-api-declarations #e-api-path-param-unbound #e-api-endpoint-unknown #e-api-req-shape-mismatch #section-60-2 #section-60-4 #section-60-6 #request-type-resolve #api-endpoint-registry #ss7-reflect-variants #reflect-variants-bare-string #s212 #g-tailwind-not-scanned-in-match-arms #each-block-body-children #match-block-body-children #match-block-arm-body-children #g-nested-each-no-own-subscription #nested-each-reactive-subscription #scrml-effect-inner-each #approach-c-inner-each #tier-0-nested-each #emit-tier0-nested-each #g-lift-concurrent-transitive-exclusion-tdz #transitive-dep-closure #dep-closure #collect-reassigned-names #decl-is-reassigned-later #reassigned-let-gate #const-destructure-tdz #lift-concurrent-scheduler #scheduling-ts #body-dg-builder #collect-expr-facts #collect-lambda-body-reads #lambda-body-free-reads #lin-scope-boundary #lambda-expr #for-each-ident-in-expr-node-stops-at-lambda #conservative-over-approximate #zero-new-codes #a2-w4 #w-api-response-not-variant #api-endpoint-for-emit #build-api-endpoint-registry #emit-api-url-expr #emit-request-node-api-mode #seq-guard #refetch-fn #mounted-cleanup #parse-variant-decode-iife #emit-parse-variant-decode-iife #response-enum-annotation #check-api-declarations-w4 #section-60-5 #section-60-3 #section-60-6 #section-60-7 #g-bare-ref-event-handler #bare-ref-event-wiring #on-star-bare-ident #bare-ref-handler-flag #fn-name-map-resolve #direct-listener-wire #g-match-arm-reactive-attr-effects #s217 #giti-029 #giti-030 #giti-031 #g-onmount #_onmounteffect #_onlifecyclelift #toplevel-on-lifecycle-re #comment-flushed-on-mount #on-mount-unconditional-effect #subscribe-sub-path #subpath-dispatch #on-cell-subfield #raw-content-verbatim #pre-code-verbatim #raw-content-element-names #inside-raw-content #escalation-2 #generate-route-name #author-route-path #esm-syntax-error-fix #ri-route-binding #explicit-route #within-node-classifier-rebaseline #strip-keys-rebaseline #arm-tagged-class-directive #arm-tagged-attr-template #lower-class-directive-condition #lower-attr-template-value #emit-bindings-shared-helpers #registry-current-arm-context #directive-selector #directive-js-expr #directive-refs #wireable-directives #disposers-push-effect #arm-root-query #collect-ts-88-children-only
