...
compiler/tests/  — **1039 .test.js files total at `1ff06eae`** (verified `find compiler/tests -name '*.test.js' | wc -l` = 1039; +3 S214: render-shadowing-ss16-c3.test.js integration + eq-payload-variant-lint-ss16-c4.test.js unit + g-on-mount-bare-call-render-slot.test.js unit; also EXTENDED: bug-1-tailwind-unrecognized-class.test.js + emit-html-meta.test.js + bare-variant-nested-context-inference.test.js NEW + parser-conformance-within-node-allowlist.json); was **1036 at `fec0a054`** (verified `find compiler/tests -name '*.test.js' | wc -l` = 1024; +4 S210) (verified `find` count) across all categories (S173: +2; S174: log()/any-reject; S175: +5 typed-SQL-row; S176: +6 unknown-type-name/scalar-vocabulary; S177: +8; S179: +2; S180: +5; **S181: +2 [display-text-overquote.test.js, server-keyword-error-msg-canon.test.js]**)
