...
**CSS — the §65 scrml-native model (V1.0 Wave-1 gate)** — deletes cascade specificity. `<theme>` (program-scope named-token block, lowers to §25 CSS custom properties) + `<defaults>` (app-wide bare-element defaults, the "base" @layer) are structural elements (NOT HTML; attribute-registry.js:474/492/505), Nominal/spec-ahead except the §65.2 conflict-checker itself which is LANDED. Resolution order: `style=` explicit value > co-located `#{}` scoped rule (conflict-checked) > `<defaults>` > native inheritance > built-in reset. Fixed `@layer reset, thirdparty, base, tokens, utilities, author` (§65.8) — Tailwind utilities sit BELOW author rules. See error.map.md for the E-STYLE-CONFLICT / W-STYLE-CONFLICT-POSSIBLE checker detail.
**Realtime — §38.13 `<channel watches=table>`** — a change-feed-over-external-DB-writes primitive, distinct from the general §38 WebSocket `<channel>`. Phase 1 (front-end recognition + `RowChange` synthesis, `channel-watches.ts`) + Phase 2 (Postgres trigger DDL install + the bundled-`pg` LISTEN bridge + client `__change` frame dispatch) are both landed. `<onchange>` is the client-side handler element (attribute-registry.js:474).
