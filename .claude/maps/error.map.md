...
withdrawn.** Mechanically re-derived at `d0763cff`: §34 carries **185 distinct code prefixes**
(`E-CG`, `W-AUTH`, `I-TENANT`, …); the family table below names **67**. The other **118 prefixes
...
0. **`.claude/maps/error.generated.md`** — the MECHANICAL index: code -> message snippet ->
   `file:line` emit sites, 353 entries. Faster and more precise than any prose table here for a code
   it covers. **Two hard limits:** it is **E-codes ONLY** (zero `W-*`, zero `I-*` — so it cannot
   answer a warning lookup at all), and it is stamped **2026-06-25**, i.e. ~5 weeks and ~50 commits
   stale, with drifted line numbers (it puts `E-PA-002` at `protect-analyzer.ts:790`; the emit is at
   `:828`). Treat its FILE attribution as reliable and its LINE numbers as a starting point.
   Regenerated by `flogence/scripts/mapgen.ts`, not by this repo's CI.
1. Read the **"Diagnostic families by feature area"** table below and take its `Fire site` column —
   for the 67 prefixes it covers (`E-PA-*` -> protect-analyzer.ts, `*-TAILWIND-*` ->
...
**800 distinct diagnostic codes** cataloged in §34 at `d0763cff` — re-extracted this pass with the
UTF-8-safe `awk -F'|'` field-split methodology, `comm` set-diff-verified against the 799 baseline at
`115e8b1b`. **+1, zero removed.** §34 spans `SPEC.md` :18734 (`## 34. Error Codes`) to :19577
(`## 35.`) at this HEAD.
...
| **`115e8b1b..d0763cff` (THIS pass)** | **799 -> 800** | **+1, zero removed: `W-AUTH-MIDDLEWARE-AUTO-INJECTED` (a SPLIT out of `W-AUTH-001`, not a new behaviour — the fire already existed)** |
...
5. **RETIRED NOTE — `W-NAV-CHUNK-LOAD-FAILED`.** Prior generations of this map said "NOT implemented
...
## THIS window (`115e8b1b` -> `d0763cff`) — 1 code split, 0 new behaviour, 1 silent placement change

### `W-AUTH-MIDDLEWARE-AUTO-INJECTED` (Warning, §40.1.1/§12.2, S299) — a CODE SPLIT, not a new fire
`compiler/src/route-inference.ts:5648`, Step 8b. Fires when a compilation unit declares `protect=`
fields but carries NO explicit `auth=` on `<program>` or any `<page>`: the compiler auto-injects
`auth="required"` / `csrf="auto"` / `sessionExpiry="1h"` / `sessionSecure=true` and tells the adopter
a gate was chosen for them. Silent when an explicit `auth=` exists at either level — an explicit
`"optional"`/`"none"` is honoured and never escalated (that override was the
`g-protect-overrides-page-auth` defect), and an explicit `"required"` registers the gate without the
warning.

**The defect that was fixed is a MAP-shaped one, so state it plainly.** This fire previously used
`W-AUTH-001`, which §34 and §52.11 define as an entirely unrelated guarantee — "`<var server>` has
no detectable initial load", fired from `type-system.ts`. One code, two meanings, two files, one of
them undocumented. Ruled the same way `E-IMPORT-007` was at S297: **allocate a fresh code rather
than renumber the documented one**, because additive is `inert` for existing source while
renumbering is `semantics-changed` on a code adopters may key on. `W-AUTH-001` keeps §52.11
unchanged. NAMED rather than numbered — `W-AUTH-003` is free but sits in a gap beneath a used
`W-AUTH-004`, and a skipped number is a retired-code hazard; the family already uses semantic names
(`W-AUTH-PAGE-INFERRED`, `W-AUTH-LOGIN-MISSING`, `W-AUTH-RUNTIME-FALLBACK`).

**Consumer impact — the count assertions moved.** `compiler/tests/unit/session-auth.test.js` (8
assertions) and `compiler/tests/integration/trucking-dispatch-smoke-integration.test.js` both key on
the code string. The trucking smoke test's `W-AUTH-001 = 20` baseline is now split across two codes;
read its header comment before re-baselining.

### §12.2 Trigger 3 — server-only stdlib import escalates (S299). NO code. A SILENT relocation.
`compiler/src/route-inference.ts`. A function that REFERENCES a binding imported from an escalation
server-only stdlib module is now escalated to the server. **It emits no diagnostic and it is not
supposed to** — §12.2 placement is inferred, not reported. Two consequences for anyone reading this
map to debug a diagnostic:
- **A function that "disappeared from the client bundle" with zero errors and zero warnings is the
  EXPECTED shape of this trigger**, not a bug. Confirm by checking whether the file imports
  `scrml:auth`/`crypto`/`cron`/`fs`/`process`/`redis`/`store`/`path`/`mcp`/`oauth`.
- The reason kind is deliberately the EXISTING `server-only-resource`, not a new
  `EscalationReason` variant, with `resourceType` carrying the module specifier. So
  `describeServerTrigger` renders "the server-only resource `scrml:auth`" with no change, and
  `W-DEPRECATED-SERVER-MODIFIER` now correctly reports an explicit `server` keyword as redundant for
  this class. A fresh variant would have broken `emit-server.ts isBodyOnlyEscalation`'s
  `.every(kind === "server-only-resource")` §12.6 gate and silently re-attached an HTTP wrapper.
See domain.map.md "§12.2 Trigger 3" and dependencies.map.md for the loci.

## Prior window (`c700c435` -> `115e8b1b`) — 4 new codes, 2 changed messages
...
**NEW runtime helper this window, and it is NOT error machinery:** `_scrml_ifrow_apply(cur, el, ph,
...
## Tags
#scrml #map #error #diagnostics #routing #semdiff #css65 #diagnostic-partition #result-warnings #lint-diagnostics #tab-span-lift #outlet #tenant-floor #ssr-auth-scoped #sql-lex #sql-table-refs #catalog-count-audit #catalog-vs-impl #w-lint-uncatalogued #dbauth #e-dbauth-sqlite #e-dbauth-no-tenant-column #w-dbauth-marker-nearmiss #w-schema-destructive-drop #db-migrate #rls #secdef #e-cg-018 #w-each-bind-item-field-deferred #e-schema-010 #e-schema-011 #w-schema-constraint-tightened #w-schema-constraint-drift-unapplied #w-nav-chunk-load-failed #navigate-wave1c #e-match-invalid-arm #ghost-pattern #w-dead-function #e-pa-002 #protect-analyzer #tailwind #w-tailwind-unrecognized-class #e-tailwind-001 #outline-family #w-server-import-unemitted #dist-space #d4 #on-mount #gh237 #gh234 #messages-chunk #w-auth-001-split #w-auth-middleware-auto-injected #code-split #trigger-3 #escalation-server-only #route-inference #prefix-coverage-audit #error-generated-index #not-a-diagnostic #w-lift-tier0 #ifrow-apply
