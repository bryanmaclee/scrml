...
776 distinct diagnostic codes cataloged: 642 `E-*` (Error), 127 `W-*` (Warning/Info-partitioned), 7 `I-*` (recomputed this pass directly from the §34 table by first-cell extraction). Up from 766 (636/124/6) at the f079d0a9 (S255) watermark — net +9. The map watermark was S255, not S263, so the bulk of the +9 accrued across S256-S263 §34 maintenance (the #83/#85 catalog cleanup: retire 6 dead codes + reserve 10 spec-ahead + repoint E-CTX-002; plus conformance-driven mints E-CTRL-010 / E-LIN-004/005 / E-ROUTE-005 …). The S264 window added NO new catalog rows — its three source PRs wired FIRE SITES to already-NAMED codes (E-SQL-003, E-SQL-004, E-MARKUP-001), moving them from reserved/named to firing. The S265 window (this pass) added +1 catalog row — E-THEME-TOKEN-UNKNOWN (§65 CSS Wave-1 emission, PR #95) — which is MINTED and FIRES in the same window (see "New fire sites this window (S265)" below). §34 is a lookup index only — each code's normative definition lives in the SPEC section that introduces it (cited in the table's Section column). Do not enumerate all codes here; grep `compiler/SPEC.md` for a specific `E-XXX`/`W-XXX` code, or read §34 directly for the full table.
...
| CSS (§65 native model) | E-STYLE-* / W-STYLE-* / E-THEME-* / E-DEFAULTS-* | 4 live (E-STYLE-001, E-STYLE-CONFLICT, W-STYLE-CONFLICT-POSSIBLE, E-THEME-TOKEN-UNKNOWN) | codegen/css-conflict-check.ts + api.js Stage 3.4 (§65.2 checker) + codegen/emit-theme-reset.ts (§65 Wave-1 emission, S265) |
...
## New fire sites this window (S265, 0a79d838 -> bf316828 — §65 CSS Wave-1 emission)
- **E-THEME-TOKEN-UNKNOWN** (§65.3.2 / §65.6, PR #95) — MINTED and FIRING in the same window (+1 §34 catalog row). Fires from the NEW `compiler/src/codegen/emit-theme-reset.ts` (12 sites): a `#{}` value's `@name` reference matching neither a declared `<theme>` token NOR a declared reactive/derived cell (a bare identifier is literal CSS and never errors); a `.Variant`/`@media` re-bind of a name absent from the global base token set also fires. emit-theme-reset.ts is the CSS Wave-1 EMISSION half (theme-token `@`-sigil lowering + built-in `@layer reset` + `:where()`-flat wrapping) that emit-css.ts's `generateCss` delegates to; the conflict-CHECKER (§65.2, css-conflict-check.ts) is unchanged. Separately (§25, PR #98) a reactive `#{}` CSS custom property now ALWAYS bridges onto `document.documentElement` (emit-reactive-wiring.ts).
