...
export interface Row { id: string; state: string; raw: string; }
 ...
 */
export function classify(statusCell: string, ruleCell: string): string {
 ...
 * ⚑ Split on /\r?\n/, NEVER on a bare "\n".
 *
 * On a CRLF checkout (every Windows clone with `core.autocrlf=true`) a plain `"\n"` split leaves a
 * trailing `\r` on every line. The row regex below ends `(.*)$`, and `.` never matches `\r`, so `$`
 * is unreachable and **0 of 43 rows match**. This probe then printed
 * `0 queued · ✓ nothing owed` while 43 items sat banked, 3 UNRUN and 4 ADVISORY awaited bryan
 * (S401, PA-verified by execution against BOTH encodings: CRLF 0 / LF 43).
 *
 * That is exactly the `pa-base` §8 hollow gate — a MANDATORY boot probe reading green while
 * measuring nothing — landing on the one instrument whose whole purpose is to make an invisible
 * obligation visible. LF-identical, so this is inert on Linux.
 *
 * Same fix and same reason as `state.ts:666` and `delta-lint.ts:82`; this is the THIRD instance of
 * the class in this directory, which is why the split lives in one named constant and the parse is
 * pinned by `compiler/tests/unit/dpa-debt-parse.test.js`.
 */
export const LINE = /\r?\n/;

/** The authoritative status TABLE — the surface the queue itself declares canonical. */
...
export function parseItems(text: string): Map<string, string> {
...
export function findDrift(rows: Row[], items: Map<string, string>): string[] {
  const drift: string[] = [];
  for (const r of rows) {
...
// ⚑ Gate the CLI on `import.meta.main` so the test can import the parse without running the probe
// (Bun sets it only for the entrypoint). Same shape as review-debt.ts:209 / corpus-zero-debt.ts:281.
if (import.meta.main) {
  ...
  const drift = findDrift(rows, items);
