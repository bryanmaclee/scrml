...
//
// ⚑ AND EITHER MAY CARRY A CONTRIBUTOR SUFFIX: `wrap(s401-peter):`, `wrap(S372-bryan):`.
// Both patterns below used to demand `)` IMMEDIATELY after the digits, so every
// suffixed wrap was invisible to this index. Measured over the last 600 commits at
// S410: **100 wrap subjects, 21 matched, 79 DROPPED** — 51 peter-suffixed and 28
// bryan-suffixed, i.e. it was never lane-specific, it hit whoever used the suffix.
// `sessionNumOf` had the identical defect and returned `null` for those subjects, so
// the per-session dedup below was dead for them too — the two halves failed together
// and neither could reveal the other.
//
// ⚑ MEASURE THIS WITH CARE IF YOU RE-DERIVE IT. The obvious probe for the dropped
// set, `wrap\(s[0-9]+[^)]`, is WRONG: `[0-9]+` backtracks, so it matches `wrap(s408)`
// by taking `40` and letting `[^)]` eat the `8`. It reports every wrap as dropped.
// The character after the digit run must be excluded as BOTH `)` and a digit.
// FOUR live subject shapes, measured over the last 600 commits on origin/main at S410 —
// not read off the regex, executed against real merge subjects:
//
//   wrap(sNNN): …            paren, plain            21
//   wrap(sNNN<sep>who): …    CONTRIBUTOR-suffixed    79   ← was dropped entirely
//   wrap/sNNN (#PR)          PR-FLOW branch title     3   ← was dropped entirely
//   docs(sNN): WRAP — …      S150–S167 era
//
// ⚑ The suffix SEPARATOR is not just `-`. Real subjects use `-`, `.`, a space and `·`
// (`wrap(S313.bryan)`, `wrap(S312 peter)`, `wrap(S310·peter)`), which is why this
// accepts any non-`)` run rather than an enumerated separator set. An earlier S410
// attempt allowed only `-` and still dropped 19 of the 79.
//
// ⚑ The SLASH form is not a collaborator convention that could be deprecated — it is
// what the project's own wrap procedure GENERATES: the overlay commits onto a
// `wrap/sNNN` branch and `gh pr create --fill` titles the PR from the branch, so the
// squash subject carries no parenthesis at all. A widening that handles only the paren
// family still drops every PR-flow wrap (S404 amendment says exactly this).
const WRAP_PAREN = /\bwrap\(s(\d+)[^)]*\)/i;
const WRAP_SLASH = /\bwrap\/s(\d+)\b/i;
// ⚑ The era form's `wrap` must not be a PREFIX of a longer token. `maps(S391): wrap-6c
// refresh …` satisfied the old `/\(s\d+\):\s*wrap\b/` — `\b` matches before the `-` —
// so a routine maps commit was admitted as a session anchor and EVICTED a real wrap
...
export function sessionNumOf(subj: string): string | null {
  const m = WRAP_PAREN.exec(subj) ?? WRAP_SLASH.exec(subj) ?? WRAP_ERA.exec(subj);
  return m ? m[1] : null;
}
/**
