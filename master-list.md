...
**Last updated:** 2026-05-11 (S80 — **auth/protect/csrf attribute-host codification SHIPPED · E-MW-001 retired · D3 csrf= drift resolved (collapse to "auto"|"off" per §52.13) · `<channel protect=>` → `<channel auth=>` renamed · `<program protect=>` shorthand retired · Bootstrap L3 library-mode meta-block strip-bug FIXED (paren-aware regex narrowing) · A5-7 canonical-sample family LANDED (engine-005 through engine-008 covering A5 features F1/F1a/F1b/F2/F3) · self-host ast.scrml parity sync · 6 commits / 0 regressions / full-suite 11,139 pass · 1 day**)

**Tests (current — S80 close, pre-commit subset):** **10,416 pass / 62 skip / 1 todo / 0 FAIL** (506 files; `bun test compiler/tests/{unit,integration,conformance} --bail` ~12s). **Full suite (S80 close, `bun run test` incl. browser/lsp/commands/self-host):** **11,139 pass / 73 skip / 1 todo / 0 FAIL** (534 files). +4 new compile-test samples picked up by `pretest` (engine-005 through engine-008). Pre-commit hook firing every commit (installed S79); cross-machine sync clean.
...
| **A8 — test-bind (effects-as-data middle path, Insight 22)** | `test-bind <serverFnName> = <handler>` declaration in `~{}` blocks; compile-time conditional at §47 server-fn call site; production binary unchanged (dead-code-eliminated) | **~6-12h** | ⏸️ pending | **S67 RATIFIED via Insight 22.** Closes OQ-8 partially (server-function mockability); re-files OQ-8b for `<onTransition>` body effects beyond server-fn calls. Position B (effect-record schemas) NOT ADOPTED at this time; structurally extensible later (no flip-condition gating per S67 methodology rule). E-TEST-004 + E-FN-004 + Insight 21 unchanged. Sub-steps: (1) parser `test-bind` declaration in `~{}`; (2) typer: scope-local table; key-validation against §47 encoded names; (3) codegen: compile-time conditional dispatch at server-fn call sites in test mode (release builds dead-code-eliminate); (4) tests including fail-fast on unbound server fn in active test-bind context; (5) optional `scrml:test` API alignment. **Post-S72 sequencing constraint (A9 integration):** A8 dispatches AFTER A9 Ext 4 (test-bound stubs inherit auto-`!` semantics consistently per body-split integration design dive 2026-05-08). |
