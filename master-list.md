...
**Last updated:** 2026-05-10 (S77 close — **A5 computed-delay family CLOSED (A5-4 + A5-5 + A5-5b SHIPPED) · A5-6 Feature 2 (`<onIdle>` watchdog) SHIPPED · 7 SHIPs + 2 chore + 1 SPEC docs commits + 1 scrml-support docs commit · +82 tests · 0 regressions · memory-leak deep-dive REFRESHED in scrml-support · W-LEAK-010 SPEC row landed**)

**Tests (current — S77 close):** **10,961 pass / 64 skip / 1 todo / 6 fail** (513 files; via `bun run test` ~17s). The 6 fails are ALL ENVIRONMENTAL on this machine: 3 self-host artifacts not built (F-BUILD-002 §3 + Bootstrap L3 + Self-host tokenizer parity); 3 test-bind A6-5 integration tests with hard-coded `/home/bryan-maclee/` cwd. Cumulative across S77 (from S76 close baseline 10,879): 10,879 → 10,890 (codegen-tightening +11) → 10,940 (A5-4+5 +50 net) → 10,945 (STRING-token quote-fix +5) → 10,948 (A5-5b +3) → 10,961 (A5-6 +13). **Net S77 delta: +82 pass / +4 skip / 0 todo / +6 fail (env-only on this machine).**
...
**In flight:** **scrml v0.2.0** breaking-change rewrite. Phase A1+ implementation: **A1a COMPLETE · A1b FUNCTIONALLY COMPLETE (B1-B22 + B17.2 + B17.3 SHIPPED) · A7 parser+typer COMPLETE (A5-2 + A5-3 SHIPPED) · A1c FULLY CLOSED (Waves 1-6, C0-C23 ALL SHIPPED) · C15 follow-up family CLOSED · B17.x FAMILY CLOSED · A8 family CLOSED (A6-1+A6-2+A6-3+A6-4+A6-5 ✅; A6-6 TBD) · A9 body-split min-viable v0.2.0 SHIPPED (S72 Ext 4 + S76 Ext 5) · A5-4 + A5-5 SHIPPED S77 (`<onTimeout>` codegen + computed-delay across both temporal surfaces) · A5-5b SHIPPED S77 (legacy `<machine>` body-parser `${...}` preservation) · A5-6 Feature 2 SHIPPED S77 (`<onIdle>` engine event-timeout watchdog).** A5-6 Feature 1 (named timer + `cancelTimer` builtin) DEFERRED on engine state-child body-render dependency. Insight 28 standing OQs at 1 (bridge-5 deferred). **Memory-leak detection deep-dive REFRESHED at S77** — Stage 7.5 slot moved to Stage 7.6 (BP took 7.5); 1 NEW leak surface confirmed (W-LEAK-010 idempotency unbounded growth, info-level lint).

**Origin sync at wrap-trigger:** scrmlTS 6 ahead of origin (A5-4+5 + W-LEAK-010 SPEC + STRING-quote-fix + A5-5b + A5-6 SCOPE + A5-6 SHIP); scrml-support 1 ahead (memory-leak deep-dive refresh). Both repos push at wrap step 7.
