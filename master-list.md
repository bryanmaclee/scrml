...
**Last updated:** 2026-05-09 (S74 close — **A1c Wave 4 (engines) CLOSED · B17.x family CLOSED · §51.0.H spec-complete · 8 ships · +245 tests · 0 regressions**)

**Tests (current — S74 close):** **10,553 pass / 65 skip / 1 todo / 0 fail** (487 files, 35,353 expect calls; via `bun run test` ~11s). 3 pre-existing self-host parity fails (F-BUILD-002 §3, Bootstrap L3, Self-host: tokenizer parity) NOT included in `bun run test` chain on this machine; pre-commit hook excludes them. Cumulative since S66 close: 9,090 → 9,241 (S67) → 9,425 (S68) → 9,626 (S69) → 9,734 (S70-PA-corrected) → 9,734 (S71 docs-only) → 9,854 (S72 full-suite-with-self-host) / 9,872 (S73 open via `bun run test`) → **10,308 (S73 close)**. **S73 net delta: +436 pass / -4 skip / +0 todo / +0 new fails across 9 ship commits.** C3 +23 / C4 +54 / C5 +34 / C6 +79 / C7 +61 / C8 +54 / C9 +35 / C10 +61 / C11 +36 = arithmetic 9,872 + 437 = 10,309, ±1 floating drift.
...
**In flight:** **scrml v0.2.0** breaking-change rewrite (~297-454h focused engineering — S67 ratified extensions remain on plan: DD-Harel hierarchy + Item C temporal surface migration + Item G B-shakeable timeouts + Insight 22 test-bind, ~57-94h additional). See **§0 v0.2.0 Migration Status** below for live progress dashboard. Spec target frozen post-Stage-0b (S58) as base; S67 amendments LANDED at S68 (`1de05ef`). Phase A1+ implementation: **A1a COMPLETE · A1b FUNCTIONALLY COMPLETE (B1-B22 + B17.2 + B17.3 SHIPPED) · A7 parser+typer COMPLETE (A5-2 + A5-3 SHIPPED) · A1c WAVES 1+2+3+4 CLOSED (C0-C15 ALL SHIPPED, S70+S72+S73+S74) · B17.x FAMILY CLOSED (parser+typer+codegen for `<onTransition>`+`effect=` SHIPPED S74; §51.0.H spec-complete) · A8/A6-1 SPEC SHIPPED.**

**Origin sync:** 8 commits ahead at S74 close — push at wrap. Cross-machine pickup expected on the other machine.
