...
const KEYWORD_DOCS = {
  ...
  "tare": "**tare(@cell)** / **tare(@cell, expr)** -- Statement-position reset baseline.\n\nThe statement twin of the `default=` attribute, for a cell that has no declaration site to put one on (an implicit `@x = 0`).\n\n`tare(@cell)` promotes the cell's CURRENT init expression into its reset default, at the point the statement runs -- so where you put the call is what decides the baseline. `tare(@cell, expr)` sets that default explicitly.\n\nBoth store the EXPRESSION, evaluated at reset time, never a snapshot. Same three target shapes as `reset`. See SPEC §6.8.4.",
...
export const SCRML_KEYWORDS = [
  ...
  { label: "tare", detail: "Set a cell's reset baseline from a statement (§6.8.4)", kind: CompletionItemKind.Keyword },
