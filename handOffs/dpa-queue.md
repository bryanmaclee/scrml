...
status: complete   # banked → running → complete → ratified(by PA) · COMPLETE dPA 2026-09-07 (ADVISORY) → debate artifact written, NOT ratified. **VERDICT: the banked question rests on a false premise and Call 1 outranks it — §4.18's display-text literal is implemented ONCE (native parser, 71 test refs) and DROPPED at `parse-file.js:316`, reaching emit in 1 of 3 loci; the spec-correct form renders with visible quotes while the spec-rejected form renders correctly. On the banked question itself: (a′) is NOT a new camp — and that STRENGTHENS it, because the real axis is resolution-freeness and scrml (b) FAILS it by execution (one apostrophe changes body extent), sitting alone in a content-classifying middle camp while every shipped markup system is in the delimited-text camp (a′) would join. The organizing result is the Class D (desync, two scans) vs Class C (collision, one scan) split: (a′) eliminates Class D by construction and ZERO of Class C. Whitespace — nominated as where (a′) dies — is where it is strongest: §4.18.5's second regime does not exist, and scrml is structurally immune to lit#114. NOT ONE of five voices argued its own brief; all five recommend (a′)'s MECHANISM; none recommends ratifying (a′) as worded. The FREE MOVE (delete the string branches from the engine closer-scan, ~35 LOC = the S196 delta) is newly-accepting, already licensed by S109+S196, and needs no camp ruling. CORRECTS THE ITEM: `g-splitblocks` is Class C and is NOT closed by (a′) — do not defer that fix on this ruling. Three new unfiled defects (`\${` silent wrong output; `<`+space vs HTML5; the S108 `?{` regression SPEC calls fixed).** → `scrml-support/docs/debates/plain-markup-text-as-string-dpa-045-2026-09-07.md`
