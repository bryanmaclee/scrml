...
`status:    complete`  # banked → running → complete → ratified(by PA) · COMPLETE dPA 2026-09-03 (ADVISORY) → DD written, NOT ratified. **VERDICT: substantially DUPLICATIVE of dpa-028 (RATIFIED S347) — do not re-run; POST the ratified direction to #509.** ★ dpa-028's ratified data contract is BROKEN: `chunks.json` names 3 chunk URLs, **2 are never written** (orchestrator-reproduced) → `cache.addAll()` rejects atomically → the cold boot fails. Use `_SCRML_IMMUTABLE ∪ {document}` instead. Panel 5/5 reproduced dpa-028's direction; 6 corrections to the record. · **origin:** GitHub issue **#509** (adopter, opened 2026-08-11) · banked S393-bryan 2026-09-01 · artifact `scrml-support/docs/deep-dives/offline-pwa-cold-boot-dpa-038-2026-09-03.md`
...
status: complete   # banked → running → complete → ratified(by PA) · COMPLETE dPA 2026-09-06 (ADVISORY) → DD written, NOT ratified. **VERDICT: the discriminator is FALSE as framed — strings force identical state and nobody proposes banning apostrophes. The hazard is a FOUR-member run-to-EOF class (`/* */` · `<!-- -->` §27.2, a third comment form the item never mentions · strings · backticks), the tokenizer GUESSES too (`readBlockComment` :1366, no `E-*UNTERMINATED*` exists anywhere), and the corpus instances are ALL in the forms the proposal KEEPS (unpaired: `/*` 0 · `<!--` 0 · string 7 · backtick 1 — the seven are prose apostrophes = the S402 arc). The sole HIGH is triggered by `//` AT FIRST SIGNIFICANT CHAR — bryan's exact surviving form — and its own second shape needs no comment at all (§40.8 root). Take the restriction as a ≤3.6% blast-radius cap, but Call 1 (make unterminated delimiters a DIAGNOSTIC — no surface removed, no migration) is the real 'big swath'.** → `scrml-support/docs/deep-dives/stateful-scanning-simplify-to-ship-dpa-044-2026-09-06.md`
