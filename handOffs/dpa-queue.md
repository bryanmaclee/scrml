...
| dpa-038 | **BANKED — UNRUN** S393. Adopter #509 offline/PWA cold-boot boundary. ⚑ Q3 pre-answered by PA fact: 6nz has ZERO service-worker/manifest/serviceWorker refs — the "offline-first PWA" line is a README design goal, not a shipped pattern | issue #509 + PA verification 2026-09-01 |
| dpa-039 | **BANKED — UNRUN** S393. Adopter #471 enterprise document workflows: non-JSON egress + `handle()` outside the protect/tenant envelope + inbound upload. ⚑ Premises are adopter-surveyed and NOT PA-verified — phase 1 of the DD is to verify 1/2/3 by execution | issue #471 |
...
| dpa-040 | **BANKED — UNRUN** S395. Should an if-as-expression ARM BODY be a `~` context boundary in its own right (§32.4, beside `${}` contexts and function bodies)? ⛑ **Banked BY the S395 ruling, not after it** — bryan answered *"your rec"* to *"(a) now, with (c) named as the question behind it"*, so per S276/S130 the ratification adopted BOTH halves and (c) is explicitly NOT closed. Limb (a) settles the §17.6.2-vs-§32.2 conflict by CARVE-OUT; (c) would remove the conflict's possibility BY CONSTRUCTION — squarely the S322 retrofit-vs-by-construction test applied to `~`. ⛑ **Urgency evidence:** §32's enforcement DOES NOT FIRE — six probes incl. SPEC §32.5's own verbatim `${ process(~) }` and §17.6.6's partial-`if` example produce ZERO `E-TILDE` diagnostics on merged main, though `TildeTracker`/`checkLinear` exist and `checkLinear` is invoked unconditionally at `type-system.ts:24470`. Cause NOT traced; phase 1 MUST re-verify by execution before reasoning from it. Boundary SET is the question — `~`-as-`lin` (§32.3) is NOT on the table. Adding a boundary is the LIMIT direction (row 1) but is `semantics-changed` and owes a MEASURED migration. Corpus-zero is blast radius ONLY, never demand evidence (S346). | the S395 `~` ruling (user-voice S395) |
...
`status: BANKED — UNRUN` · **origin:** GitHub issue **#509** (adopter, opened 2026-08-11, **open 21 days at banking**) · banked S393-bryan 2026-09-01
...
`status: BANKED — UNRUN` · **origin:** GitHub issue **#471** (adopter, opened 2026-08-08, **open 24 days at banking**) · banked S393-bryan 2026-09-01
...
`status: BANKED — UNRUN` · **origin:** the S395 `~` ruling; banked BY that ruling, not after it · banked S395-bryan 2026-09-02
