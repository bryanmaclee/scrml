...
| dpa-047 | **BANKED — UNRUN (S422 2026-09-18).** ⚑ **REOPENS S418 RULING 1 (`E-ASSIGN-003` fires).** bryan, on being shown the compiled code: *"I thought at some point I said that bare naming like this defaults to const, and if you want to mutate a variable, it had to be let."* PA-VERIFIED the implementation already does limb 1 — a fresh statement-position bare assignment emits `const count = args.length;`. ★★ **AND RULING 1 IS MIS-SCOPED:** §50.8.4's own trigger is *"an identifier that has not been declared appears on the left-hand side of an **assignment expression**"*, and §50.1 calls §50 *"a narrow, precisely scoped feature"* for **expression** position. S418 quoted §50.3.5 as the governing sentence; that sentence lives inside that narrow scope and **does not reach statement position** — the case that emits `const`. Building ruling 1 as ruled would be a WIDENING past its own governing sentence, not the conformance restoration it was ruled as. Three limbs to rule. **DO NOT BUILD RULING 1 UNTIL THIS DRAINS.** | **bryan** — REOPENS S418 ruling 1; banked S422 2026-09-18; artifact -> `scrml-support/docs/debates/` on drain |
| dpa-048 | **BANKED — UNRUN (S422 2026-09-18).** The anonymous typed binding: bryan, verbatim, sketching the throwaway-value shape — *"if I had my druthers this would look like `:int = args.length` / `_{ console.log("args: " + ~ ) }`"*. An unnamed typed declaration whose value is read back as `~`. Reads consistent with the S397 axiom (*`~` is ONE thing — the value of the preceding unbound expression statement*) and would moot the unused-binding question for throwaways by construction, since there is no name to go unread. AXIOM-ADJACENT — touches §32, which §0's own banner says is half spec-ahead with a REVERTED read path (S395 landed / S397 reverted; in-arm `~` reads are conditional today). Banked SEPARATELY from dpa-047 per the no-batch-axioms floor. | **bryan** — AXIOM-ADJACENT (§32); banked S422 2026-09-18; check dpa-040 for overlap first |
...
`status:    banked`  # S422 2026-09-18. ⚑ REOPENS S418 ruling 1. DO NOT BUILD E-ASSIGN-003 UNTIL THIS DRAINS.
...
`status:    banked`  # S422 2026-09-18. AXIOM-ADJACENT (§32). Banked SEPARATELY from dpa-047 per the no-batch-axioms floor.
