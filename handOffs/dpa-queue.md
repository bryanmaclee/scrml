...
| dpa-038 | **COMPLETE (ADVISORY) dPA 2026-09-03 — awaiting bryan; return leg = a #509 comment.** ★★ **SUBSTANTIALLY DUPLICATIVE of dpa-028, which bryan RATIFIED S347** — same issue, same three verbatim questions, and the bank carries no reference to it. The S347 return leg was posted FACTS-ONLY by design (dpa-028's own anti-goal: *"Do NOT answer Q1/Q2 on the issue — bryan does"*), so **the adopter has waited 23 days on a COMMENT, not a deliberation.** ★★ **But dpa-028's ratified data contract is BROKEN:** it named `chunks.json` as the precache list; orchestrator-reproduced, **2 of its 3 chunk URLs are never written** (empty-payload hash, write elided by design at `api.js:3366` to preserve determinism-replay). `cache.addAll()` rejects atomically ⇒ install fails ⇒ **the cold offline boot fails completely.** A correct fetchable set already exists and is already serialized: **`_SCRML_IMMUTABLE` in `_server.js`** (verified). 5-pole panel reproduced dpa-028's direction **5/5** (data out, worker never) and added three things not on its record: the **role axis is a SECURITY question** (a glob worker undoes §40 role partitioning into origin-scoped cache surviving logout); the **App Shell model does not transfer** to SSR-first (no shell artifact — the unit is a route-document cache); precache is **not** redundant with the existing prefetch tiers (only Cache Storage has app-controlled lifetime). **6 corrections:** PRIMER §13.5's *"`_{}` sliver-empty, 0 uses"* is stale (**flogence = 249**) · a SW is **mechanically inexpressible** via `_{}` (retract the "first consumer" framing) · FACT D narrowed (serving path EXISTS; the gap is a static **input** path) · 6nz line is **README.md:22**, not :419 · `chunks.json` keys are absolute build-machine paths. **Rec: close as duplicative + POST THE S347 DIRECTION to #509 + file the `chunks.json` defect.** | **RATIFIED S405 2026-09-07** (bryan: *"dpa-038 close it"*) — CLOSED as duplicative of dpa-028. Return leg POSTED to #509 and the issue CLOSED; the S347 facts-only constraint is superseded. ⚑ The posted direction carries a PA-verified CORRECTION to dpa-028's ratified recipe: `chunks.json` is NOT a precache source (2 of 3 URLs never written, PA-reproduced by execution) — use `_SCRML_IMMUTABLE`. Defect filed `g-chunks-json-names-chunk-urls-that-are-never-written` (HIGH) |
| dpa-039 | **RATIFIED + CLOSED S405 2026-09-07** (bryan: *"all your recs look good"* → 1a · 2 · 3b · 4a). Closed as duplicative-but-fact-shifted; corrections live as filed gaps with reproductions. Return leg posted to #471. — PRIOR VERDICT: **COMPLETE (ADVISORY) dPA 2026-09-03 — awaiting bryan; return leg = a #471 comment.** ★★ **TIME-SENSITIVE — dpa-030 must not be ratified on its current wording.** dpa-030 reasons that the host-escape fork *"(a) was never a live option"* because `handle()`+`request.formData()` was RUNTIME-BROKEN. **It WORKS** — `injectHandleRequestAwaits` (`scheduling.ts:662-711`); RUN: `200 / text/plain / got:note.txt:11`. **Fork (a) is live again.** Also stale: dpa-029's *"§40.3.5 fails E-SCOPE-001"* (**fixed S352**, `a7e99e8f`). ★★ **THE LEAK IS LARGER THAN RECORDED AND STILL LIVE at ~40 sessions after 7/7 said "defects FIRST".** dpa-029 recorded the bypass as `globalThis.Response`. **Ordinary function extraction defeats it** — orchestrator-reproduced: query in a helper + `new Response` in `handle()` **compiles clean** while the same code in one body fires `E-PROTECT-004`. The gate slices **per-`fn` source span**. **A member-chain walk does not catch this, so the "improve the regex" repair is permanently dead.** Aggravations: the predicate is **duplicated** (`protect-egress.ts:296` / `tenant-egress.ts:391`); `reveal` suppression is **EXISTENCE-keyed** (any `.reveal(` disarms every protected column in that body) — worse than dpa-033's name-keyed occlusion. ★★ **TWO NEW silent fail-opens, on no record:** (1) the **default baseline-CSRF arm has no `instanceof Response` guard** (`:4232` vs the guarded `:4502`) — a deliberate 403/redirect ships **200 / application/json / `{}`**, no diagnostic; the guard's own comment shows it was placed under a precondition **S352 then removed**. (2) A **`<schema>`-only app gets an EMPTY tenant set** — orchestrator-confirmed: `parseSchemaBlock` (`schema-differ.js:31`) recognizes the DSL grammar only, so raw-DDL `<schema>` yields **zero tables silently**; five silent-drop points in series, **none diagnostic**. ⚑ **NOT the S288 regression it first looked like — S288 is live; the failure is a layer down**, and the tenant conformance suite cannot see it (every case also has a `<db>`). **§60.7 RESOLVED AGAINST the refusal** (the simplicity pole conceded it): you cannot omit a Content-Type, so it is **constitutive, not adjacent** — §61.5 chose `application/json` and hard-coded it at `emit-server.ts:4724`. `produces=` is scope **disclosure**, not growth. **`accepts=:enum` is the analogue of Servant's `ReqBody '[JSON] SumType`, NOT of its content-type list — orthogonal axes, so `produces=` needs no enum redesign.** Q3 **5/5 against widening `accepts=`** (its exhaustiveness proof needs a single static discriminant; multipart has none). **5/5: a source-text predicate cannot carry a confidentiality guarantee**; the in-repo `egress-field-scan.ts` regex→acorn promotion is the shipped template. **⚑ 2 poles converged on the `instanceof Response` passthrough guard; the typed-media-type pole says landing it alone on the protect arm is a CONFIDENTIALITY REGRESSION** — pair it: guard the unprotected arms, make an author `Response` under `protect=` a **compile error**, delete the regex. | RULED S405-bryan · four calls taken · #471 answered |
...
| dpa-040 | **COMPLETE (ADVISORY) dPA 2026-09-03 — awaiting bryan.** ★★ **THE QUESTION CANNOT BE ANSWERED AS BANKED — the checker has never run.** `E-TILDE-001`/`002` fire **0/6** on the SPEC's own worked examples (incl. §32.5's verbatim `${ process(~) }` and §17.6.6's partial-`if`), while `E-LIN-001/002/003` fire correctly **from the same `checkLinear`**. Root cause traced by instrumentation (70 files, **243 invocations, `tildeInit=0` in every one**): the three `tt` sites key on `tilde-init`/`tilde-ref`/`lift-stmt`, which have **ZERO producers**; `bare-expr` has no case and its `default:` is a **complete no-op** (payload is on `.expr`, not `.body`/`.children`). So every candidate boundary set is **extensionally identical today**. The in-flight carve-out ships **`-pos` cases only, and a `-neg` case is structurally impossible.** ★★ **`~` was DELIBERATELY EXCLUDED, not merely unwired** — `type-system.ts:19259` `if (name.startsWith("@") || name === "~") return;` under the comment *"Tilde accumulator is '~' — not a lin variable"*, **contradicting §32.3 AND §35.8, both of which say at HEAD that it SHALL be one.** That guard sits in a **field-driven** walk that would catch every buried `~` read for free ⇒ the repair may be near a two-line deletion. Also: the `~` half **reimplemented the branch join twice, wrongly, in opposite directions** — `if-stmt` `:18855` takes the consequent and ignores the alternate (**fail-open**); `match` `:18942` discards every arm (spuriously fail-closed) — while the generic `lin` join beside them is right once. ★★ **THE BLAST RADIUS WAS MEASURED ON THE WRONG AXIS.** §32.2's SHALL is **unconditional** (verified at HEAD — no void exemption), so **two consecutive bare calls in ANY block are already E-TILDE-002**; the arm is where the conflict was *noticed*, not where it lives. Orchestrator scan: **~828 consecutive bare-call runs across 3,407 files** vs the **17** bound-position arm sites the fork was scoped to (a prior figure said 5 — wrong by 3.4×), of which **zero** have a bare-call intermediate. *(Line-based heuristic — order of magnitude, not the integer.)* ★★ **THE SUBSTRUCTURAL POLE ARGUES THE BOUNDARY IS UNSOUND, NOT UNNECESSARY** — a §32.4-style **opacity** boundary at the arm manufactures an **un-consumable `~`** (`compute()` then `if (c) { lift f(~) } else { lift g(~) }` leaves no reachable consumption site) and forbids the canonical linear-branching program. Two things are being called "boundary": **opacity** (§32.4) vs **reconciliation** (`lin`'s branch rule). **If it holds, limb (c) closes as REJECTED-on-soundness, not deferred.** It disagrees sharply with the type-discipline pole's *anonymity* derivation (which makes the arm a boundary as a theorem) — **the fork is stated, not adjudicated.** ★★ **§32.6's LOOP ELISION IS INVERTED and makes §35.8 UNSATISFIABLE** — `lin` prohibits outer consumption in a loop (`E-LIN-002`); §32.6 blesses produce-n-times/consume-zero. One tracker cannot both prohibit and permit the same construct. **Harder than the banked conflict, and it was not on the fork.** **Rec: fail-closed codegen floor first (replace `emit-expr.ts:1178`'s `null /* ~ orphaned */` — needs zero analysis strength), then honour §35.8, then WARNING-tier for one release on the ratified S365 `W-TYPE-031-UNPROVEN` precedent, then rule.** Highest-leverage artifact: **a `-neg` gate on `scripts/s34-census.ts`** — every Error code must have a conformance case that actually produces it. | **RATIFIED S409 2026-09-08 — CLOSED** (bryan: *"I think stale read is a unnecessary foot gun if the cost is reasonable, and it seems to be"*) — **option (c): KEEP the no-double-read half, DROP must-consume-before-scope-exit.** The banked arm-body-boundary question **DISSOLVES**: with the checker dead every candidate boundary set was *extensionally identical*, so it carried no observable content. Re-bankable once the kept half is built, if it still matters then. Kept half bounded at **127 `~` read sites / 71 files**; the struck clause measured **~1,330 bare-call statements / 1,928 files**. Grounds + the verified measurements: user-voice S409. artifact `scrml-support/docs/deep-dives/tilde-arm-body-boundary-dpa-040-2026-09-03.md` |

| dpa-041 | **COMPLETE (ADVISORY) dPA 2026-09-05 — awaiting bryan; FOUR calls, §8.** ⚑ Call 1 BLOCKING — the question cannot be answered as framed: at a BOUND site lift mode is already hard-coded by CONSTRUCT KIND at context creation (`emit-logic.ts:4839` array / `:4755` `:5266` single); the outward scan + its §7392 tie-breaker live in STATEMENT position (`:4442/:4446/:4488/:4491`), which has no binding site and therefore no declared type. Type-directed selection targets the deterministic case and cannot reach the ambiguous one. ⚑ Call 2: an annotation at these sites is a NET LOSS OF INFORMATION — `!letAnnot` (`type-system.ts:10573`) suppresses `W-TYPE-031-UNPROVEN` while `E-TYPE-031` (`:10446`) covers only {number,string,boolean} vs a literal; no assignability relation exists anywhere (grep: 0 matches). ⚑ Call 3: 4/4 poles — the silent-wrong is INDEPENDENT of this proposal (repoints `var`, not `mode`); sever and ship the `:4891` guard. ⚑ Call 4: `docs/changes/s397-tilde-one-or-two/progress.md` is a DANGLING POINTER cited by the master-list banner. All 5 PA claims reproduce; §10.2 cite corrected to §10.4/`:7097` (+ a 4th uncited scan statement at `:7067`). → `scrml-support/docs/deep-dives/lift-mode-declared-type-dpa-041-2026-09-05.md` | **RATIFIED S409 2026-09-08 — CLOSED** (bryan: *"close it"*) — **the banked question is DEAD AS FRAMED** (Call 1): lift mode is fixed by CONSTRUCT KIND at context creation, while the ambiguous site sits in STATEMENT position with no binding site and therefore no declared type, so type-directed selection structurally cannot reach it. Per S276/S130 this adopts the full rec text: **Call 3 was ALREADY DISCHARGED** — its `:4891` guard landed at **#830 on 2026-09-03, two days BEFORE this item ran** (`_isDirectArmBodyStmt(declNode, opts)`, carrying the S395 limb-(a) ruling); **Call 2 is CORRECTED** — its *"no assignability relation exists anywhere (grep: 0 matches)"* is literally FALSE, `fieldTypeAssignable` exists with exactly ONE caller bounded to §14.8.8 and a docstring saying *"NOT a general subtyping relation"* (substantively right, checkably wrong as worded; matches the S404 finding); **Call 4 FIXED in this landing** — `docs/changes/s397-tilde-one-or-two/{progress,BRIEF}.md` never reached main at all (they lived only on unmerged worktree branch `worktree-agent-a9225a06dce5e5f3c`), so the master-list §0 banner cited the evidence matrix for a RATIFIED axiom ruling at a path no reader could open — S346 class; both files RECOVERED and landed rather than the citation deleted. Grounds: user-voice S409. |
| dpa-042 | **COMPLETE (ADVISORY) dPA 2026-09-05 — awaiting bryan; FOUR calls, §9.** ⚑ Call 1 OUTRANKS the banked question — SPEC §17.6.6's OWN canonical example compiles GREEN (exit 0, zero diagnostics, `node --check` clean) and emits JS that throws `ReferenceError`; an in-repo fixture already asserts the missing `E-TILDE-001`. ⚑ Call 2: §35.8's "the full `lin` rule set applies to `~`" is FALSE ON THE MERITS (orchestrator-verified) — `TildeTracker:18380` is a one-cell init/consume state machine (definite assignment + single introduction) while `lin`'s three codes count ELIMINATIONS; not one is about what `~` enforces. ⚑ Call 3: the non-cosmetic unification is one WALK + one JOIN parameterized by a lattice (3/4 poles converge) — NOT the rule set; delete both `~` joins (`:18855` fail-open, `:18942` fail-closed), reify the producer at PARSE. ⚑ Call 4: the two live verdicts — distinct-but-shared-engine vs retire-and-desugar-to-`lin` — are INCOMPATIBLE (a 1:1 desugar changes the error set); yours. Zero-producers CONFIRMED exhaustively; guard-deletion DISPROVEN by in-memory execution (byte-identical, 6/6). §32.6 loop inversion convergent with dpa-040 Call 3. → `scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md` | **RATIFIED S409 2026-09-08 — Call 4 CLOSED** (bryan: *"I think stale read is a unnecessary foot gun if the cost is reasonable, and it seems to be"*) — **option (c) is distinct-engine with a NARROWED rule set**, which answers Call 4's incompatible pair (*distinct-but-shared-engine* vs *retire-and-desugar-to-`lin`*): **not a desugar.** Calls 1-3 stand as advisory findings and are unaffected. Grounds: user-voice S409. artifact `scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md` |
| dpa-043 | **COMPLETE (ADVISORY) dPA 2026-09-06 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call.** ★★ **Call 1 OUTRANKS the banked question — it is FALSE AS FRAMED.** scrml has had a normative lazy-pull evaluation model since **SPEC.md:2898 §6.6.3 "Evaluation Strategy — Lazy Pull with Dirty Flags"** (`const <name> = expr` derived cells: re-evaluated on read, dirty-flagged on write, DAG-checked §6.6.10) — it PREDATES §13.6. The real fork is narrower: does scrml also need an **ITERATOR-shaped** lazy pull alongside the declarative one? ★★ **Call 2 — A and C REJECTED 3/3, by two independent derivations.** The origin's lexical-boundary premise defeats §7040 but **never reaches §13.6, which already legalizes `function*`/`yield` INSIDE `${}`** (SPEC.md:7905, verbatim). So A does not dissolve §7040's footgun — it **recreates it one level down inside `${}`**, where the boundary cannot save it, and would re-derive §10.2 coercion + §10.5.5 concurrent-emission + §17.6.10 + §48.5 under pull semantics across **219** `lift` files (violating "I don't want go and change everything"). ★★ **Call 3 — the live fork is B vs D; panel split 1/1/1** (lazy=B · eager=B+D · simplicity=D). ⚑ **The lazy-pull pole was briefed to argue A or C and REFUSED**, declining to "manufacture a widening my own discipline would reject from anyone else" — the strongest signal in the run. ★★ **Call 4 (ORCHESTRATOR-MEASURED, CORRECTS A POLE) — scrml's SSE lowering has NO BACKPRESSURE.** `emit-server.ts:3684` emits `async start(_scrml_ctrl)` **not** `pull()`, and **`desiredSize` occurs 0 times in all of `compiler/src/`**; the unbounded `while(true){yield}` form (scrml's OWN conformance case) enqueues without bound. The surface justifying §13.6 does not implement the capability that would justify lazy/pull. **Call 5 — UNANIMOUS FLOOR, all three poles independently: adjudicate `lift` vs `yield` IN WRITING IN THE SPEC whatever the ruling** (+ record the measured outcome; + rename `E-SSE-001`, whose own SPEC text at :7979 apologizes for its name). **MEASURED BY EXECUTION at HEAD `c64c8400`** over TRACKED `.scrml`: **2,476** corpus · **12** generator-using · **11/11 `function*` are `server function*`** · **0** in stdlib(53)/samples(877)/examples(71)/compiler(123)/benchmarks(7) · **219** use `lift`. ⚑ Two measurement traps corrected in-run: naive `find` reports **261,307** (258,886 are `.claude/` session artifacts); naive `grep` reports generators in the self-host + native parser (those are `"yield"` as a **string in an EXPR_KEYWORDS set** + comment text — `compiler/` has **zero**). ⚑ **Corpus-zero is COST + MIGRATION evidence, NEVER demand evidence** (LLM-authored corpus) — all three poles held to this; D breaks **zero** existing programs. **NOT RATIFIED — RUN-not-RATIFY.** ⚑ **SIDE-FINDINGS, route to PA, NOT part of the ruling:** (1) **SILENT-WRONG CODEGEN — `type X:enum = { A  B }` (space-separated) drops every variant after the first**, both emits, **exit 0, no diagnostic**; comma/newline/pipe all correct; **space is NOT a SPEC-sanctioned form** and should be a parse error. Blast radius **1 file** — `conformance/cases/server-fn/sse-generator-binding-seed-survives/case.scrml`, which therefore has `Phase.Active === undefined` and an `<Active>` arm that can never fire, **yet still passes** (it asserts only the pre-event seed, `#active count 0` — right for the wrong reason). (2) SSE errors still swallowed by an empty `catch` at `emit-server.ts:3768` — giti-025 unfixed. (3) `W-LINT-018` false-fires TWICE on the sanctioned generator surface. | **RATIFIED S409 2026-09-08 — option (d), ON A DEPRECATION WINDOW. CLOSED.** (bryan: *"d, on a deprecation window"*) — **NARROW §13.6's `function*`/`yield`/`yield*` grant back to `server function*` (§37 SSE) + `_{}`.** Per S276/S130 adopts the full text (d) carried, incl. the stated grounds: fork-rule rows 1-3 all discriminate toward it (LIMITS · fails CLOSED · newly-rejecting is the REVERSIBLE direction) and the first discriminating row decides; §13.6's cohesion argument (*"restricting to SSE-only would have forced SSE to a different mechanism with no cohesion gain"*) is **RE-PRICED not refuted** — usage is already SSE-only so narrowing orphans nothing; and Pillar 5 cannot be claimed while two overlapping emit-and-continue primitives survive unadjudicated. ⚑ **Cost carried with the ruling, NOT free:** the parser recognizes `function*` uniformly, so a positional restriction needs NEW machinery (~85-line grant + 2 worked examples traded for a ~15-line positional clause + a renamed diagnostic — net reduction, not zero). ⚑ **"On a deprecation window" binds §63:** Stage-1 SOFT-DEPRECATED, so the landing co-lands {a parses-identically W-lint} + {a reserved E- named in §34} + {a `scrml fix` rule OR a designer-card} per §63.2, and **SHALL NOT name a removal version** (§63.2; removal only at a MAJOR per §63.3). Template: `W-MATCH-ARROW-LEGACY` / `W-GIVEN-ARROW-LEGACY` / `W-CONST-AT-DEPRECATED`. **Call 5's floor rides with it (unanimous, was never part of the fork):** amend §13.6/§7040 to state the `lift` (eager, producer decides) vs `yield` (lazy, consumer decides) distinction IN WRITING; record the measured outcome per the S131 convention; rename `E-SSE-001`, whose SPEC text at :7979 apologizes for its own name. **PA-VERIFIED INDEPENDENTLY before the ruling:** 11 of 2,553 tracked `.scrml` carry a `function*` declaration and **11/11 are `server function*`** — zero non-SSE producers; and Call 4 holds — `desiredSize` occurs **0×** in all of `compiler/src/`, `pull(` **0×** in `emit-server.ts`, the emit is `async start(_scrml_ctrl)` at `:3997`, so the backpressure capability that would justify a lazy/pull primitive is unimplemented even where generators ARE used. ⚑ **Corpus-zero was treated as blast-radius ONLY, never demand evidence** (bryan's S346 rule; the corpus is 100% LLM-authored) — the discoverability confound is UNRESOLVED and is the live risk this ruling accepts. Grounds: user-voice S409. |
| dpa-042 | **COMPLETE (ADVISORY) dPA 2026-09-05 — awaiting bryan; FOUR calls, §9.** ⚑ Call 1 OUTRANKS the banked question — SPEC §17.6.6's OWN canonical example compiles GREEN (exit 0, zero diagnostics, `node --check` clean) and emits JS that throws `ReferenceError`; an in-repo fixture already asserts the missing `E-TILDE-001`. ⚑ Call 2: §35.8's "the full `lin` rule set applies to `~`" is FALSE ON THE MERITS (orchestrator-verified) — `TildeTracker:18380` is a one-cell init/consume state machine (definite assignment + single introduction) while `lin`'s three codes count ELIMINATIONS; not one is about what `~` enforces. ⚑ Call 3: the non-cosmetic unification is one WALK + one JOIN parameterized by a lattice (3/4 poles converge) — NOT the rule set; delete both `~` joins (`:18855` fail-open, `:18942` fail-closed), reify the producer at PARSE. ⚑ Call 4: the two live verdicts — distinct-but-shared-engine vs retire-and-desugar-to-`lin` — are INCOMPATIBLE (a 1:1 desugar changes the error set); yours. Zero-producers CONFIRMED exhaustively; guard-deletion DISPROVEN by in-memory execution (byte-identical, 6/6). §32.6 loop inversion convergent with dpa-040 Call 3. → `scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md` | dPA advisory — PA to review |
| dpa-043 | **COMPLETE (ADVISORY) dPA 2026-09-06 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call.** ★★ **Call 1 OUTRANKS the banked question — it is FALSE AS FRAMED.** scrml has had a normative lazy-pull evaluation model since **SPEC.md:2898 §6.6.3 "Evaluation Strategy — Lazy Pull with Dirty Flags"** (`const <name> = expr` derived cells: re-evaluated on read, dirty-flagged on write, DAG-checked §6.6.10) — it PREDATES §13.6. The real fork is narrower: does scrml also need an **ITERATOR-shaped** lazy pull alongside the declarative one? ★★ **Call 2 — A and C REJECTED 3/3, by two independent derivations.** The origin's lexical-boundary premise defeats §7040 but **never reaches §13.6, which already legalizes `function*`/`yield` INSIDE `${}`** (SPEC.md:7905, verbatim). So A does not dissolve §7040's footgun — it **recreates it one level down inside `${}`**, where the boundary cannot save it, and would re-derive §10.2 coercion + §10.5.5 concurrent-emission + §17.6.10 + §48.5 under pull semantics across **219** `lift` files (violating "I don't want go and change everything"). ★★ **Call 3 — the live fork is B vs D; panel split 1/1/1** (lazy=B · eager=B+D · simplicity=D). ⚑ **The lazy-pull pole was briefed to argue A or C and REFUSED**, declining to "manufacture a widening my own discipline would reject from anyone else" — the strongest signal in the run. ★★ **Call 4 (ORCHESTRATOR-MEASURED, CORRECTS A POLE) — scrml's SSE lowering has NO BACKPRESSURE.** `emit-server.ts:3684` emits `async start(_scrml_ctrl)` **not** `pull()`, and **`desiredSize` occurs 0 times in all of `compiler/src/`**; the unbounded `while(true){yield}` form (scrml's OWN conformance case) enqueues without bound. The surface justifying §13.6 does not implement the capability that would justify lazy/pull. **Call 5 — UNANIMOUS FLOOR, all three poles independently: adjudicate `lift` vs `yield` IN WRITING IN THE SPEC whatever the ruling** (+ record the measured outcome; + rename `E-SSE-001`, whose own SPEC text at :7979 apologizes for its name). **MEASURED BY EXECUTION at HEAD `c64c8400`** over TRACKED `.scrml`: **2,476** corpus · **12** generator-using · **11/11 `function*` are `server function*`** · **0** in stdlib(53)/samples(877)/examples(71)/compiler(123)/benchmarks(7) · **219** use `lift`. ⚑ Two measurement traps corrected in-run: naive `find` reports **261,307** (258,886 are `.claude/` session artifacts); naive `grep` reports generators in the self-host + native parser (those are `"yield"` as a **string in an EXPR_KEYWORDS set** + comment text — `compiler/` has **zero**). ⚑ **Corpus-zero is COST + MIGRATION evidence, NEVER demand evidence** (LLM-authored corpus) — all three poles held to this; D breaks **zero** existing programs. **NOT RATIFIED — RUN-not-RATIFY.** ⚑ **SIDE-FINDINGS, route to PA, NOT part of the ruling:** (1) **SILENT-WRONG CODEGEN — `type X:enum = { A  B }` (space-separated) drops every variant after the first**, both emits, **exit 0, no diagnostic**; comma/newline/pipe all correct; **space is NOT a SPEC-sanctioned form** and should be a parse error. Blast radius **1 file** — `conformance/cases/server-fn/sse-generator-binding-seed-survives/case.scrml`, which therefore has `Phase.Active === undefined` and an `<Active>` arm that can never fire, **yet still passes** (it asserts only the pre-event seed, `#active count 0` — right for the wrong reason). (2) SSE errors still swallowed by an empty `catch` at `emit-server.ts:3768` — giti-025 unfixed. (3) `W-LINT-018` false-fires TWICE on the sanctioned generator surface. | **RATIFIED S409 2026-09-08 — option (d), ON A DEPRECATION WINDOW. CLOSED.** (bryan: *"d, on a deprecation window"*) — **NARROW §13.6's `function*`/`yield`/`yield*` grant back to `server function*` (§37 SSE) + `_{}`.** Per S276/S130 adopts the full text (d) carried, incl. the stated grounds: fork-rule rows 1-3 all discriminate toward it (LIMITS · fails CLOSED · newly-rejecting is the REVERSIBLE direction) and the first discriminating row decides; §13.6's cohesion argument (*"restricting to SSE-only would have forced SSE to a different mechanism with no cohesion gain"*) is **RE-PRICED not refuted** — usage is already SSE-only so narrowing orphans nothing; and Pillar 5 cannot be claimed while two overlapping emit-and-continue primitives survive unadjudicated. ⚑ **Cost carried with the ruling, NOT free:** the parser recognizes `function*` uniformly, so a positional restriction needs NEW machinery (~85-line grant + 2 worked examples traded for a ~15-line positional clause + a renamed diagnostic — net reduction, not zero). ⚑ **"On a deprecation window" binds §63:** Stage-1 SOFT-DEPRECATED, so the landing co-lands {a parses-identically W-lint} + {a reserved E- named in §34} + {a `scrml fix` rule OR a designer-card} per §63.2, and **SHALL NOT name a removal version** (§63.2; removal only at a MAJOR per §63.3). Template: `W-MATCH-ARROW-LEGACY` / `W-GIVEN-ARROW-LEGACY` / `W-CONST-AT-DEPRECATED`. **Call 5's floor rides with it (unanimous, was never part of the fork):** amend §13.6/§7040 to state the `lift` (eager, producer decides) vs `yield` (lazy, consumer decides) distinction IN WRITING; record the measured outcome per the S131 convention; rename `E-SSE-001`, whose SPEC text at :7979 apologizes for its own name. **PA-VERIFIED INDEPENDENTLY before the ruling:** 11 of 2,553 tracked `.scrml` carry a `function*` declaration and **11/11 are `server function*`** — zero non-SSE producers; and Call 4 holds — `desiredSize` occurs **0×** in all of `compiler/src/`, `pull(` **0×** in `emit-server.ts`, the emit is `async start(_scrml_ctrl)` at `:3997`, so the backpressure capability that would justify a lazy/pull primitive is unimplemented even where generators ARE used. ⚑ **Corpus-zero was treated as blast-radius ONLY, never demand evidence** (bryan's S346 rule; the corpus is 100% LLM-authored) — the discoverability confound is UNRESOLVED and is the live risk this ruling accepts. Grounds: user-voice S409. |
| dpa-044 | **COMPLETE (ADVISORY) dPA 2026-09-06 — awaiting bryan; FOUR calls, §7.** ⚑ **Call 1 OUTRANKS the banked question** — silent consume-to-EOF is UNIVERSAL (the tokenizer does it too, `readBlockComment` tokenizer.ts:1366; **no `E-*UNTERMINATED*` diagnostic exists anywhere in the compiler**), so making it a DIAGNOSTIC removes NO surface, costs NO migration, and closes all four members. ★★ **The discriminator is FALSE as framed** and the hazard is a FOUR-member class — `/* */` · `<!-- -->` (§27.2, a THIRD comment form the item never mentions, 35 corpus files) · strings · backticks. **Measured unpaired openers: `/*` 0 · `<!--` 0 · string 7 · backtick 1** — the proposal removes the two forms with ZERO instances and keeps the two with EIGHT (prose apostrophes = the S402 unlandable arc). **The sole HIGH is triggered by `//` at first significant char — bryan's exact SURVIVING form** — and its second shape drops a statement with no comment at all (§40.8 `default-logic` root). Restriction still worth taking (≤3.6% migration; deletes `urlSlashesAt` + collapses 3 line scanners), but as a blast-radius cap, not as the defect fix. ⚑ Item's headline **"22 files" does not reproduce — it is 10** at its own commit. ⚑ **This row did not exist until the dPA added it** — the item was banked with no table row, so `dpa-debt` read `0 UNRUN`; third witnessed obligation-vs-probe mismatch (§8). | **RATIFIED S405 2026-09-07 — CALL 1 ONLY** (bryan: *"dpa-044 call 1"*). Unterminated delimiters become a DIAGNOSTIC. ⛑ **SCOPE NOT RULED and the build is split:** PA-verified by execution that `<p>it's ready</p>` compiles CLEAN in a plain-markup body and only breaks NESTED IN A STATE-CHILD — so the diagnostic in a free-text body would FALSE-POSITIVE on ordinary prose and would newly assert a delimiter role S109 denied and dpa-045 reopens. **CODE-position limb buildable now; markup-body limb GATED ON dpa-045.** Calls 2-4 + the `//`-restriction NOT ruled |
| dpa-045 | **COMPLETE (ADVISORY) — BOTH ROUNDS RUN. Round 2 drained by the dPA 2026-09-08 → `scrml-support/docs/debates/plain-markup-text-as-string-dpa-045-round2-2026-09-08.md` (committed, `c92534e`+`891ffca`). Awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call. FIVE calls (A-E).** ⚑ **THIS ROW IS FOUR DAYS LATE AND THE CAUSE IS ON THE RECORD.** `fd69d1fc` (land(s409)) states in its own message *"Also lands the dPA's dpa-045 round-2 completion (queue row + delta entry)"* — **the delta entry landed (`[2987]`); the queue row did NOT.** Measured: that commit's `dpa-queue.md` diff changes **exactly four rows — dpa-040/041/042/043 — and nothing for dpa-045.** **Consistent with** — not proven to be — the clobber class that commit DOES narrate repairing (a cherry-pick `git checkout --theirs` blind resolution). The class is established independently by `5d4f038d` — *"fix(dpa-queue): restore the dpa-039 and dpa-030 rulings — **a third row lost to the #936 clobber**"* — on `review/s409-peter-language-surface`, pushed, **not yet on main** (which is why `dpa-debt` on main still reads dpa-039 as ADVISORY). **dpa-045 is the FOURTH such row and it was not in that repair.** That repair's verification — *"the diff vs main is EXACTLY the four dpa-04x rows and nothing else"* — **confirmed the rows it was looking for and could not see the row it was not.** An axis-vs-method miss inside the very commit that landed the axis-vs-method durable one level up for the SPEC-INDEX gate. Consequence: `dpa-debt` read **1 UNRUN** for four days on an item whose artifact was committed and whose breadcrumb was on main. ★★ **ROUND 2's VERDICT — Q1 the wording EXISTS and is DRAFTED** (the §4.18.1/.1a/.1b two-mode **dual pair**, never "a string literal"; + the §4.18.5 collapse-regime DELETION in the same amendment; + six implementer pins, of which **#4 cooked-or-raw** was named by nobody on the panel until the cost voice found it and **two shipped modules already disagree**; + a **Nominal/spec-ahead** status tag volunteered by (a′)'s own advocate). **Q2 — none of the three costs sinks (a′)**: the control-flow diagnostic is a **CREDIT** (1-of-5 recall, 4 live false fires, ~40 LOC retired), the `${` escape **re-priced DOWN to 8-16h of ROUTING** by the author of round 1's "largest unpriced cost" framing, and §40.8 is **ORTHOGONAL** — (a′) is a scope REDUCER for ruling 3's markup arm, **NOT a precondition** (the dPA's own hypothesis, refuted by two voices). **Q3 — Class C is ORTHOGONAL to the camp**, over-determined: nine measured failures, **two root causes, and a camp ruling fixes neither.** **Q4 — the M6/MD window is DEAD** (98-180h unstarted, one reverted flip at **845 test failures**, ~270 sessions stationary); its author WITHDREW the urgency framing. ★★ **CALL 1 OUTRANKS THE BANKED QUESTION AND IT RE-PRICES BOTH ROUNDS: round 1's "free move" shipped SIX HOURS after round 1 (`e523478b`, #892) and its own commit says "IT WAS NOT A FREE MOVE"** — four rounds, five HIGHs, every one silent behaviour loss at exit 0, because *"the string branches were doing DOUBLE DUTY … accidentally shielding four opener-blind flat scanners … Deleting them unmasked a pre-existing architectural gap."* It did **not** delete the Class D apostrophe family — it **MOVED it one layer down into `findOpenerEnd` and WIDENED its reach by five scan paths** (`g-findOpenerEnd-inQuote-treats-apostrophe-as-delimiter`, MED/open). **S109 → S196 → S405 is the same per-locus fix applied three times and the third enlarged the fourth.** ★★ **NEW GENERAL PRINCIPLES:** *"newly-accepting only" is valid about a PREDICATE, never about a SCANNER* (a scanner's branches decide EXTENT, and extent propagates bidirectionally) · *a Class C remedy is coherent only if SINGLE-SCAN-SUFFICIENT* — so **delimiter-escapes stay bounded and character-escapes are Class D machinery** (which is why the panel recommends **DELETING** `\"` from §4.18.3 rather than fixing it). ★★ **THE HIGHEST-VALUE ITEM IN TWO ROUNDS IS NOT A CAMP RULING — it is the LOOP CENSUS, 4-8h, and THREE voices converged on it independently.** The unit is the **LOOP, not the locus** (11 loops at the one locus round 1 counted as "1"; 4 in class; function-level analysis reported the dangerous ones SAFE). It converts the largest unpriced cost in the item — an **80-200h scanner migration priced at n=1, whose one measurement came in 6-10× over this panel's own estimate** — into a number, and it is **the one work item not wasted under ANY ruling.** ⚑ **TWO round-1 calls WITHDRAWN BY THEIR OWN AUTHORS** ("implemented ONCE" → **twice**, `emit-match.ts:647` live on the default path; "changing §4.18 emits zero output" → **FALSE, 82 of 83 sites move through codegen**) and **TWO dPA hypotheses REFUTED by the panel.** ⚑ **§4.18.3's normative escape catalog is 1-for-3 at emit from ONE discard** (`parse-file.js:316` drops the correctly-`cooked` payload) **plus one scanner with no backslash case** — neither a camp question. **NOT ruled — RUN-not-RATIFY.** **ROUND 1 (2026-09-07) — verdict retained verbatim: ** **COMPLETE (ADVISORY) dPA 2026-09-07 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call. FIVE calls.** ★★ **Call 1 OUTRANKS the banked question.** The item asks whether to EXTEND §4.18's by-construction answer from the 3 code-bearing loci to plain markup. **Measured: §4.18's display-text literal is implemented ONCE, completely, in the native parser and DROPPED at `native-parser/parse-file.js:316`** (`DisplayTextLiteral` → `synthTextNode(…, source)` — "verbatim literal source", quotes included). It reaches emit in **1 of 3 loci** (the `:`-shorthand). **The spec-correct form renders WITH visible quotes; the spec-rejected form renders correctly** — polarity inverted. Root cause proven by probe: a bare identifier in a block-form state-child emits `return "someUndeclaredIdentifier";` — a STRING, not a code ref, so the body is scanned as FREE-TEXT, not code-default. `<match>` arms identical. Corpus: **1 of 433** block-form state-children uses the canonical form; **82** use the shorthand. ★★ **Call 2 — demand #1 ANSWERED: (a′) is NOT a new camp, and that STRENGTHENS it.** JSXText's real active set is **5 chars + newline normalization** (Babel 8 `jsxReadToken`), so (a′) is strictly stronger, not equal. But camp membership is the wrong test; the axis is **resolution-freeness**, and **scrml (b) FAILS it by execution** — two files, identical structure, prose differing by ONE apostrophe: one compiles, one raises `E-ENGINE-STATE-CHILD-MISSING`. scrml sits ALONE in a content-classifying middle camp; every shipped markup system (Scribble/HTML5/JSX/Vue/Svelte/Lit) is already in the camp (a′) proposes joining. ⚑ **THE TAXONOMY (the run's sharpest result, from the (a′) pole): Class D = DESYNC** (two scans, different active sets, same bytes) **vs Class C = COLLISION** (one scan, active sequence legitimately in content). **(a′) eliminates Class D by construction and EXACTLY ZERO of Class C.** The compiler labels the class itself — Class D prints `stage: BS`, content errors print `stage: TAB`; **an invented entity name absent from source (`'7'`, `'logic'`, a phantom unclosed `<p>`) is the observable signature of a desync.** ★★ **Call 3 — the whitespace objection is DEAD three ways.** §4.18.5's second regime **does not exist** (emit is byte-verbatim; the collapse is the BROWSER's); Scribble proves a by-construction mode can carry a positional whitespace policy without becoming a classifier; and **scrml does NOT have lit#114** — the space between two adjacent holes survives, because scrml anchors parts to real `<span>` ELEMENTS, not lit's comment markers. ★★ **Call 4 — NOT ONE of five voices argued its own brief; all five recommend (a′)'s MECHANISM; none recommends ratifying (a′) as worded.** ⚑ **THE FREE MOVE, authorizable today and needing NO camp ruling:** delete the `"`/`'`/backtick branches from `engine-statechild-parser.ts:1418-1452` — **~35 LOC, the exact S196 delta** (`match-statechild-parser.ts` `skipMatchComment` IS `skipCommentOrString` with those branches removed). Newly-accepting only; cannot break the corpus; **already licensed by S109 + S196.** ★★ **Call 5 — a CLOSING WINDOW, one line.** Changing §4.18 today changes **ZERO emitted output** because the bridge drops the literal. Once `parse-file.js:316` lands a real AST node, blast radius goes corpus-wide — a phase change, not rework. ⚑ Item's timing premise corrected: **the native parser is ALREADY on the default path in 3 places** (`meta-eval.ts:35/388`, `codegen/emit-match.ts:727/961`, `native-walker/`); `cli.js:64` help text and the README M-ladder are stale. ⚑ **CORRECTS THE ITEM'S OWN UPSTREAM CLAIM — changes a build decision.** The item says if (a′) holds, BOTH peter's apostrophe arc (#862/#865) and `g-splitblocks-consumes-dollar-brace…` (HIGH) are "closed BY CONSTRUCTION and neither fix should be built." **Measured: only the FIRST is.** The apostrophe arc is Class D (closed by (a′)); **`g-splitblocks` is Class C — `${` stays active under EVERY pole and (a′) does NOT close it.** Do not defer that fix on this ruling. ⚑ **THREE NEW UNFILED DEFECTS, file regardless of the ruling:** (1) **`<p>Cost: \${5}</p>` compiles at exit 0, ZERO diagnostics, emits `Cost: \5` — the backslash is content AND the interpolation fires; there is NO `${` escape in a free-text body** (largest unpriced cost of (a′), volunteered by its own advocate). (2) **`<` + SPACE** diverges from HTML5 tag-open (`a < b` cascades; `<3`/`<-`/`<=` are fine) — 17 corpus files, SPEC has zero `tag-open`/`U+003C` text. (3) **the S108 `?{`-in-prose regression** — SPEC §4.17 cites that exact sentence shape as FIXED ("adopters no longer need entity-escapes"); `<p>The ?{ syntax opens SQL.</p>` still EOF-cascades; documented nowhere else. ⚑ **THREE CORRECTIONS TO MY OWN FINDINGS, two from poles arguing against their own interest:** (i) block-splitter **4011 LOC CONFIRMED** but **the growth is NOT in the text/code heuristic — S109 DELETED it and called it "net-negative"**; ~28% is the `<`-overload classifier + attr scanning that **no pole deletes**, so (a)'s surface is *smaller* now than at S111. (ii) "literal implemented zero times" was WRONG — it is implemented once, completely, with 71 test refs, and dropped at the bridge. (iii) the body-scanner census is **4-5 loci, not ~20** (dpa-044's "10 comment sites"/SCOPING's "~20" are the COMMENT count — using them inflates (b)'s tax ~4×); the open brief's "is there a THIRD scanner?" answer is **`native-parser/parse-markup.js`, which hand-rolls string state in 3 places.** ⚑ **(a′)'s `inert` claim WITHDRAWN by its own advocate:** control flow in a plain markup body raises `E-CONTROL-FLOW-IN-MARKUP` **today** (`stage: TAB`) and would **silently render** under (a′) — a real DX regression generalizing the §40.8 hole to all free-text bodies. ⚑ **NO POLE closes the §40.8 `default-logic` hole** — three voices said so unprompted; do not let any pole claim it. ⚑ **METHODOLOGICAL, and it governs every corpus number here (JSX voice):** odd-apostrophe prose DOES NOT COMPILE, the corpus compiles, **so the corpus is BY CONSTRUCTION free of the cases that prove the heuristic bites** — the filter is the COMPILER, not just the LLM author population. **§4.18.2's own frequency rationale is unmeasurable in principle on a green corpus**, which cuts against (b), whose foundation it is. **MEASURED BY EXECUTION at HEAD `0d8d7eac`** (item framed at `4d057a58`): tracked corpus **2518** (naive `find` reports **266,514** — 99% `.claude/` artifacts) · apostrophe files **454 (18%)** · live-risk ∩ engine/match = **1 file, the bug's own reproducer** · interpolating templates **640/262 files** (item said 630/262 — files exact) · two-mode diagnostics **11 of 1099**, with **0 conformance cases** for either §4.18 author-facing rule · corpus is **68% test fixtures** with **897 asserted-output oracles** (you cannot regenerate an oracle). **NOT RATIFIED — RUN-not-RATIFY.** | banked S405-bryan · round 1 drained 2026-09-07 · round 2 re-banked S405 · round 2 drained 2026-09-08 (advisory) · queue row lost in `fd69d1fc`, reconciled by the dPA 2026-09-12 · bryan's call outstanding on both rounds |
| dpa-041 | **COMPLETE (ADVISORY) dPA 2026-09-05 — awaiting bryan; FOUR calls, §8.** ⚑ Call 1 BLOCKING — the question cannot be answered as framed: at a BOUND site lift mode is already hard-coded by CONSTRUCT KIND at context creation (`emit-logic.ts:4839` array / `:4755` `:5266` single); the outward scan + its §7392 tie-breaker live in STATEMENT position (`:4442/:4446/:4488/:4491`), which has no binding site and therefore no declared type. Type-directed selection targets the deterministic case and cannot reach the ambiguous one. ⚑ Call 2: an annotation at these sites is a NET LOSS OF INFORMATION — `!letAnnot` (`type-system.ts:10573`) suppresses `W-TYPE-031-UNPROVEN` while `E-TYPE-031` (`:10446`) covers only {number,string,boolean} vs a literal; no assignability relation exists anywhere (grep: 0 matches). ⚑ Call 3: 4/4 poles — the silent-wrong is INDEPENDENT of this proposal (repoints `var`, not `mode`); sever and ship the `:4891` guard. ⚑ Call 4: `docs/changes/s397-tilde-one-or-two/progress.md` is a DANGLING POINTER cited by the master-list banner. All 5 PA claims reproduce; §10.2 cite corrected to §10.4/`:7097` (+ a 4th uncited scan statement at `:7067`). → `scrml-support/docs/deep-dives/lift-mode-declared-type-dpa-041-2026-09-05.md` | dPA advisory — PA to review |
| dpa-042 | **COMPLETE (ADVISORY) dPA 2026-09-05 — awaiting bryan; FOUR calls, §9.** ⚑ Call 1 OUTRANKS the banked question — SPEC §17.6.6's OWN canonical example compiles GREEN (exit 0, zero diagnostics, `node --check` clean) and emits JS that throws `ReferenceError`; an in-repo fixture already asserts the missing `E-TILDE-001`. ⚑ Call 2: §35.8's "the full `lin` rule set applies to `~`" is FALSE ON THE MERITS (orchestrator-verified) — `TildeTracker:18380` is a one-cell init/consume state machine (definite assignment + single introduction) while `lin`'s three codes count ELIMINATIONS; not one is about what `~` enforces. ⚑ Call 3: the non-cosmetic unification is one WALK + one JOIN parameterized by a lattice (3/4 poles converge) — NOT the rule set; delete both `~` joins (`:18855` fail-open, `:18942` fail-closed), reify the producer at PARSE. ⚑ Call 4: the two live verdicts — distinct-but-shared-engine vs retire-and-desugar-to-`lin` — are INCOMPATIBLE (a 1:1 desugar changes the error set); yours. Zero-producers CONFIRMED exhaustively; guard-deletion DISPROVEN by in-memory execution (byte-identical, 6/6). §32.6 loop inversion convergent with dpa-040 Call 3. → `scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md` | **RATIFIED S409 2026-09-08 — Call 4 CLOSED** (bryan: *"I think stale read is a unnecessary foot gun if the cost is reasonable, and it seems to be"*) — **option (c) is distinct-engine with a NARROWED rule set**, which answers Call 4's incompatible pair (*distinct-but-shared-engine* vs *retire-and-desugar-to-`lin`*): **not a desugar.** Calls 1-3 stand as advisory findings and are unaffected. Grounds: user-voice S409. artifact `scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md` |
...
| dpa-044 | **COMPLETE (ADVISORY) dPA 2026-09-06 — awaiting bryan; FOUR calls, §7.** ⚑ **Call 1 OUTRANKS the banked question** — silent consume-to-EOF is UNIVERSAL (the tokenizer does it too, `readBlockComment` tokenizer.ts:1366; **no `E-*UNTERMINATED*` diagnostic exists anywhere in the compiler**), so making it a DIAGNOSTIC removes NO surface, costs NO migration, and closes all four members. ★★ **The discriminator is FALSE as framed** and the hazard is a FOUR-member class — `/* */` · `<!-- -->` (§27.2, a THIRD comment form the item never mentions, 35 corpus files) · strings · backticks. **Measured unpaired openers: `/*` 0 · `<!--` 0 · string 7 · backtick 1** — the proposal removes the two forms with ZERO instances and keeps the two with EIGHT (prose apostrophes = the S402 unlandable arc). **The sole HIGH is triggered by `//` at first significant char — bryan's exact SURVIVING form** — and its second shape drops a statement with no comment at all (§40.8 `default-logic` root). Restriction still worth taking (≤3.6% migration; deletes `urlSlashesAt` + collapses 3 line scanners), but as a blast-radius cap, not as the defect fix. ⚑ Item's headline **"22 files" does not reproduce — it is 10** at its own commit. ⚑ **This row did not exist until the dPA added it** — the item was banked with no table row, so `dpa-debt` read `0 UNRUN`; third witnessed obligation-vs-probe mismatch (§8). | **RATIFIED S405 2026-09-07 — CALL 1 ONLY** (bryan: *"dpa-044 call 1"*). Unterminated delimiters become a DIAGNOSTIC. ⛑ **SCOPE NOT RULED and the build is split:** PA-verified by execution that `<p>it's ready</p>` compiles CLEAN in a plain-markup body and only breaks NESTED IN A STATE-CHILD — so the diagnostic in a free-text body would FALSE-POSITIVE on ordinary prose and would newly assert a delimiter role S109 denied and dpa-045 reopens. **CODE-position limb buildable now; markup-body limb GATED ON dpa-045.** Calls 2-4 + the `//`-restriction NOT ruled |
| dpa-045 | **COMPLETE (ADVISORY) dPA 2026-09-08 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call.** ROUND 1 (ADVISORY) 2026-09-07; **ROUND 2 (ADVISORY) 2026-09-08 — both advisory, neither ratified.** ★★ **Call 1 OUTRANKS the banked question:** round 1's "free move" shipped SIX HOURS after round 1 (`e523478b`) and its own commit says **"IT WAS NOT A FREE MOVE"** — four rounds, five HIGHs, every one silent behaviour loss at exit 0, because the string branches were **accidentally shielding four opener-blind flat scanners**. It did **not** delete the Class D apostrophe family — it **MOVED it one layer down into `findOpenerEnd` and WIDENED its reach by five scan paths.** ★★ **Q1 ANSWERED — the wording EXISTS**, drafted (§4.18.1/.1a/.1b + the §4.18.5 deletion) with **six implementer pins**; pin 4 (cooked-or-raw to codegen) was named by nobody until the cost voice found **two shipped modules already disagreeing**. ★★ **Q2 — none of the three sinks (a′):** (a) is a **CREDIT** (a 1-of-5-recall regex with **4 live false fires**; 12-24h to keep at warning tier, and it halves ruling 3) · (b) is **routing, not design** (10-20h + 25-45h for the `\"` half) · (c) is **ORTHOGONAL** (§40.8 is logic-default; (a′) never reaches it). **The real unpriced cost is an 80-200h scanner migration priced at n=1.** ★★ **Q3 — Class C is ORTHOGONAL to the camp, confirmed twice by measurement.** New result: **delimiter-escapes stay bounded; character-escapes are Class D machinery.** ★★ **Q4 — the M6/MD clock is DEAD** (M5.2 deferred 98-180h; **M6.7's flip REVERTED at 845 failures**; ~270 sessions stationary). **The real window is `findOpenerEnd`, its unit is SESSIONS, and it moved the wrong way 10 days ago.** ⚑ **THREE VOICES CONVERGED INDEPENDENTLY on the next action: the LOOP CENSUS, 4-8h, not wasted under any ruling.** **TWO round-1 calls WITHDRAWN BY THEIR OWN AUTHORS** ("implemented once" → twice; "closing window" → withdrawn); **two dPA claims corrected by the panel.** | artifact `scrml-support/docs/debates/plain-markup-text-as-string-dpa-045-round2-2026-09-08.md` · awaiting bryan |
| dpa-045 | **COMPLETE (ADVISORY) dPA 2026-09-08 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call.** ⚑ **ROW REPAIRED BY THE dPA 2026-09-15 — it led with `BANKED — UNRUN` for seven days after round 2 had actually run.** Round 2 drained 2026-09-08 → `scrml-support/docs/debates/plain-markup-text-as-string-dpa-045-round2-2026-09-08.md` (47 KB, committed `c92534e`+`891ffca`, delta-log breadcrumb `[2987]` landed) — but **step 5(b) never wrote the table**, so `dpa-debt` reported a completed AXIOM-LEVEL deliberation as *needing firing* and the next dPA boot was aimed at re-running it. **This is the contract's own named failure (S347) recurring on the very item that documented it.** Round 2's verdict: the WORDING EXISTS and is drafted; none of the three costs sinks (a′); Class C is orthogonal to the camp; the M6/MD window is DEAD (the real one is `findOpenerEnd`, its unit is SESSIONS). Round 1 text retained below. — bryan: *"go another round for the dpa"*. ROUND 1 COMPLETE (ADVISORY) 2026-09-07, five calls, verdict retained in the body entry. ⚑ **Round 2 exists because round 1 converged on the MECHANISM and rejected the WORDING:** not one of five voices argued its own brief, all five recommend (a′)'s mechanism, **none recommends ratifying (a′) as worded.** Round 2 asks what the ratifiable form IS, and prices the three costs round 1 surfaced but did not close. **COMPLETE (ADVISORY) dPA 2026-09-07 — awaiting bryan; AXIOM-LEVEL, ladder row 7, his non-delegable call. FIVE calls.** ★★ **Call 1 OUTRANKS the banked question.** The item asks whether to EXTEND §4.18's by-construction answer from the 3 code-bearing loci to plain markup. **Measured: §4.18's display-text literal is implemented ONCE, completely, in the native parser and DROPPED at `native-parser/parse-file.js:316`** (`DisplayTextLiteral` → `synthTextNode(…, source)` — "verbatim literal source", quotes included). It reaches emit in **1 of 3 loci** (the `:`-shorthand). **The spec-correct form renders WITH visible quotes; the spec-rejected form renders correctly** — polarity inverted. Root cause proven by probe: a bare identifier in a block-form state-child emits `return "someUndeclaredIdentifier";` — a STRING, not a code ref, so the body is scanned as FREE-TEXT, not code-default. `<match>` arms identical. Corpus: **1 of 433** block-form state-children uses the canonical form; **82** use the shorthand. ★★ **Call 2 — demand #1 ANSWERED: (a′) is NOT a new camp, and that STRENGTHENS it.** JSXText's real active set is **5 chars + newline normalization** (Babel 8 `jsxReadToken`), so (a′) is strictly stronger, not equal. But camp membership is the wrong test; the axis is **resolution-freeness**, and **scrml (b) FAILS it by execution** — two files, identical structure, prose differing by ONE apostrophe: one compiles, one raises `E-ENGINE-STATE-CHILD-MISSING`. scrml sits ALONE in a content-classifying middle camp; every shipped markup system (Scribble/HTML5/JSX/Vue/Svelte/Lit) is already in the camp (a′) proposes joining. ⚑ **THE TAXONOMY (the run's sharpest result, from the (a′) pole): Class D = DESYNC** (two scans, different active sets, same bytes) **vs Class C = COLLISION** (one scan, active sequence legitimately in content). **(a′) eliminates Class D by construction and EXACTLY ZERO of Class C.** The compiler labels the class itself — Class D prints `stage: BS`, content errors print `stage: TAB`; **an invented entity name absent from source (`'7'`, `'logic'`, a phantom unclosed `<p>`) is the observable signature of a desync.** ★★ **Call 3 — the whitespace objection is DEAD three ways.** §4.18.5's second regime **does not exist** (emit is byte-verbatim; the collapse is the BROWSER's); Scribble proves a by-construction mode can carry a positional whitespace policy without becoming a classifier; and **scrml does NOT have lit#114** — the space between two adjacent holes survives, because scrml anchors parts to real `<span>` ELEMENTS, not lit's comment markers. ★★ **Call 4 — NOT ONE of five voices argued its own brief; all five recommend (a′)'s MECHANISM; none recommends ratifying (a′) as worded.** ⚑ **THE FREE MOVE, authorizable today and needing NO camp ruling:** delete the `"`/`'`/backtick branches from `engine-statechild-parser.ts:1418-1452` — **~35 LOC, the exact S196 delta** (`match-statechild-parser.ts` `skipMatchComment` IS `skipCommentOrString` with those branches removed). Newly-accepting only; cannot break the corpus; **already licensed by S109 + S196.** ★★ **Call 5 — a CLOSING WINDOW, one line.** Changing §4.18 today changes **ZERO emitted output** because the bridge drops the literal. Once `parse-file.js:316` lands a real AST node, blast radius goes corpus-wide — a phase change, not rework. ⚑ Item's timing premise corrected: **the native parser is ALREADY on the default path in 3 places** (`meta-eval.ts:35/388`, `codegen/emit-match.ts:727/961`, `native-walker/`); `cli.js:64` help text and the README M-ladder are stale. ⚑ **CORRECTS THE ITEM'S OWN UPSTREAM CLAIM — changes a build decision.** The item says if (a′) holds, BOTH peter's apostrophe arc (#862/#865) and `g-splitblocks-consumes-dollar-brace…` (HIGH) are "closed BY CONSTRUCTION and neither fix should be built." **Measured: only the FIRST is.** The apostrophe arc is Class D (closed by (a′)); **`g-splitblocks` is Class C — `${` stays active under EVERY pole and (a′) does NOT close it.** Do not defer that fix on this ruling. ⚑ **THREE NEW UNFILED DEFECTS, file regardless of the ruling:** (1) **`<p>Cost: \${5}</p>` compiles at exit 0, ZERO diagnostics, emits `Cost: \5` — the backslash is content AND the interpolation fires; there is NO `${` escape in a free-text body** (largest unpriced cost of (a′), volunteered by its own advocate). (2) **`<` + SPACE** diverges from HTML5 tag-open (`a < b` cascades; `<3`/`<-`/`<=` are fine) — 17 corpus files, SPEC has zero `tag-open`/`U+003C` text. (3) **the S108 `?{`-in-prose regression** — SPEC §4.17 cites that exact sentence shape as FIXED ("adopters no longer need entity-escapes"); `<p>The ?{ syntax opens SQL.</p>` still EOF-cascades; documented nowhere else. ⚑ **THREE CORRECTIONS TO MY OWN FINDINGS, two from poles arguing against their own interest:** (i) block-splitter **4011 LOC CONFIRMED** but **the growth is NOT in the text/code heuristic — S109 DELETED it and called it "net-negative"**; ~28% is the `<`-overload classifier + attr scanning that **no pole deletes**, so (a)'s surface is *smaller* now than at S111. (ii) "literal implemented zero times" was WRONG — it is implemented once, completely, with 71 test refs, and dropped at the bridge. (iii) the body-scanner census is **4-5 loci, not ~20** (dpa-044's "10 comment sites"/SCOPING's "~20" are the COMMENT count — using them inflates (b)'s tax ~4×); the open brief's "is there a THIRD scanner?" answer is **`native-parser/parse-markup.js`, which hand-rolls string state in 3 places.** ⚑ **(a′)'s `inert` claim WITHDRAWN by its own advocate:** control flow in a plain markup body raises `E-CONTROL-FLOW-IN-MARKUP` **today** (`stage: TAB`) and would **silently render** under (a′) — a real DX regression generalizing the §40.8 hole to all free-text bodies. ⚑ **NO POLE closes the §40.8 `default-logic` hole** — three voices said so unprompted; do not let any pole claim it. ⚑ **METHODOLOGICAL, and it governs every corpus number here (JSX voice):** odd-apostrophe prose DOES NOT COMPILE, the corpus compiles, **so the corpus is BY CONSTRUCTION free of the cases that prove the heuristic bites** — the filter is the COMPILER, not just the LLM author population. **§4.18.2's own frequency rationale is unmeasurable in principle on a green corpus**, which cuts against (b), whose foundation it is. **MEASURED BY EXECUTION at HEAD `0d8d7eac`** (item framed at `4d057a58`): tracked corpus **2518** (naive `find` reports **266,514** — 99% `.claude/` artifacts) · apostrophe files **454 (18%)** · live-risk ∩ engine/match = **1 file, the bug's own reproducer** · interpolating templates **640/262 files** (item said 630/262 — files exact) · two-mode diagnostics **11 of 1099**, with **0 conformance cases** for either §4.18 author-facing rule · corpus is **68% test fixtures** with **897 asserted-output oracles** (you cannot regenerate an oracle). **NOT RATIFIED — RUN-not-RATIFY.** | banked S405-bryan · round 1 drained 2026-09-07 · **round 2 drained 2026-09-08 (advisory; bryan's call outstanding on BOTH rounds)** · table row repaired by the dPA 2026-09-15 |
...
| dpa-047 | **COMPLETE (ADVISORY) dPA 2026-09-19 — calls 1 + 3 RULED by bryan S422; CALL 2 and the residuals still owed.** ⚑ *The drain wrote, committed and PUSHED both artifacts and then flipped neither status site nor dropped a breadcrumb, so this row read `BANKED — UNRUN` while a completed deliberation sat on origin — the `[3394]` class (dpa-045, repaired 2026-09-15) recurring on the very next drain, and worse: that one at least left a delta-log entry.* ★★ **Call 2 is not a design question at all — `E-ASSIGN-004` at statement position is ALREADY normatively mandated (§50.8.5, verbatim: "applies equally to statement-form") and has NEVER fired**, so the compiler emits `E-CODEGEN-INVALID-LOGIC` ("This is a compiler defect. Please report it.") on the exact shape bryan described, while the SPEC's own remediation text already reads *"Use `let`."* The ruling needed is **build it + fix §34's row**, ~1h. ★★ **Call 1 — the SPEC is SILENT at statement position and NO prior ruling exists.** A wider ledger sweep (design-insights, deep-dives, SPEC, and ALL user-voice files, not just `user-voice-scrml.md`) confirms bryan's recalled ruling is not on the record. **S418 ruling 1 goes the OTHER way and rests on a MIS-SCOPED citation** — §50.8.4 and §50.1 are EXPRESSION-position only, so ruling 1 widened past its own governing sentence. ⚑ **DO NOT BUILD `E-ASSIGN-003` as ruled; narrow it to expression position.** ★★ **Call 3 cost MEASURED: 195 bindings / 138 files as a FLOOR, ≥15.4 percent of those rejections provably wrong**, 21.3 percent-dense in `examples/`, ~3x tail in the self-host tree; the nested-container false-positive family (`?{}`, `each in=`, `for`-header, `_{}`) must be fixed FIRST. ⚑ **S418's "INERT — nothing migrates" NO LONGER HOLDS.** → `scrml-support/docs/deep-dives/bare-assignment-const-and-unused-binding-dpa-047-2026-09-19.md` **PRIOR BANK:** ⚑ **REOPENS S418 RULING 1 (`E-ASSIGN-003` fires).** bryan, on being shown the compiled code: *"I thought at some point I said that bare naming like this defaults to const, and if you want to mutate a variable, it had to be let."* PA-VERIFIED the implementation already does limb 1 — a fresh statement-position bare assignment emits `const count = args.length;`. ★★ **AND RULING 1 IS MIS-SCOPED:** §50.8.4's own trigger is *"an identifier that has not been declared appears on the left-hand side of an **assignment expression**"*, and §50.1 calls §50 *"a narrow, precisely scoped feature"* for **expression** position. S418 quoted §50.3.5 as the governing sentence; that sentence lives inside that narrow scope and **does not reach statement position** — the case that emits `const`. Building ruling 1 as ruled would be a WIDENING past its own governing sentence, not the conformance restoration it was ruled as. Three limbs to rule. **DO NOT BUILD RULING 1 UNTIL THIS DRAINS.** | **bryan** — REOPENS S418 ruling 1; banked S422 2026-09-18; drained 2026-09-19 as an **R2 deliberation** and filed to `deep-dives/` not `debates/` (the item carried no expert/forge list); calls 1+3 ruled by bryan S422 in `user-voice-scrml.md` and the PA owes the formal row here; **call 2 + residuals OPEN** |
| dpa-048 | **COMPLETE (ADVISORY) dPA 2026-09-19 — awaiting bryan; AXIOM-ADJACENT (§32), ladder row 7; MUST NOT be ruled in the same pass as dpa-047 (no-batch-axioms floor).** ★★ **THE ITEM'S CENTRAL ARGUMENT IS INVERTED BY THE SPEC IT CITES.** The bank argued `:int = expr` *"composes with the S397 axiom rather than fighting it"*; **§32.2 says the opposite twice, and the second is a SHALL NOT** — `SPEC.md:19392` *"Declarations do not initialize `~`"* and `:19398` *"A variable declaration … SHALL NOT initialize `~`."* `:int = expr` **is** a declaration, so the proposal needs either a **THIRD `~`-initialization source** (§32.2 admits exactly two — an unassigned expression statement, and `lift`) or an explicit **carve-out from a standing SHALL NOT**. Either is a §32 amendment, not a free composition. ★★ **The quoted S397 axiom is NOT spec text** — `grep "ONE thing"` over `SPEC.md` returns 1 hit, §51.0.S, about engine transitions. ★★ **The proposed shape does not compile, and fails as a COMPILER DEFECT rather than a parse error.** ★★ **A scaffolding correction that binds dpa-047 too: there is no §0 in `SPEC.md`** — both items cite "§0's own banner"; §32's status banner lives inside §32.2.1. ★★ **dpa-040 overlap CHECKED and it is CLOSED** (S409 — the arm-body boundary question dissolved), so nothing blocks this on that axis. **TWO NEW DEFECTS surfaced, in no queue item.** → `scrml-support/docs/deep-dives/anonymous-typed-binding-tilde-readback-dpa-048-2026-09-19.md` **PRIOR BANK:** The anonymous typed binding: bryan, verbatim, sketching the throwaway-value shape — *"if I had my druthers this would look like `:int = args.length` / `_{ console.log("args: " + ~ ) }`"*. An unnamed typed declaration whose value is read back as `~`. Reads consistent with the S397 axiom (*`~` is ONE thing — the value of the preceding unbound expression statement*) and would moot the unused-binding question for throwaways by construction, since there is no name to go unread. AXIOM-ADJACENT — touches §32, which §0's own banner says is half spec-ahead with a REVERTED read path (S395 landed / S397 reverted; in-arm `~` reads are conditional today). Banked SEPARATELY from dpa-047 per the no-batch-axioms floor. | **bryan** — AXIOM-ADJACENT (§32); banked S422 2026-09-18; dpa-040 overlap checked (closed S409); drained 2026-09-19; **must not be ruled in the same pass as dpa-047**; nothing ruled yet |
| dpa-049 | **COMPLETE (ADVISORY) dPA 2026-09-24 — awaiting bryan; one fork ROUTED (wire a CLI/toml lint flag at all? — touches S422 call 3).** ★★ **THE BANK'S HOME FOR IT DOES NOT EXIST:** §58 has no settings slot (closed 5-kind enum), `build-story.lock` is repo-only, `chunks.json` is emitted only under `--emit-per-route` and lacks §62.4 `language`; **no CLI/`scrml.toml` path to ANY `lint.*` flag exists** (JS API only, 2 unit tests); **only 1 of §28's 4 rows is wired, and a 5th wired knob has no row.** ★★ **A suppression changes ZERO emitted bytes** → no honest taint; record DESCRIPTIVELY in diagnostic output (3/3). ★★ **"Don't ship this" belongs to BEHAVIOUR-changing conditions** — and `scrml build` (the "production-ready" command) **ships `log()` that §20.6.8 says SHALL strip** (probe-verified). → `scrml-support/docs/deep-dives/project-wide-lint-suppression-taint-dpa-049-2026-09-24.md` **PRIOR BANK:**  ⚑ **A GENERAL BUILD-INTEGRITY MECHANISM, not a lint question** — banked separately from dpa-047 call 3 for exactly that reason. bryan, verbatim: *"is there a way that we can compile into development mode only when an unreccomended flag is set true? … at least the compiler can say 'don't ship this' … if this is in the wild, it should not be considered production ready scrml."* ★★ **IT ALREADY HAS A HOME:** §58's build story is `compile(source, buildStory) → artifact` with a **mandatory human-inspectable `build-story.lock` sidecar**, and a suppression IS a build input by construction; `chunks.json` already carries `compiler` + `language` identity fields that travel with the artifact (a non-strippable channel); `dev.js`/`build.js` already split the modes. §58 is **Nominal/spec-ahead — nothing is built.** ★★ **THE TENSION TO RULE ON:** call 3 went lint-first SPECIFICALLY so the ~30 false positives are not blockers. If suppressing a lint taints the build, an adopter hitting them must choose between noise and a tainted artifact — **partially re-creating what lint-first solved.** ★★ **Five options, weakest→strongest:** (1) nothing · (2) RECORD which lints were suppressed in the build-story/`chunks.json` — descriptive, reversible, nearly free given §58 · (3) taint the artifact, `scrml build --target` warns/refuses · (4) disclaim in the wild — ⚑ **output comments are STRIPPABLE by any minifier**; only a runtime banner or `chunks.json` survives · (5) refuse the flag entirely (Zig: *"Zig will never have a 'sloppy mode' flag"*) — but §28 already ships four such flags, so removal is itself breaking. ★★ **Precedent is real at the strong end:** Android `debuggable=true` is rejected by the Play Store; dev-signed iOS cannot ship; Chrome shows an unsupported-flag banner **to the end user**. **PA lean: (2)** — you can tighten a record into a refusal; walking back "we called your build unsafe" is harder. | **bryan** — banked S422 2026-09-19; governs EVERY suppression scrml ships, not just the unused-binding lint; drained 2026-09-24 as an R2 deliberation (3-voice panel); ⚑ §28's "four existing flags" premise measured FALSE (1 wired, 3 phantom, 1 unlisted); nothing ruled |
| dpa-050 | **READY TO RUN — handed to the dPA by bryan S430 ("yes, hand it to the dPA, retirement pole closed"). Q1–Q5 RULED in conversation (stress-test them, do NOT reopen); RUN Q6 (instance self-write ⇒ engine/component convergence). Retirement pole CLOSED.** *Banked S430 2026-09-24.* ⚑ **AXIOM-LEVEL, ladder row 7 — ONE-AT-A-TIME; it decides what a DECLARATION IS, and it must land before the bootstrap writes a line.** bryan, verbatim: *"we either really justify the <> syntax, or get rid of it."* His original model (S430): `<swatch label:string hex:string> = "Brand", "#338967"` — the tag declares the cell's SHAPE (typed user attributes + stdlib attributes), the RHS installs defaults; `<swatch/>` in MARKUP renders the markup associated with it; `<swatch>` in LOGIC returns the data as a struct; and it serves imported COMPONENT and THEME libraries. ★★ **Q1 (the fork everything else hangs on): is a declaration ONE VALUE (a singleton cell) or a TEMPLATE (each markup use an instance with its own attribute values)?** His two uses pull opposite ways — "`<swatch>` in logic returns the data" is singleton; "`<swatch hex=…/>` from a library" is instance. It collides with Move 20 / S178 (engines singleton, components multi-instance, "do not collapse") and with the abandoned `< Card>`-declares / `<Card>`-instantiates spacing distinction. ★ Later Qs: where the associated markup is written · named vs positional defaults (PA: positional is fragile — reorder silently swaps same-typed values) · attributes (scalar fields) vs children (nested state) · access forms (`<swatch>` vs `@swatch` in logic) · relationship to structs §14, Shape-2 render specs §6.2, components §15, `<theme>` §65. **Justify-or-remove is a legitimate outcome** — a keyword form (`state x = …`) is the retirement pole and must be argued as a first-class option, not a strawman. | **bryan** — banked S430 ("bank it and lets discuss"); ground every call in FULL worked programs (an app, a component library, a theme library) in each shape, never snippets; read user-voice S54/S55 (the state-steals-markup's-syntax thread), S130 (modifiers inside the tag), S178 + Move 20, dpa-022 (RATIFIED S352 — markup is a state kind that renders) FIRST |
| dpa-051 | **BANKED — UNRUN (S430 2026-09-24). ⚑ DEPENDS ON dpa-050 Q6.** The bootstrap CODEGEN ARCHITECTURE design doc — the first bootstrap step, ratified S430 as a design doc BEFORE any bootstrap code (bryan S425: *"starting to write code, when you are still forming the picture means you end up with a prototype, forever stuck"*). Must decide: the IR between AST and emitters; how scope / declared names are CARRIED by structure (this session's lost-write + TDZ bug family came from hand-threaded names); every body — incl. `!{}` handler arms and value-form match arms — reaches every stage as a TREE (5 S430 fixes that reasoned over text failed review); a compile-scoped parser handed to emitters (the 7-file PARSE_REENTRY gap); per-emitter "done" under the P5 hybrid gate; and what a dpa-050 declaration compiles to. Evidence base: S428's 37-plumbing-defect aggregation, the S430 review record (docs/pr-reviews.md #1042–#1048), flogence's 104-loop census (dpa-045 Call C). | **bryan** — ratified S430 as part of the dpa-050 plan; run after dpa-050 Q6 rules |
...
## [dpa-037] deep-dive — the non-finite float class: NaN and Infinity have no normative home, and `==` is not reflexive because of it

`status: complete` (dPA 2026-08-31, ADVISORY — NOT ratified) · banked S390-bryan (2026-08-31)
· **artifact:** `scrml-support/docs/deep-dives/non-finite-float-class-dpa-037-2026-08-31.md`
· **verdict:** 6/6 poles back a reflexive `==`, but the forged IEEE-754 voice (polled adversarially) STRUCK the panel's map-codec premise (it implements clause 5.10 `totalOrder`, which is SUPPOSED to disagree with `==`) and RAISED the scope — `==` alone yields `a == b` true while `a <= b` is false, matching none of 754's four outcomes, so the unit of repair is the comparison FAMILY (Postgres fixed a whole opclass; Java shipped a whole second relation). The argument that survives is new: Kahan's `NaN != NaN` rationale is explicitly conditional on a language LACKING `isNaN` — scrml ships one. ⚑ bryan's `1/0` lean is CONTRADICTED: 754 splits three ways and `1/0` (divideByZero) is EXACT and the least NaN-like of the four measured expressions; the `-0`-supports-#3 argument is unsound and inverted. Corpus measured for the first time: ~10 real division sites, 0 `**`, 0 `toNumber` call sites, plus one live shipped defect (`formatBytes(-5)` renders `"NaN undefined"`). #2 ≈ 20-45h vs #3 240-490h. · **rung: R2 minimum, axiom-ADJACENT — run SEPARATELY, do not batch** (it touches what a scrml *value* is and whether `==` is reflexive; the no-batch-ratify floor applies).

### ⚑ NOTHING IN THIS ITEM IS RATIFIED. bryan's position is a WORKING TAKE and he said so.

bryan, verbatim (S390), thinking aloud:

> **NaN and Infinity are NOT absence values** … *"what we have here 1/0 ***IS*** not a number"* … *"and then Inf well…"*

and then, immediately and unprompted:

> **"ok hold on I am not ratifying NaN! TBC"**

⚑ **The PA initially banked the first quote as a RULED premise and a floor for this DD. That was wrong and bryan corrected it inside the same turn.** *"My take is this"* is not a ratification, and the S276/S130 durable (a terse answer adopts the full text it answers) applies to an ANSWER — it does not convert thinking-aloud into a ruling. **Recording the error here because a dPA run against the original wording would have treated the absence question as settled and never examined it.**

**Operating instruction for whoever runs this:**

- **EVERY limb is live, including "make `not` absorb NaN."** Score it. Do not drop it because bryan leans against it.
- **His lean, carried as a lean and worth arguing WITH rather than around:** `not` means *there is no value here*; NaN means *there is a value here and it is the result of an undefined operation* — so collapsing them may be LOSSY, destroying the distinction between *the row was not there* and *the arithmetic failed*. That is the strongest form of his position; the DD should test it, not assume it.
- **NaN may be honestly named** — an impossible calculation genuinely did not produce a number. Also a lean, also testable.
- **Infinity is the most open of all** — he trailed off mid-sentence. The runtime already treats it differently from NaN (below), which is independent evidence for splitting them rather than ruling them as one class.
- **TBC means the conversation is unfinished.** If the DD's answer depends on which way bryan goes, say so and stop rather than picking for him.

### ⭐ THE SHARPENING bryan supplied next — and it narrows the DD considerably

> **"I am saying, not a number is still something, as is infinity"**

This is **not a new axiom — it is §42.1.1 extended one step**, and stating it that way is what makes it tractable. §42.1.1 (S89, ratified) already holds that `""` / `0` / `false` / `[]` / `{}` are **DEFINED VALUES, not absence** — you HAVE something; it is just empty. bryan is placing NaN and Infinity on that same side of the line: *you have something; it is just not a finite number.*

Two consequences the DD should start from rather than discover:

**1. The absence axis may already be CORRECT in the implementation.** PA-measured: `is some` reports NaN as **present**, `is not` as **absent-false**. Under this framing that is the right answer and needs no change. If the DD confirms it, the absence limb closes as *conformant already* rather than as a build.

**2. It converts the `==` defect from murky to crisp, and this is the payoff.** If NaN is a defined value, then it is **the ONLY defined value in scrml that does not equal itself**:

```
"" == ""              -> true
0 == 0                -> true
[] == []              -> true   (structural)
Infinity == Infinity  -> true   (measured)
NaN == NaN            -> FALSE  (measured)
```

Every other defined value satisfies reflexivity. One does not, and nothing in §45 says so. That is no longer "what is NaN philosophically" — it is **an invariant the language keeps everywhere except one place**, which is a far more answerable question and is probably rulable ahead of the rest of the class.

⚑ Still bryan's TAKE, still TBC. Recorded here because the framing changes what the DD has to prove, not because it settles it.

### ⭐⭐ THE THIRD SHARPENING — bryan splits the class, and it may be the DD's actual spine

> **"infinity and NaN are different. inf is either: 1 proof of a value js cant express or calc. 2 intended (rare, but Im sure we could think of a use case) but NaN is just an error as far as I can figure. I could be wrong."**

**This is a different claim from the second sharpening and the DD must not blur them.** *"NaN is still something"* places it outside absence. *"NaN is just an error"* places it outside the VALUE DOMAIN ALTOGETHER. Both can hold — NaN is not `not`, and it is also not a number — but they point at different builds.

⭐ **The reading that makes it scrml-shaped, and the DD should test it as a first-class pole:** scrml already HAS a failure domain that is not the value domain — `fail` / `!` / `!{}` / errors-as-states (§19). If NaN is an error, then **IEEE-754 smuggled a failure into the value domain and scrml has somewhere better to put it.** An operation that cannot produce a number would `fail` with a typed variant; the caller discharges it at the call site and routes it into a state, exactly as every other failure in the language does. It also explains the reflexivity anomaly without special-casing anything: **NaN never was a value, so of course it does not behave like one.**

⚑ **THE COUNTER-PRESSURE THE DD OWES THIS POLE — do not let it in unopposed:**

1. **COLORING.** Making arithmetic failable colors every numeric expression. bryan rejected `async`/`await` in exactly these words (S114): *"I hate leaky abstractions and colored functions."* A `!`-typed `/` is that objection aimed at the most common operator in the language. **The DD must answer this or the pole dies.** The strongest available reply is that the compiler owns the plumbing (the §19.9.3 body-split precedent) and that a §53 refinement — a denominator statically known non-zero — DISCHARGES the failure at compile time, so ordinary arithmetic stays uncolored and only genuinely-unproven division pays. **That is a by-construction answer versus a retrofit lint, i.e. the S322 re-examination test applied to this class.**
2. **NOT ALL NaN COMES FROM A CHECKABLE OPERATION.** `toNumber("abc") → NaN` is documented and deliberate (§41.18). A parse that fails is a different shape from an arithmetic that fails, and the stdlib currently answers it with a sentinel. If NaN is an error, `toNumber`'s signature is wrong too — and that is a stdlib surface change with its own blast radius.
3. **IEEE-754 HAS A REASON.** NaN propagates so numeric pipelines need not branch. That reason is much weaker in a UI language than in numerics, but the DD should say so rather than ignore it.

⭐ **AND bryan's Infinity case-1 SPLITS FURTHER — a refinement of his own point, not a contradiction:**

- **overflow** (`1e308 * 10`) — a real finite value exists and the float cannot hold it. This is his case 1 exactly, and it is arguably *also* an error: a silent precision catastrophe, not a value.
- **`1 / 0`** — no value exists at all; the limit diverges, and it is `-Infinity` from the left. This looks far more like NaN's situation than like overflow.

So *Infinity* may itself be two things, and the tidy split may not be NaN-vs-Infinity but **failure-vs-saturation cutting across both.** The measured runtime asymmetry (`Infinity == Infinity` true, `NaN == NaN` false) is evidence for a split; it is not evidence for *that particular* split.

- **case 2 (intended Infinity)** — real: an unbounded sentinel (`min`-seeding, "no limit"). The DD should ask whether scrml spells that `Infinity` or reaches for a refinement/`not`, and whether one rare idiom should influence the class at all.

⚑ **Still TBC. bryan's own words: *"I could be wrong."* He has invited the counter — the DD's job is to supply it properly, not to ratify his instinct back to him.**

### [EXEC] MEASURED BY THE PA AT `952cecc6` — do not re-derive, verify only if you doubt it

Compiled `0/0`, `1/0`, `(0-1)**0.5`, `1e308*10` in derived cells:

| expression | result | renders into the DOM as | diagnostic |
|---|---|---|---|
| `0 / 0` | `NaN` | the literal text `NaN` | **none** |
| `1 / 0` | `Infinity` | `Infinity` | **none** |
| `(0 - 1) ** 0.5` | `NaN` | `NaN` | **none** |
| `1e308 * 10` | `Infinity` | `Infinity` | **none** |

**Exit 0, zero diagnostics.** Arithmetic is straight IEEE-754 host passthrough.

⭐ **AND THE EQUALITY RELATION IS NOT REFLEXIVE.** `==` lowers to `_scrml_structural_eq` (`runtime-template.js:4097`), whose first line is `if (a === b) return true;` and whose scalar path is `return a === b`. Executed directly against that function:

```
_scrml_structural_eq(NaN, NaN)           => false
_scrml_structural_eq(Infinity, Infinity) => true
_scrml_structural_eq(0, -0)              => true
```

**`@x == @x` is FALSE when `@x` is NaN.** Nobody decided this — it is inherited IEEE-754 semantics arriving through `===` on line 4098. ⚑ Note the asymmetry: **Infinity behaves like a value under `==`; NaN does not.** That is independent evidence for bryan's instinct that the two are different things.

⚑ **The irony is load-bearing and the DD should say so plainly:** §45.6 asserts *"There is no identity comparison operator in scrml … Exposing identity comparison as a language operator would be a JS abstraction leak."* The equality scrml DOES expose leaks IEEE-754 NaN semantics through the front door, in the same section that refuses a leak through the side.

### [EXEC] THE SPEC IS SILENT — this is Rule 4 outcome 2, which makes it a RULING, not a bug fix

Searched: **§45 (Equality) — 0 mentions of NaN. §42 (absence) — 0 mentions.** Whole-SPEC total: **6 mentions across 37,549 lines**, every one incidental:

- `SPEC:24392` §41.18 — `toNumber("abc") → NaN`, documented as the coercion result.
- `SPEC:24393` — `isNaN` shipped in `scrml:math` (`Number.isNaN`, non-coercing), with tests in `stdlib/math/index.scrml:159`.
- `SPEC:5875` + `:31331` §51.12.3.1 — **negative/NaN durations CLAMPED TO ZERO by the timer runtime.**
- `SPEC:21457` — a malformed `channel-reconnect=` (incl. NaN) silently falls back to 2000ms.
- `SPEC:15524` — `isNaN`/`isFinite` listed in the JS-stdlib appendix.

⭐ **The tell: the language ALREADY takes a position on NaN in exactly one place — durations — and nowhere else.** So NaN was never considered-and-permitted at the language level; one surface hit the problem and fixed it locally. There is no `E-`/`W-` code for it anywhere.

### The questions the DD owes (given the premise is fixed)

1. **Is `==` reflexivity a separable bug?** `@x == @x` being false is arguably wrong regardless of how the class is classified — a `structural` equality that is not reflexive is broken on its own terms. Options: special-case NaN to equal itself (diverges from JS, matches SameValueZero, what `Map`/`Set`/`Array.includes` already do); keep IEEE-754 (a leak §45.6 forbids in principle); or refuse `==` where an operand can be non-finite. ⚑ **PA lean: this is smaller than the rest of the class and should probably be ruled FIRST and separately** — but that is a scheduling opinion, not a finding.
2. **Does Infinity belong in the same class as NaN at all?** The runtime already says no. Saturation-vs-failure is a real distinction: `1/0` diverged; `0/0` is undefined.
3. **Is silent DOM rendering of `NaN` acceptable?** It is the silent-wrong-output class the project treats as HIGH everywhere else — a user sees `NaN` in the UI, exit 0, no diagnostic. Whether the answer is a lint, a refinement type, or nothing is the question.
4. **Does the type system want to say this?** scrml already has machinery for *a value that is not yet what it claims to be* — refinement predicates (§53) and the `(not to T)` lifecycle (§14.12). A `number` that can be NaN is arguably not a `number`; a refined *finite* number is expressible with existing primitives. This is the by-construction limb against the retrofit limb (a lint), which is the S322 re-examination test applied to this class.
5. **What does `is some` / `is not` report for NaN?** Measured: NaN is a DEFINED value, so `is some` is true. That is CORRECT under bryan's premise and should be confirmed as intended, not changed.

### Do NOT

- Do not treat bryan's take as ruled — see the banner. It is explicitly TBC.
- Do not propose a fifth absence spelling.
- Do not batch this with dpa-022/023 — they are the markup and async axioms; this is the numeric-value axiom, and the no-batch floor binds all three.
- Do not treat the §51.12.3.1 duration clamp as precedent for a language-wide rule; it is a local fix on one surface and the DD should say whether it should generalise.

**Origin:** surfaced mid-session while bryan was editing the "If You Give a Dev an Enum" draft; he asked *"if scrml does an impossible math calculation, what do we get back. does it error, NaN?"* and then supplied the premise above. **Routes to: scrml PA. RUN-not-RATIFY.**

---

## [dpa-038] DD — Offline / PWA: is a cold offline boot native scrml someday, or the host-JS boundary by design?

`status:    ratified`  # RATIFIED S405 2026-09-07 (bryan: "dpa-038 close it") — closed as duplicative of dpa-028; #509 answered + closed with a PA-verified correction to the ratified precache recipe; `g-chunks-json-names-chunk-urls-that-are-never-written` filed HIGH.

⚑ **Banked because naming it was not enough.** S346 recorded that Direction issues named at every boot sat 3-7 days unowned; this one sat **21**. A deliberation banked outside this file was never banked at all (S319) — so it lives here, not in a hand-off.

### Scope-lock

The adopter's design is deliberately thin and mostly already-native: append-only per person (no CRDT), a `client_id` UUID + a UNIQUE column for idempotent replay through existing server-fns, `localStorage` persistence via `when @var changes`, and the §20.8 persistent shell to precache. **The one piece with no native story is the COLD OFFLINE BOOT** — service-worker + Cache-API shell + manifest, so the app loads with zero network.

Three questions, verbatim:
1. Is native offline something scrml could/should grow eventually, or is SW/manifest/static-asset serving the intended **host-JS boundary** for an app to own? (They are not asking us to build it now — they are asking whether to treat it as "coming someday" or "mine to hand-roll.")
2. Was the 2026-07-05 BaaS-parity **"clear skip"** a permanent design call, or deprioritised under the bug wave?
3. Is there a pattern in 6NZ to point at?

### ⚑ Q3 IS ALREADY ANSWERED BY FACT — PA-VERIFIED, do not re-derive

The adopter writes: *"6NZ is 'an offline-first PWA written entirely in scrml.' If there's a pattern there — how it registers its SW and serves `sw.js` / `manifest.json` as static assets — a pointer would save me building blind."*

**There is no pattern to point at.** PA-verified on the `6nz` clone (2026-09-01): **zero** service-worker files, **zero** `manifest.json` / `.webmanifest`, and **zero** `serviceWorker` references in any `.scrml` / `.js` / `.html`. The phrase appears once, in `6nz/README.md:22`, inside a **design-goal feature list** (*"viewport, NeoVim superset + mouse, CM6 + canvas overlay, offline-first PWA"*) — aspirational, not shipped. **Telling them this is itself worth the return leg**: it stops them hunting for a phantom, and it is a doc-currency defect in 6nz's own README.

### Load-bearing constraints (do not let the DD drift past these)

- **This is a boundary question, not a feature request.** The adopter explicitly says they will ship ~150 lines of quarantined host-JS (a service worker + a write-queue) and *"rip it back out the moment there's a native path"* — so the cost of answering "host-escape, permanently" is low and honest, and the cost of answering "coming someday" without meaning it is high.
- **`_{}` foreign code is sliver-empty** (0 source-level uses, PRIMER §13.5). A SW is the first plausible real consumer — worth asking whether that changes the §23 picture.
- The **corpus-is-artifact kernel applies in reverse**: zero corpus offline apps is blast radius, not demand evidence (S346). The real question is whether apps do this and whether scrml can express it.
- Rule 2: do NOT reach for *"users won't notice"* / *"ship the smaller surface."*

### Do NOT

- Do not answer Q1 by pointing at 6NZ. See above — the pattern does not exist.
- Do not batch with dpa-039; they are different boundaries (client shell vs server egress) and the no-batch floor is about axioms, but these two also have genuinely different answers.
- Do not treat "we skipped it on 2026-07-05" as a ruling. Q2 asks precisely whether that skip was a design call or a queue artifact, and the DD must go read WHY it was skipped rather than restate that it was.

---

## [dpa-039] DD — Enterprise document workflows: is non-JSON egress + inbound upload native, or `handle()` by design?

`status:    complete`  # banked → running → complete → ratified(by PA) · COMPLETE dPA 2026-09-03 (ADVISORY) → DD written, NOT ratified. **VERDICT: duplicative of dpa-029/030 in QUESTION, but a third of the factual base MOVED — and one move invalidates dpa-030's pending ratification** (*"(a) was never a live option"* is now false; `handle()`+`formData()` WORKS). ★★ The `protect=` leak reproduces ~40 sessions on and is **larger than recorded — ordinary function extraction defeats it**, so the member-chain-walk repair is dead. ★★ TWO new silent fail-opens: the default CSRF arm's missing `instanceof Response` guard, and an EMPTY tenant set for `<schema>`-only apps (`parseSchemaBlock` is DSL-only — NOT the S288 regression it looks like). §60.7 resolved AGAINST the refusal (constitutive, not adjacent). Phase 1: premise 1 PARTIAL, premise 2 FALSIFIED-as-stated-but-leak-real, premise 3 VERIFIED. · **origin:** GitHub issue **#471** (adopter, opened 2026-08-08) · banked S393-bryan 2026-09-01 · artifact `scrml-support/docs/deep-dives/document-workflows-egress-upload-dpa-039-2026-09-03.md`

⚑ **This is the adopter bryan calls the enterprise proving ground**, asking about customer-facing delivery: bids/quotes emailed, blendsheets printed at the bulk plant, job packets printed + emailed, unit/service reports.

### Scope-lock — five questions, and they are NOT independent

1. **Response envelope is JSON-locked.** A server fn / `<endpoint>` can only return JSON; `return new Response(...)` in a server fn is reported as `E-SCOPE-001`, and `<endpoint>` forces `application/json`. The only binary/`Content-Disposition` path is `handle()` returning a raw Bun Response. → **Is a chosen-Content-Type response path from a server fn or `<endpoint>` on the roadmap, or is `handle()` the long-term answer?**
2. **`handle()` sits OUTSIDE the tenant / `protect=` egress guarantees** (`E-PROTECT-004` / `E-TENANT-RAW-EGRESS`). Since every document/email path must ride `handle()`, the delivery layer cannot inherit scrml's data-protection invariants. → **Should document/attachment egress come inside the protect/tenant envelope natively?**
3. **No inbound multipart / file-upload parser.** `<endpoint accepts=:enum>` decodes JSON only; uploads need `handle()` + `request.formData()`. → **Native inbound-upload primitive, or host-escape by design?**
4. **Document generation (HTML→PDF) + templating** — no `renderToString` / server HTML render; `format` is data-only. → **Vendored-library case, or a render primitive of interest?**
5. **Email** — they read our stance as injected `sendEmail` / `scrml:http` → provider, deliberately not native. → **Confirm that is the blessed long-term pattern.**

### ⚑ THE PREMISES ARE ADOPTER-SURVEYED AND ARE **NOT** PA-VERIFIED

The adopter says *"We surveyed the source before asking"* and states 1-3 as fact. **This project's own record is that relayed premises fail about one in three** ([[feedback_relayed_premises_fail_at_one_in_three]]). **The DD's FIRST phase is to verify each of 1/2/3 by execution** — compile a server fn returning `new Response(...)` and read the actual code; compile an `<endpoint>` and read the emitted Content-Type; check whether `E-PROTECT-004` / `E-TENANT-RAW-EGRESS` genuinely have no `handle()` path. **If any premise is wrong the question changes shape**, and answering the stated version would be answering a question nobody has.

### Load-bearing constraints

- **Q2 is the load-bearing one and it is a SECURITY-shaped question, not an ergonomics one.** It is the same family as the S385 confidentiality cluster ruling: *"any expression position that reaches a server-only binding is treated exactly as a function body reaching one … Silence is never an option at this boundary."* An egress path that cannot inherit `protect=` is a hole of that class, and the cluster ruling's own logic may already decide it.
- **§60/§61 precedent binds.** `<api>` (outbound) and `<endpoint>` (inbound) both LIMIT deliberately — §60.7 explicitly refuses retry/cache/pagination. Whatever is decided here must say why binary egress is or is not the same kind of limit.
- **Q3 (upload) and Q2 (egress) may be one question**: both are "the typed boundary handles JSON and nothing else." Check whether they collapse before designing two mechanisms (the S385 four-fixes-vs-one-policy shape).
- **`adopter:` provenance is the undeliberated band** (base Rule 4b). A real consumer needing it is a recorded reason, not a ratification — this DD is what converts it.

### Do NOT

- Do not accept "we'd vendor a PDF lib via the `.js` sidecar" (Q4) as settling Q4. That is what they will do ANYWAY; the question is whether scrml wants a story.
- Do not answer Q5 by restating the stance. They are asking for confirmation it is LONG-TERM, which is a commitment, not a description.
- Do not let Q1 be answered as "use `handle()`" without answering Q2, because Q2 is the reason `handle()` is unsatisfying.

---

## [dpa-040] deep-dive — should an if-as-expression ARM BODY be a `~` context boundary in its own right?

`status:    complete`  # banked → running → complete → ratified(by PA) · COMPLETE dPA 2026-09-03 (ADVISORY) → DD written, NOT ratified. **VERDICT: not answerable as banked — `E-TILDE` fires 0/6 on the SPEC's own examples; root cause TRACED (dead node kinds + a DELIBERATE `name === "~"` exclusion contradicting §32.3/§35.8).** ★★ Blast radius measured on the wrong axis: **~828 consecutive bare-call runs vs 17 arm sites** — §32.2's SHALL is unconditional, so the conflict is not about arms. ★★ The substructural pole argues the boundary is **UNSOUND** (an opacity boundary at the arm manufactures an un-consumable `~`) — if it holds, limb (c) is REJECTED, not deferred. Sharp disagreement with the anonymity derivation is STATED, not adjudicated. ★★ §32.6's loop elision is inverted and makes §35.8 unsatisfiable. Rec: fail-closed codegen floor → honour §35.8 → warning tier → rule. · **origin:** the S395 `~` ruling; banked BY that ruling · banked S395-bryan 2026-09-02 · artifact `scrml-support/docs/deep-dives/tilde-arm-body-boundary-dpa-040-2026-09-03.md`

⚑ **This is limb (c) of a three-limb fork bryan ruled on 2026-09-02, and it was banked BY the ruling itself.** He answered *"your rec"* to *"(a) now, with (c) named as the question behind it"* — so per the S276/S130 durable the ratification adopted **both halves**: limb (a) governs today, and **(c) is explicitly NOT closed.** Do not treat the S395 landing as having settled this.

### The question

§32.4 makes `~` non-crossing at exactly two boundaries: a `${}` logic context, and a function body. **Should an if-as-expression arm body be a third?**

Under (c) there is nothing to arbitrate between §17.6.2 and §32.2: a bare call inside an arm fills the *arm's own* `~` slot, `lift` designates the *arm's* result through a different mechanism, and the two sections cannot contradict each other **by construction** rather than by carve-out.

### Why it was not just ruled at S395

The ruling took (a) — §17.6.2 governs, a bare call in an arm is a side effect — because it is specific-over-general, fixes the root, and restores a shape SPEC ships a worked example for. **But (a) is a CARVE-OUT: it settles the conflict by amending §32.2 to exempt arm bodies.** (c) would remove the conflict's possibility instead. That difference is the whole question, and it is squarely the S322 re-examination test — *retrofit vs by construction* — applied to `~`.

### ⚑ THE EVIDENCE THAT MAKES THIS URGENT — the enforcement does not fire

**Six probes on merged main, including SPEC §32.5's own verbatim `${ process(~) }` and §17.6.6's partial-`if` example, produce ZERO `E-TILDE` diagnostics.** Not from absence: `TildeTracker`, `checkLinear`, and both `E-TILDE-001`/`E-TILDE-002` exist in `type-system.ts` with real messages, and `checkLinear` is invoked unconditionally at file level (`:24470`). It runs; it does not fire on the documented shapes. **Cause NOT traced** — filed as `g-tilde-lin-enforcement-does-not-fire-on-spec-own-examples`.

**That silence is why a live contradiction between two normative sections survived unnoticed.** A DD on (c) that does not account for it is reasoning about a rule nothing enforces.

### Load-bearing constraints (do not let the DD drift past these)

- **`~` is a `lin` variable (§32.3) and that is not on the table.** The question is its BOUNDARY SET, not its exactly-once semantics.
- **Direction matters:** adding a boundary NARROWS `~`'s reach — the LIMIT direction, fork-rule row 1, which favours it. But it is also `semantics-changed` for any existing program that relied on `~` crossing an arm boundary, so it owes a MEASURED migration, not an assumed-zero one.
- **The S395 blast-radius measurement is a floor, not an answer:** the bare-expr-in-arm shape is **corpus-zero** (of 5 bound-position if-as-expression sites, the only one with an intermediate statement uses a `let`-decl, which does not rebind). Per bryan's own S346 ruling, corpus-zero is **blast radius ONLY, never demand evidence** — and this corpus is 100% LLM-authored, so it carries almost no ergonomic signal. Do not close (c) on corpus silence.
- **Phase 1 MUST re-verify the E-TILDE non-firing by execution** before reasoning from it. It is PA-measured but the cause is untraced, and a DD that inherits an untraced claim is the laundering shape.
...
status: complete   # dPA 2026-09-05 — ADVISORY, NOT ratified. Verdict: the question cannot be answered as framed — at a bound site mode is hard-coded by construct kind; the ambiguous case is statement position, which has no binding site. Artifact: scrml-support/docs/deep-dives/lift-mode-declared-type-dpa-041-2026-09-05.md
...
status: complete   # dPA 2026-09-05 — ADVISORY, NOT ratified. Verdict: share the WALK and the JOIN, not the rule set — §35.8 is false on the merits; and a live green-compile ReferenceError outranks the banked question. Artifact: scrml-support/docs/deep-dives/tilde-lin-unification-producer-gap-dpa-042-2026-09-05.md
...
status: complete   # RAN dPA 2026-09-06 as a SINGLE-ITEM batch (per the S399/2026-09-05 staging note). ADVISORY — NOT ratified.
# artifact: scrml-support/docs/debates/lazy-pull-yield-emit-primitive-dpa-043-2026-09-06.md
# verdict: THE BANKED QUESTION IS FALSE AS FRAMED — scrml has had normative lazy pull since SPEC.md:2898 (§6.6.3 "Evaluation Strategy —
#   Lazy Pull with Dirty Flags"), predating §13.6; the real fork is whether an ITERATOR-shaped lazy pull is ALSO needed. A and C rejected
#   3/3 (the lexical-boundary premise defeats §7040 but never reaches §13.6, which already legalizes generators inside ${} — so A
#   RECREATES the footgun one level down). Live fork is B vs D, panel split 1/1/1; the lazy-pull pole was briefed to argue A/C and
#   REFUSED. Orchestrator-measured correction: scrml's SSE lowering has NO backpressure (start() not pull(); desiredSize 0x in
#   compiler/src) — the surface justifying §13.6 does not implement the capability that would justify lazy/pull. UNANIMOUS floor: all
#   three poles demand lift-vs-yield be adjudicated IN WRITING in the SPEC whatever the ruling. bryan rules (AXIOM-LEVEL, ladder row 7).
...
`status:    ratified (Call 1 only)`  # RATIFIED S405 2026-09-07 (bryan: "dpa-044 call 1") — unterminated delimiters become a diagnostic. SCOPE SPLIT: code-position limb buildable now; markup-free-text-body limb GATED ON dpa-045 (a diagnostic there would false-positive on prose apostrophes — PA-verified). Calls 2-4 NOT ruled.
banked:    S402 2026-09-06 (bryan: "bank the dpa")
...
`status:    complete (round 2)`  # ROUND 1 complete (ADVISORY) 2026-09-07. **ROUND 2 COMPLETE (ADVISORY) dPA 2026-09-08 — NOT ratified.** Artifact: `scrml-support/docs/debates/plain-markup-text-as-string-dpa-045-round2-2026-09-08.md`. **VERDICT: the wording EXISTS (drafted, + six implementer pins); none of the three costs sinks (a′) — (a) is a CREDIT, (b) is routing not design, (c) is ORTHOGONAL; Class C is orthogonal to the camp, confirmed twice; and the M6/MD window is DEAD — the real one is `findOpenerEnd`, its unit is SESSIONS, and S405 WIDENED it.** ★★ Call 1 OUTRANKS the banked question: round 1's "free move" shipped SIX HOURS after round 1 (`e523478b`) and its own commit says **"IT WAS NOT A FREE MOVE"** — four rounds, five HIGHs, all silent behaviour loss at exit 0, because the string branches were **accidentally shielding four opener-blind scanners**; it did not delete the Class D apostrophe family, it **MOVED it one layer down and widened its reach**. ★★ §4.18 is implemented **TWICE** (`emit-match.ts:647` is live on the default path) so round 1's *"changing §4.18 changes zero emitted output"* is FALSE — **82 of 83 sites move through codegen**. ★★ §4.18.3's normative escape catalog is **1-for-3 at emit**, from **two root causes, neither a camp question**. ★★ **THREE VOICES INDEPENDENTLY CONVERGED on the next action: run the LOOP CENSUS (4-8h)** — the largest unpriced cost (80-200h scanner migration) is priced at n=1. **TWO round-1 calls WITHDRAWN BY THEIR OWN AUTHORS; two dPA claims corrected by the panel.**
`status:    complete (ADVISORY, both rounds)`  # R1 2026-09-07 → `…/plain-markup-text-as-string-dpa-045-2026-09-07.md`;
#          R2 2026-09-08 → `…/plain-markup-text-as-string-dpa-045-round2-2026-09-08.md`. NEITHER ratified — bryan's call.
#          ⚑ The round-2 drain flipped neither this line nor the table row; repaired by the dPA 2026-09-15.
...
### ⚑ ROUND 2 — banked S405 2026-09-07 (bryan: "go another round for the dpa")

**Round 1 did not fail — it converged and then declined to ratify its own convergence.**
Not one of five voices argued its assigned brief; all five recommend **(a′)'s mechanism**;
**none recommends ratifying (a′) as worded.** That is a wording gap, not a direction gap, and it
is what round 2 is for. **Start from round 1's CORRECTED base** — it overturned three of the
item's own premises and three of its own findings; do not re-derive them, and do not re-run the
camp comparison, the whitespace objection, or the prior-art survey. Those are CLOSED.

**Q1 — THE LOAD-BEARING ONE. What is the ratifiable wording?** Every pole endorsed the
mechanism and refused the text. Produce the normative sentence(s) a §4.18 amendment would
actually carry, or state that no wording survives and the mechanism must ship under a
different frame. A recommendation nobody will sign is not a recommendation.

**Q2 — Price the three costs round 1 surfaced and left open.** (a) The `inert` claim was
**WITHDRAWN by (a′)'s own advocate**: `E-CONTROL-FLOW-IN-MARKUP` fires today and would
**silently render** under (a′) — a DX regression generalizing the §40.8 hole to every free-text
body. (b) **There is no `${` escape in a free-text body at all** — `<p>Cost: \${5}</p>` emits
`Cost: \5` at exit 0; round 1 called this *the largest unpriced cost of (a′)*, volunteered by its
own advocate. (c) **No pole closes the §40.8 `default-logic` hole** — three said so unprompted.
**Does any of the three sink (a′), or are they buildable alongside it?**

**Q3 — Class C is untouched and that was not the item's premise.** Round 1's taxonomy has
(a′) eliminating **Class D (desync) by construction and EXACTLY ZERO of Class C (collision)**.
`${` stays active under every pole. **So what is the Class C answer**, and does having one
change which camp is right? If the answer is "nothing — collisions are inherent and every
system has them", say that plainly; it is a legitimate result and it narrows the ruling.

**Q4 — the closing window is a claim about SEQUENCING; test it.** Round 1: changing §4.18
today changes **zero emitted output** because the literal is dropped at `parse-file.js:316`,
and once that bridge lands a real AST node the blast radius goes corpus-wide — *a phase change,
not rework*. **Is that true, and how much time is actually left?** If the window is real it
dominates the cost side of the ruling; if it is not, the ruling can wait.

**OUT OF SCOPE for round 2:** the free move (`engine-statechild-parser` string branches) —
**bryan authorized it and it is BUILDING now**, licensed by S109 + S196 and needing no camp
...
`status:    complete (ADVISORY)`  # dPA 2026-09-15 → `scrml-support/docs/deep-dives/undo-tree-vs-arc-graph-dpa-046-2026-09-15.md`
#          verdict: the item CANNOT BE RULED AS FRAMED — its crux is falsified in the source (--untouch DELETEs,
#          --move DELETEs+re-adds; only `utterance` is append-only), its "exactly ONE cursor" premise is false
#          (gcursor holds THREE rows — the ONE-STORE-TWO-CURSORS pole was BUILT here and RETIRED at S39, leaving
#          two rotted rows), and it names no problem the undo-tree prevents. Return leg is ONE question: which
#          nouns get versioned — (a) text buffers, (b) repo changes [already substantially built: vcs_save/
#          project_vcs/giti-mount slice 3], or (c) the graph itself. ⚑ dPA-MEASURED: the panel's Q2 cost number
#          (4 of 6 voices) is WRONG BY 3 ORDERS — 1M edges/depth 1M walks in 1.73s; the real ceiling is the
#          `cross` self-join in --path (entity degree), which did not finish at 10k nodes in ten minutes.
...
## [dpa-047] deliberation — Statement-position bare assignment: is it a `const` declaration? And does unused-binding bind every binding form?
`status:    complete`  # banked S422 2026-09-18 → drained 2026-09-19 (ADVISORY, dPA-produced). Artifact: `scrml-support/docs/deep-dives/bare-assignment-const-and-unused-binding-dpa-047-2026-09-19.md`. VERDICT: call 2 is a CONFORMANCE VIOLATION, not a design question — §50.8.5 already mandates `E-ASSIGN-004` at statement position and it has never fired; call 1 is genuinely unruled (wider ledger sweep) and S418 ruling 1 is mis-scoped to expression position; call 3 cost measured at 195 bindings / 138 files. ⚑ Calls 1+3 RULED by bryan S422; call 2 + residuals OPEN. ⚑ DO NOT BUILD E-ASSIGN-003 AS RULED — narrow it to expression position.
banked:     S422 2026-09-18 (bryan: "yes. lets reopen.")

### Origin — bryan, verbatim, S422

Shown four compiled examples of bare assignment, he replied:

> I don't really understand the point of it all, i guess. in the first example the "never read" itself
> should be a compiler error, wether it is a const or a let. Also it dosn't mutate as far as I can see.
> I thought at some point I said that bare naming like this defaults to const, and if you want to mutate
> a variable, it had to be let. so "count = ar.length \n log(count) // OK" "... count+=1 // error use
> let." if we are going to allow variables that the compiler cant track (at runtime).

### ⚑ Why this reopens S418 ruling 1 — the governing sentence does not reach the case

S418 ruled `E-ASSIGN-003` FIRES, as an Error, citing §50.3.5. **PA-verified at S422 that §50.3.5 does
not govern the shape in question.**

- **§50.8.4** (E-ASSIGN-003's own definition) — *"**Trigger:** An identifier that has not been declared
  appears on the left-hand side of an **assignment expression**."* Its Note: *"Undeclared identifiers
  in **expression-position assignment** would otherwise create implicit globals."*
- **§50.1** — *"a bare assignment (`x = value`) MAY appear in any **expression** position… This is a
  **narrow, precisely scoped** feature."*

So E-ASSIGN-003 is **expression-position only**. A bare *statement* `count = args.length` is outside it.
Building ruling 1 to fire there is a **widening past its own governing sentence**, and the direction
classification S418 recorded (conformance restoration, newly-rejecting-toward-the-contract) does not
hold for the statement case.

⚑ **This also explains S418's own finding that §50.8.4's rationale is "factually wrong".** It is not
wrong — it is correct about EXPRESSION position, where an undeclared target genuinely leaks a global.
At STATEMENT position the builder emits a block-scoped `const`. **One code, two positions, two
behaviours, and no one had separated them.**

### The evidence — all PA-EXECUTED at S422 on `787d4cb4`, with controls

| source (inside a `function` body) | result today |
|---|---|
| `count = args.length` · `return count` | **exit 0**, emits `const count = args.length;` |
| `count = args.length` · `count = count + 1` · `return count` | exit 1 `E-CODEGEN-INVALID-LOGIC` ("This is a compiler defect. Please report it.") |
| `count = args.length` (never read) | exit 1 `E-MU-001` |
| `count = args.length` · read ONLY inside `_{ … }` | exit 1 `E-MU-001` — ⚑ **FALSE POSITIVE**, the value IS used |
| `let spare = args.length` (never read) | exit 0 clean |
| `const spare = args.length` (never read) | exit 0 clean |

Three-way control on the foreign-block case: read in plain scrml → clean; read in both → clean; read
ONLY in `_{}` → fires. The `_{}` interior is opaque (§23.2.3) so the must-use tracker never sees it.
⚑ This lands hardest on `kind="tool"`, because §64.2 routes host I/O through bare `_{}` — a CLI that
computes a value and logs it is the likeliest way to hit it.

### The three calls

**Call 1 — is a statement-position bare `name = expr` a `const` DECLARATION?** The implementation says
yes and the SPEC is SILENT there (§50 covers expression position only). bryan's stated intent says yes.
If yes: write it normatively, and ruling 1 narrows to expression position where it belongs. If no:
ruling 1 stands as ruled and the emitter's `const` lowering is the defect.
⚑ **Search the ledger before ruling** — the PA swept `user-voice-scrml.md` at S422 for
`bare naming|keywordless|defaults to const|must be let|reassign` and found **no prior ruling**. bryan
recalls making one. Absence of a hit is not proof; sweep wider (design-insights, deep-dives, SPEC
history) before treating this as unruled.

**Call 2 — is the mutation error `E-ASSIGN-004`?** bryan wants `count += 1` after a bare-const to say
*"use let"*. **`E-ASSIGN-004` — "Assignment to `const` Variable" — already exists and S418 measured it
at ZERO producers.** So the clean diagnostic he describes is a code we already have and never fire.
Today the author gets `E-CODEGEN-INVALID-LOGIC` blaming the compiler for their own source.

**Call 3 — does unused-binding bind EVERY binding form?** bryan: *"the 'never read' itself should be a
compiler error, wether it is a const or a let."* Today only bare assignment fires (`E-MU-001`); `let`
and `const` are clean. ⚑ **This INVERTS the PA's S422 recommendation to retire E-MU-001** — under this
model the code is not wrong for existing, it is wrong for being scoped to one binding form, and the
move is to GENERALIZE. It also kills the `_`-prefix opt-out question: an escape hatch hollows out a
rule that is meant to bind broadly.
**Cost to name:** generalizing is newly-REJECTING (reversible direction), but the migration is
**UNMEASURED** and unused locals are common in real code. **Measure by COMPILING the corpus, never by a
text scan** — the S418 text scan for this same family returned 795 candidates and was worthless.

### Rides with it, and is independent of every call above

The six §34 / message defects on `E-MU-001` (S418 ruling 4, ratified): §34's row describes
`E-ERROR-002`'s rule · its actual rule is specified nowhere · its section pointer says §35 which never
mentions must-use · the code's own `§48.3.3` ×6 citation is `E-FN-003`'s rule · the message says
"suppress this warning" at Error severity · **and (NEW, S422) the `_`-prefix opt-out the message
advertises does not exist** — verified by execution and by reading `MustUseTracker`
(`type-system.ts:18569`), which has no underscore handling in `declare()`, `unusedEntries()` or at the
fire site. Those corrections are worth doing whichever way the three calls land.

⚑ **The `tilde-decl` name is itself misleading and nearly misled the PA.** `TildeDeclNode`'s own doc
reads *"A tilde declaration: bare `name = expr` (no keyword). Declares a ~-typed must-use variable."*
It has NOTHING to do with §32's `~` sigil — that is `tilde-init`/`tilde-ref`, the kinds §0's banner
says have zero producers. Renaming is in scope for whoever builds this.

---

## [dpa-048] deliberation — The anonymous typed binding: `:int = expr` read back as `~`
`status:    complete`  # banked S422 2026-09-18 → drained 2026-09-19 (ADVISORY, dPA-produced). Artifact: `scrml-support/docs/deep-dives/anonymous-typed-binding-tilde-readback-dpa-048-2026-09-19.md`. VERDICT: the item's "composes with the S397 axiom" premise is INVERTED by §32.2 (`SPEC.md:19398` — a variable declaration SHALL NOT initialize `~`), so the proposal is a §32 AMENDMENT, not a free composition; the quoted axiom is not spec text; the shape does not compile and fails as a compiler defect. AXIOM-ADJACENT — must NOT be ruled in the same pass as dpa-047. dpa-040 overlap checked: closed S409.
banked:     S422 2026-09-18 (bryan: "yes. lets reopen." — this sketch given in the same message)

### Origin — bryan, verbatim, S422

On the third of four compiled examples (a value computed and consumed only inside a `_{}` host block):

> and the 3rd example, if I had my druthers this would look like ":int = args.length\n_{ console.log("args: " + ~ ) }"  just sayin

### What is being proposed

A declaration with a **type and no name**, whose value is read back through `~`:
...
instead of inventing a name that exists only to be consumed once.

### Why it is worth a real deliberation rather than a reply

- **It composes with the S397 axiom rather than fighting it.** S397 ruled *`~` is ONE thing — the value
  of the preceding unbound expression statement.* A `:int = expr` statement is exactly an unbound
  expression statement with a declared type, so `~` reading it needs no new meaning for `~`.
- **It moots the unused-binding question for throwaways by construction** — no name, so nothing can go
  unread. That is a by-construction answer where dpa-047's call 3 is a retrofit one, and §0's own
  re-examination test says by-construction is the signal to look for.
- **It is a widening of the language surface**, and under the S322 stance a widening CAN be right — the
  question is whether it is the best expression of the intent, not whether it adds surface.

### What it has to survive

- **§32 is half spec-ahead.** §0's banner: the WRITE half is implemented, the READ half ("a `~` read
  inside an arm resolves in the ENCLOSING context") was landed S395 and **REVERTED S397** because the
  codegen pre-scan it needs also gates accumulator allocation for sibling statements and produced a
  block-scope escape (`ReferenceError` at exit 0). Any proposal that makes `~` reads more load-bearing
  inherits that unbuilt machinery. **dpa-040 already banks the arm-body-as-§32.4-boundary question** —
  check it for overlap before running this.
- **Type-position grammar.** `:int = expr` puts a bare type annotation in statement-leading position,
  which currently has no production. Does it collide with the `:`-shorthand body form (§4.14), which
  also uses a leading `:` inside an opener?
- **Scope and lifetime.** How far does the binding reach — the next statement only, the enclosing
...
`status:    complete`  # banked S422 2026-09-19 → drained 2026-09-24 (ADVISORY, dPA-produced). Artifact: `scrml-support/docs/deep-dives/project-wide-lint-suppression-taint-dpa-049-2026-09-24.md`. VERDICT: not as banked — a lint suppression changes ZERO emitted bytes, so no honest taint exists (and no shipped toolchain marks an artifact for a suppressed lint); record DESCRIPTIVELY in DIAGNOSTIC OUTPUT, not build-story.lock/chunks.json (§58 has no settings slot; the lock never leaves the repo; chunks.json is opt-in). No CLI/scrml.toml path to ANY lint.* flag exists (only the JS API, 2 unit tests) — whether to wire one is ROUTED to bryan (touches S422 call 3). ⚑ bryan's "don't ship this" is real for BEHAVIOUR-changing conditions: `scrml build` ships log() that §20.6.8 says SHALL strip (probe-verified). Nothing ruled.
...
## [dpa-050] deep-dive — Justify the `<name>` declaration syntax, or remove it. What IS a declaration?
`status:    ready`  # S430 2026-09-24. AXIOM-LEVEL. Q1–Q5 RULED in conversation (see list); dPA: stress-test Q1–Q5 in full programs + RUN Q6; retirement pole CLOSED by bryan.
banked:     S430 2026-09-24 (bryan: "bank it and lets discuss")

### Origin — bryan, verbatim, S430

> I was actually thinking that this would be a great time to discuss some things about the language that
> don't quite make sense anymore. mostly the reactive cell declaration syntax. I had a whole plan for why
> the < > syntax was chosen for declaration. but that never got built, so now <thing> = value is a little
> strange.

> we can discuss further but this still never actually touches on the original syntax I had in mind
> <thing userDefParam:sting stdLibParam> = "hi", "#338967; or something to that effect. My thingking was
> that you could install the defafault values on the rhs. I am not saying this is the shape that I want.
> Just that we either really  justify the <> syntax, or get rid of it.

> my original thought was that when <swatch was in markup it would renter the markup associated with it.
> <swatch>, when called in a logic context would return a struct of the data. I was also thinking this
> would be useful for imported component and theme librarys

### The ground — PA-verified S430 by reading the record + SPEC

- **What `<count> = 0` does today:** the brackets mark ONLY "this line declares state". After the
  declaration `<count>` never reappears for a Shape-1 cell — every read/write is `@count`; `<count/>` in
  markup is `E-CELL-NO-RENDER-SPEC`. The tag form earns its shape only for compound state (Variant C
  children) and Shape-2 bound inputs (`<name req> = <input/>` + `<name/>`).
- **Already built — the stdlib-attribute half of bryan's model:** validators (`<name req length(>=2)>`),
  `default=`, `debounced=`/`throttled=`, `server`, `pinned` all live INSIDE the tag (S130 ruled
  modifiers-inside-the-tag). **Never built:** user-defined typed attributes, RHS-installed defaults,
  render-the-associated-markup for a general cell, logic-context `<x>` returning the struct.
- **The S54/S55 record** (user-voice ~4088–4494): "state steals markup's syntax; markup is a subset of
  state"; `<span>LIVES <lives> = 3</>` decl-at-render-position; `< Card>` declares a state type /
  `<Card>` instantiates; HTML elements are predefined state types.
- **dpa-022 RATIFIED S352:** markup is a STATE KIND that renders, not a value type state holds
  (reconciliation, canon migrates new-or-touched).
- **Collides with:** Move 20 + S178 (engines = singleton typed global store; components = multi-instance;
  "components and engines are distinct, do not collapse"; `E-COMPONENT-ENGINE-SCOPE`); components as
  `const Card = <div props={…}>` (§15); structs + Tier-3 positional sugar `<u>: T = ("a", 30)` (§5/§14);
  `<theme>` (§65 — already a theme mechanism).
...
4. **Q4 RULED (S430, bryan): *"A, attributes for data, children for validated fields"*** — attributes are bare typed values; a field with its own validators/modifiers/render/instances is a CHILD declaration. Promotion attribute→child is mechanical (compiler hint + `promote`).
5. **Q5 RULED (S430, bryan): *"1 and 2 yes, park 3 as its own question"*** — `@name` in logic is the SHARED instance's value (V5-strict survives; `<swatch>`-in-logic folds into `@swatch`); a markup instance is addressable from logic only when bound with `as=` (`<dropdown as=country …/>` → `@country`).
5b. **NEW Q6 (parked by bryan S430): how does an instance write its OWN state** (`toggle()` flipping `open` inside its `renders`) through the ruled tracked / exhaustive / typed write contract? PA framing: a declaration may carry its transitions like an engine (`open` changes only through declared rules) — the point where this model and engines CONVERGE; must reconcile with Move 20 / S178 (engines singleton, components multi-instance) and `E-COMPONENT-ENGINE-SCOPE`.
6. ~~The retirement pole~~ — **CLOSED by bryan S430** (*"retirement pole closed"*): Q1–Q5 justify `<>`.

**Deliverable per question:** the options, each as FULL worked programs (an app page, a component
library consumed 3×, a theme library), the Move-20 / §15 / §65 consequences, and a PA lean with
confidence. bryan rules each before the next opens.

## [dpa-051] deep-dive — the bootstrap codegen architecture (design BEFORE code)
`status:    banked`  # S430 2026-09-24. DEPENDS ON dpa-050 Q6 (what a declaration compiles to).
banked:     S430 2026-09-24 (bryan ratified the plan: "yes, hand it to the dPA …" — the plan named this doc)

Scope + evidence: see the table row. Deliverable: an architecture a scrml-authored codegen can be built against,
