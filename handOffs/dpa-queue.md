...
`status:    complete`  # COMPLETE dPA 2026-09-19 (ADVISORY) — NOT ratified. Artifact: `scrml-support/docs/deep-dives/bare-assignment-const-and-unused-binding-dpa-047-2026-09-19.md`. **VERDICT: CALL 2 IS NOT A DESIGN QUESTION — §50.8.5 ALREADY mandates `E-ASSIGN-004` at STATEMENT position (*"applies equally to statement-form… and expression-form"*), with bryan's own remediation text already written (*"Use `let` if the variable needs to be updated"*) — and it has NEVER fired. scrml ships a crashing binary at exit 0 for that exact shape (exit 0 → invalid emit → runtime throw; `node --check` passes).** ★★ **The asymmetry settles ruling 1:** §50.8.4 contains the word "statement" ZERO times and is expression-scoped in SIX places; §50.8.5 extends explicitly. §50 knew how to extend a code to statement form, did it ONCE, and pointedly did not for the sibling — so ruling 1 IS a widening past its own governing sentence, as the item suspected, now with a reason. ★★ **Call 1 remains UNRULED:** SPEC is SILENT on statement-position semantics; the only reaching text (§7.2, "all valid JS is valid") CONTRADICTS §50.8.4's Note; shipped semantics exist only in a parser comment (`ast-builder.js:9931`). A deep 11-corpus ledger sweep found NO prior ruling — but found that bryan's S422 recollection conflicts with **HIS OWN S418 ruling 1 three days earlier** (directly opposed on the read-only case), and the middle option he recalls was never on either ballot. ⚑ `user-voice-scrml.md` ends at S421 — there is NO S422 section. ★★ **Call 3 MEASURED at last (the item's mandate, by compiling not scanning):** 195 `let`/`const` bindings in 138 files as a **FLOOR** (7,100 compiles, faithfulness proven 2551/2553); **≥15.4% of those rejections are PROVABLY WRONG** (deletion probe); densest in `examples/` at 21.3% — NOT throwaway fixtures — `conformance/` least affected at 2.5%; and **two thirds of the exposure (3798/5645) is invisible today**, sitting in the non-compiling self-host tree = a ~3x tail. The `_{}` blast radius measures **ZERO**, but THREE other blind spots are live (`?{}` SQL interpolation 34/195 · `each in=` 20/195 · C-style `for`-header) and they are **ONE mechanism** — reads reachable only through a NESTED CONTAINER, invisible to a fixed top-level field list. So "fix the `_{}` opacity" is narrower than the precondition. ★★ **The item's opt-out reasoning INVERTS:** precedent says refuse the project-wide FLAG, grant the per-site TOKEN — which RECONCILES S422's widening with S418's already-ratified `_`-prefix instead of pitting them against each other. S418's "INERT — nothing migrates" no longer holds. ★★ Bonus: the WHOLE `E-ASSIGN` family has zero producers in `compiler/src`, and THREE sample fixtures assert codes that never fire (a test-oracle hole — incl. `081`, bryan's exact shape). `E-MU-001` appears exactly ONCE in all of SPEC.md. The `_`-prefix opt-out does not exist and its suggestion RECURSES.
...
`status:    complete`  # COMPLETE dPA 2026-09-19 (ADVISORY) — NOT ratified. AXIOM-ADJACENT (§32), ladder row 7 — **MUST NOT be ratified in the same pass as dpa-047 (no-batch-axioms floor).** Artifact: `scrml-support/docs/deep-dives/anonymous-typed-binding-tilde-readback-dpa-048-2026-09-19.md`. **VERDICT: BOTH of the item's "why it composes" arguments FAIL on verification.** (i) §32.2:19398 says, normatively, *"A variable declaration (`let x = expr`, `const x = expr`) **SHALL NOT** initialize `~`"* — `:int = expr` IS a declaration, so the proposal needs a THIRD `~`-init source or a carve-out from a standing SHALL NOT, i.e. an **amendment**, not a free composition. (ii) The S397 axiom the item quotes (*"`~` is ONE thing…"*) **is not SPEC text at all** (the only `"ONE thing"` hit is §51.0.S, on engine transitions). ★★ **The shape does not compile** — `E-CODEGEN-INVALID-LOGIC` at **CG** (TAB+TS ACCEPT it), emitting `: int = args . length;` verbatim, telling the author to *report a compiler bug* with **no source line/col**; `W-LINT-011` also misreads `:int =` as a Vue-style `:attr=`. ★★ **The worked example is legal in exactly ONE program shape and it is the shape where `~` never works:** a bare `_{}` is rejected outright in a `${ }` block (`E-FOREIGN-004`), so it is `kind="tool"`-only — and `kind="tool"` programs **never allocate a `~` slot** (4 independent shapes). `~` resolves on the markup CLIENT path and nowhere else. ★★ **`~` inside `_{}` is emitted VERBATIM into JS**, and a `_{}` that does not mention `~` AT ALL still **destroys the slot for its siblings** (it escalates its function to the server path; measured per-function-body). ★★ **MECHANISM (the decisive finding):** a `_{}` interior resolves a named binding by **LEXICAL COINCIDENCE** — codegen preserves the author's name and copies the interior verbatim, with no substitution step. An anonymous value has **no emitted name to coincide with**, so the very passthrough that makes named bindings readable there is what makes `~` impossible there. **Anonymity and verbatim opacity are structurally incompatible** — so the item's headline "unused-binding vanishes by construction" is DEFEATED in the exact position the sketch uses it. ★★ **§53.14.4 gate 2 (synonym) is the live call: a cheaper baseline already exists** — orchestrator compiled AND RAN bryan's example with **NO BINDING AT ALL** (`_{ console.log("args: " + args.length) }`), exit 0, prints `args: 3`. The proposal must beat **INLINING**, not naming. The frequency number that decides it (how often a throwaway cannot be inlined) is UNMEASURED and is what a ruling turns on. ★★ **dpa-040 overlap (the item asked): it CUTS AGAINST.** Its substructural pole holds *"`~` is not an anonymous linear binding… the canon contains no category of 'anonymous linear value with a live range'"* — the proposal would CREATE that category and generalize the contested side of the dpa-040 fork from arm bodies to a general declaration form, while the checker that would police it fires **0/6**. Its one costed precedent (Rust's anonymous temporaries) is CAUTIONARY — most confusing corner of the language, real deadlocks, needed an **edition boundary** to repair in Rust 2024. ★★ **dpa-040's own artifact is partly STALE:** its 0/6 code-fire claim HOLDS, but its "all six compile exit 0" framing is now **3/6** (the fail-closed floor it recommended has landed), its "`-neg` case structurally impossible" claim is half-superseded, and its front-matter claim that §32.2.1 *"is NOT in the tree"* is WRONG at HEAD — §32.2.1 was re-authored in the tree while dpa-040 was never ruled. ★★ **NEW DEFECT (in no queue item):** SPEC §17.6.6's OWN verbatim example (`if (@cond) lift 5;` then `const dbl = ~ * 2`) compiles at **exit 0 with ZERO diagnostics** and yields `ReferenceError: _scrml_tilde_2 is not defined` — the block-scope-escape class the S397 note cites as the REASON the READ half was reverted, still reachable at HEAD via a partial-if (not an arm body). dpa-040 lists this generalization as its *single highest-value follow-up probe* and records it NOT EXECUTED. ⚑ Verified under accessor stubs, NOT a browser — a browser run is owed before filing. ★★ Cost: **Class C ~105-255h**, and the SYNTAX is the cheap part — limbs (b) scope/lifetime and (c) the reverted all-or-nothing pre-scan (60-150h, budget ~3 review rounds per the S397 arc) dominate, and both are debt already owed. ⚑ Also: there is **no §0 in SPEC.md** — the banner both items cite lives inside §32.2.1.
