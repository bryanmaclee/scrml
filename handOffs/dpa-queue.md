...
| dpa-035 | **BANKED — UNRUN.** S352 bryan: *"the 16k budget is, and always was, about making it easy for the dev to create a snappy and responsive UX. if we go over budget, then it is about being as smart as we can. have we looked into 'above the fold' preferencing?"* **The reframe is the item:** 16 KB is a PROXY for perceived responsiveness, not the goal — so byte-shaving attacks the proxy while the fold question attacks the metric. **PA-measured, zero prior art:** `above.the.fold` = 0 in `SPEC.md` (37,152 L), 0 in `known-gaps.md`, 0 in `design-insights.md`, 0 across every deep-dive; `IntersectionObserver` = 0 in `compiler/src` and SPEC; no partial/lazy/progressive-hydration vocabulary anywhere. The perf-feel debate (`perf-feel-debate-plan-2026-05-11.md`) put Approaches A/B/C/D on the table and **all four are REACHABILITY axes**; its own "deliberately does NOT decide" list does not exclude viewport — it was never raised. **§40.9.7 splits on navigation DEPTH** (`initial_chunk` N=0 → tier-1 idle → tier-2 hover → tier-N on-demand): *how many clicks away?* Above-the-fold asks the orthogonal *can the user SEE this now?* — nothing asks it. **The hard part, stated honestly: the fold is NOT statically knowable** (viewport height, zoom, font size, device, content length are all runtime), which is why the field solves it with runtime observers or heuristic extractors. **★ The scrml-specific opening — a STATE-shaped fold that IS statically computable, PA-verified by execution:** a 4-arm `<engine>` sitting in `.Idle` ships all three non-initial arm bodies in `probe.client.js`, the initial bundle (markers `ZZBELOWFOLD_*` all present). Non-initial arms are, by construction, markup for a state the user is not in — the state machine already declares what is showing, so no new primitive, no author annotation, no `IntersectionObserver`. Same for `<match>` arms and `if=`-gated subtrees. Composes with Pillar 5b / Rule 6 (state-primacy) and passes the S322 by-construction-not-retrofit test. **Two more fall out:** §65's `#{}` is component-scoped so critical-CSS is available BY CONSTRUCTION where Critical/Penthouse guess; and the §40 reachability solver + AuthGraph already exist, so this is a new PARTITION over built machinery, not new infrastructure. **⚑ The real trade-off a dive must size:** shipping every arm eagerly is exactly what makes a variant switch instant — deferring trades switch latency for boot latency. Not a free win. **An author-declared `defer`/`priority=` attribute is the shape to beat, not adopt** — a forgettable declaration guarding a perf property, the shape §14.8.11.2 rejected for security invariants. Surfaces touched: §40.9 (splitter) · §51 (engines) · §65 (CSS) · SSR. | S352 bryan bank — **R2, UNRUN**; PA rec: fire the dPA |
