...
2. **`g-engine-server-flag-silent-swallow`** `[landed-on-branch]` (agent 85a86194; NEW §34 W-ENGINE-SERVER-DEFERRED row — SPEC-touch to flag PA; +13 within-node re-baselines; native-parser server-family residual) bug MED · tier med — `<engine ... server ...>` silently swallows the bare `server` flag (no =@source); should fire a new `W-ENGINE-SERVER-DEFERRED` lint. S199 E-leg `server=@source` IS wired. Entry: symbol-table.ts(serverSource :5204) + emit-engine.ts:1791.
