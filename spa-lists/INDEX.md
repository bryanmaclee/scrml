...
|----|-----------|---|-----------|------------------------------|-------------|
| **21** | **render-expr-codegen** | 5 | emit-lift · emit-html · emit-expr · errors-emitter/runtime | render/expr emit residuals (ss17/ss20 deferred): tableFor-col-interp · if-chain-null · request-lift-`if` · unary-`**`-paren · errors-anchor-reactive-clear | **NEW S221 · fireable** |
| **22** | **server-fn-pipeline** | 6 | type-system(server-fn) · emit-server · rewrite · emit-client · route-inference | server-fn typer-scope + server-side lowering: sse-route-typer · obj-literal-return · ecg001-overfire · peer-in-template · enum-toEnum-server · sql-row-protect | **NEW S221 · fireable** (serialize w/ ss26) |
| **23** | **library-mode-foreign-codegen** | 3 | route-inference · emit-server · db-driver · module-resolver · `_{}`-emitter | non-`<program>` codegen: safecall-`!{}`-in-library · `?{}`-db-context W5a/W5b (big) · `_{}`-crossing-shadow. flogence first consumer | **NEW S221 · fireable** |
...
| **25** | **ast-builder-parse-diagnostics** | 3 | ast-builder · block-splitter · engine-statechild-parser | legacy front-end: control-flow-`lift`-evades-diag · inline-struct-return-misparse · bug-75 after-`>` engine-shorthand | **NEW S221 · fireable** |
...
| **27** | **low-ingestion-cleanup** | 5 | (per-item disjoint) | LOW sweep (minimal ingestion): bug-19-cite · §52-doc-staleness · trucking-smoke-flake · stdlib-chunk-dead-weight · channel-migrate-autostrip | **NEW S221 · fireable** |
| **28** | **native-parser-markup-tokenization** | 3 | native-parser/* · meta-eval · markup-comment-scanner | ⚠ **HAZARD** (native-parser + conformance): markup-comment-`<tag>` · nominal-3 quoted-text-fire · rails-dev-hang | **NEW S221 · specialist** |
