...
| **ss36** | tuples (NEW feature, RATIFIED S222) | 1 | ✅ **FIREABLE — survey-first** | parser→typer→codegen (literal/type/destructure); clean multi-return; Road-B mechanics-beauty prereq |
| **ss37** | value-native Set (§59.12 un-defer) | 1 | ✅ **FIREABLE — survey-first** | §59 Map machinery as template; the DG-builder exerciser; Road-B mechanics-beauty prereq |

**REBUILT S222 (ruling addendum):** the parser-fork ruled **narrow-Road-B** (re-imagine the compiler in scrml; `docs/changes/compiler-reimagining-derisk-2026-06-26/RULING.md`). Two new lanes minted for the mechanics-beauty prereqs (ss36 tuples · ss37 Set); ss35 reframed HELD→FROZEN (native-parser superseded-pending-rewrite, fix only adopter-blockers).

**Recommended fire order:** ss31 (running) · ss32 · ss33 · **ss36 · ss37** are disjoint-surface → **safe to fire in parallel**. ss34 survey-first (SSR). ss30-continuation (W3 RS-solver) fireable. ss35 FROZEN (except the hang). **Bucket B (design-track → PA/dPA):** **FBIP feasibility (firing, agent a7e40eed — the Road-B crux)**; the W4 chunk-model debate (`dpa-queue` [dpa-014]); the scrml-native scanning stdlib (sequences with the lexer build); bug-1 @apply; `g-library-mode-sql-no-db-context` (W5b); `g-sql-row-protect-leak`. **The Road-B program** (lexer→parser→typer→codegen in scrml) sequences after the prereqs + FBIP — see RULING.md. **Deferred-parked:** a5, bug-14, bug-20, bug-21, bug-22, r28-c2, r28-2b, g-const-collides-state.
