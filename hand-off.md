...
> ⚠️ **One evidence caveat, recorded because it was cited as fact.** The reviewer's runtime truncation-detector result is sound but its **coverage is inferred, not proven per-file**. PA-verified the artifacts directly: the original run's `trunc-wt.json` is 3 bytes (`[]`) with an empty `.err` — and that string is only written after the full loop, so the run did complete with zero hits. The reviewer then re-ran it *specifically to prove per-file coverage*, and **that rerun produced a 0-byte `trunc-wt-all.json`** — by its own stated logic, the signature of a mid-loop kill. So the rerun did NOT complete and the per-file accounting was never established.
>
> **Why this does not change the verdict:** the detector was validated against a known-truncating build first (base-compiled `p1` fired 2 hits), so it is proven non-vacuous — it fires when truncation is present. And it is a *corroborating* gate, not the primary evidence: the landing rests on byte-identity 0 diffs, conformance 746/746, the root-2 conformance case that hard-fails pre-fix, and pre/post real-Chrome execution reproducing 4/0 → 4/4. Treat "zero truncation events across 62 programs" as **"the completed run found zero, on unproven-but-likely-complete coverage"** rather than a per-file guarantee. The `pa-base v2.4` unproven-gate rule is what makes the falsification check the load-bearing half here.
