...
1. **A2 — W2 (parser) IN FLIGHT** (agent **a0761f89e7066e52a**, isolation:worktree; user "go"). W0 (F1=A) + W1 (SPEC §60, `4c740ec5`) DONE. W2 scope: BS+ast-builder recognize `<api src= base=>` + endpoint decls → `api-decl` AST node + parse-codes E-API-BASE-MISSING/METHOD-INVALID/RESPONSE-TYPE-UNDECLARED/ENDPOINT-MALFORMED (+§34 rows); NO codegen/emission; §60 banner stays Nominal. **PA lands via S67 file-delta on completion** (verify tip==reported, disjoint, no leak; gap-reconcile; full suite). BRIEF-W2.md archived. delta-log [25]. **Then W3 (typer: ENDPOINT-UNKNOWN/REQ-SHAPE-MISMATCH/PATH-PARAM-UNBOUND) → W4 (codegen: `<request api=>` + parseVariant wiring) → W5 (tests + example + B-docs guide).**
